# Cucumber Best Practices For Crew E2E

Use this reference when writing or reviewing Gherkin scenarios, step definitions, parameter expressions, and step reuse in Crew's `e2e/` suite.
...
- type `this` as `CrewWorld`
- use `async function`
- keep each step to one user-visible action or assertion
- rely on `CrewWorld` and existing support code for shared context
