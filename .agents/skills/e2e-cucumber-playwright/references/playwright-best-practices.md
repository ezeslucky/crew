# Playwright Best Practices For Crew E2E

Use this reference when writing or reviewing locator, assertion, isolation, or synchronization logic for Crew's Cucumber-based E2E suite.
...
Playwright's model is built around clean browser contexts so one test does not leak into another. In Crew's suite, that principle maps to per-scenario session setup in `features/support/hooks.ts` and `CrewWorld`.

Apply it like this:

- do not depend on another scenario having run first
- do not persist ad hoc scenario state outside `CrewWorld`
...
Playwright's wider ecosystem supports traces and rich debugging tools. Crew's current suite already captures:
...
- Is a new abstraction really needed, or does it bypass the existing `CrewWorld` + step-definition model?
