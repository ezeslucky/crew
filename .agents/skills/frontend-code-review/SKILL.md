...
description: Review Crew frontend code for correctness, accessibility, component design, crew-ui usage, data/query boundaries, performance, and tests. Trigger for `.tsx`, `.ts`, `.js`, UI, React, Next.js, pending-change, or focused frontend review requests.
---
...
Use this skill when the user asks to review, audit, analyze, or sanity-check frontend code under `web/`, `packages/crew-ui/`, or frontend-adjacent TypeScript files.
...
- `web/AGENTS.md` for Crew frontend workflow, overlays, design tokens, state, and tests.
- `packages/crew-ui/README.md` and `packages/crew-ui/AGENTS.md` when code uses or changes `@crew/crew-ui/*`.
...
- [references/accessibility-ui.md](references/accessibility-ui.md) — accessibility, semantic HTML, focus, forms, keyboard, disabled states, copy, and long-content behavior. Combines Web Interface Guidelines with Crew UI, Base UI, MDN, and local primitive contracts.
- [references/crew-ui.md](references/crew-ui.md) — Crew UI primitive usage, Base UI semantics, overlays, forms, tokens, radius mapping, and primitive boundaries.
...
- [references/crew-invariants.md](references/crew-invariants.md) — stable Crew-specific runtime invariants that generic React/a11y rules will not catch.
