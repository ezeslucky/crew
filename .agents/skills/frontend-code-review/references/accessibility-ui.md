...
- `packages/crew-ui/README.md`, `packages/crew-ui/AGENTS.md`, and the relevant primitive implementation when code uses `@crew/crew-ui/*`.
...
The usual Crew layout chain is: container has width constraints, text region uses `min-w-0 flex-1 truncate`, adornments use `shrink-0`.
