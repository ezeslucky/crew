...
- New CSS modules or ad hoc CSS when Tailwind utilities and Crew tokens cover the need.
...
- Generic color utilities where Crew semantic tokens exist.
- Hardcoded magic class values for colors, spacing, radius, shadow, z-index, or typography when Crew tokens, component variants, or documented radius mappings exist.
...
- JS conditional class branches for primitive visual states already exposed by Crew UI/Base UI `data-*` selectors.
...
- Crew semantic tokens and Tailwind v4 utilities.
...
- Barrel imports from `@crew/crew-ui`; consumers must use subpath exports.
