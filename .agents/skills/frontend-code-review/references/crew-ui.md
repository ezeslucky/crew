...
Use these rules whenever a review touches `packages/crew-ui/` or code consuming `@crew/crew-ui/*`.

Before finalizing findings for those files, read the current local docs that apply:

- `packages/crew-ui/README.md`
- `packages/crew-ui/AGENTS.md`
- `web/docs/overlay.md` for floating UI
- `packages/crew-ui/src/<primitive>/index.tsx` for the primitive being changed or consumed

## Package Boundary

Flag in `packages/crew-ui`:
...
`packages/crew-ui` is a primitive layer: Base UI headless components + `cva` + `cn` + Crew design tokens.

## Imports And Exports

Flag:

- Consumer imports from `@crew/crew-ui` without a subpath.
...
Consumers use subpath exports such as `@crew/crew-ui/button`.
...
- JavaScript conditional class logic for visual states that the Crew UI/Base UI primitive already exposes through `data-*` attributes or CSS variables.
...
- Text-like fields not composed through `Field`, `FieldLabel`, and `FieldControl` when using Crew UI form semantics.
...
`Form` is the submit boundary. Crew UI form primitives are not a form state-management framework; business validation and schema-driven behavior belong in `web/`.
...
- Manual portals around Crew UI overlay primitives.
...
All Crew UI body-portalled overlays use `z-50`. Toast uses `z-60`. DOM order handles stacking between overlays.
...
- Manually recreating UI behavior or chrome already owned by `@crew/crew-ui/*` or `web/app/components/base/*`, such as buttons, inputs, toggle groups, popovers, dropdown menus, alert dialogs, switches, avatars, scroll areas, toasts, borders, focus states, disabled states, segmented controls, or existing feature components.
- Styling a raw Base UI primitive directly in `web/` when a Crew UI primitive exists.
- Wrapping a Crew UI primitive in a feature component that hides its label, error, disabled, or focus contract.
...
- Using crew-ui `Input` as a drop-in replacement for legacy inputs that include search, clear, copy, unit, localized placeholder, or number normalization behavior.
...
- Generic colors where semantic Crew tokens exist.
- Hardcoded design values where Crew tokens, component variants, or documented Figma radius mappings exist.
...
Use the Figma radius mapping from `packages/crew-ui/AGENTS.md`; for example `--radius/sm` maps to `rounded-md`, and `--radius/md` maps to `rounded-lg`.
...
New custom SVG icons belong in `packages/iconify-collections/assets/...`. Regenerate with `pnpm --filter @crew/iconify-collections generate`, validate with `pnpm --filter @crew/iconify-collections check:dimensions`, and consume the generated icon with Tailwind `i-custom-*` classes.
