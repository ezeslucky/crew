...
description: Write, update, or review Crew frontend tests using Vitest and Testing Library. Trigger for frontend specs, test coverage requests, regressions, testability, or testing strategy under web/ or packages/crew-ui/.
---

# Crew Frontend Testing

Use this skill for Vitest work under `web/` and `packages/Crew-ui/`. Do not use it for Python tests or Cucumber/Playwright tests under `e2e/`.
...
Run Crew UI Storybook tests with `vp test --project storybook --run`. Run broader checks only after the focused behavior passes.
