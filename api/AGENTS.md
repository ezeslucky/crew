...
class TriggerConfig(BaseModel):
    ...
    def ensure_secret_prefix(cls, value: str) -> str:
        if not value.startswith("crew_"):
            raise ValueError("secret must start with crew_")
        return value
```
...
- Use `configs.crew_config` for configuration—never read environment variables directly.
