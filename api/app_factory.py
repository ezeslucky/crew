...
# ----------------------------
def create_flask_app_with_configs() -> CrewApp:
    """
    create a raw flask app
    with configs loaded from .env file
    """
    crew_app = CrewApp(__name__)
    crew_app.config.from_mapping(crew_config.model_dump())
    crew_app.config["RESTX_INCLUDE_ALL_MODELS"] = True

    # add before request hook
    @crew_app.before_request
    def before_request():
...
def create_app() -> tuple[socketio.WSGIApp, CrewApp]:
...
def initialize_extensions(app: CrewApp):
...
def create_migrations_app() -> CrewApp:
    app = create_flask_app_with_configs()
