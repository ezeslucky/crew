"""API-side errors for the Crew Agent backend integration.

The wire protocol and low-level HTTP behaviour are owned by ``crew-agent``.
...
class AgentBackendRequestBuildError(AgentBackendError):
    """Raised when Crew product/workflow state cannot be mapped to a run request."""
