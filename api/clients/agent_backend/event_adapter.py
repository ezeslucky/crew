"""Adapt public ``crew-agent`` run events into API-internal event semantics.

The adapter does not define a new cross-service event contract. It consumes
``crew_agent.protocol.RunEvent`` and produces small API-internal models that the
workflow Agent Node maps to Graphon/AppQueue events. Deferred external tool calls
remain Crew Agent ``run_succeeded`` payloads on the wire; API code turns them
...
class AgentBackendInternalEventBase(BaseModel):
    """Common fields preserved from public Crew Agent run events."""
...
class AgentBackendDeferredToolCallInternalEvent(AgentBackendInternalEventBase):
    """API-internal representation of a Crew Agent deferred external tool call."""
...
class AgentBackendRunEventAdapter:
    """Maps public ``crew-agent`` event variants to API-internal event variants."""
