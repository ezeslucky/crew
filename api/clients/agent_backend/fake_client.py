"""Deterministic fake Agent backend client using public ``crew-agent`` events.
...
class FakeAgentBackendScenario(StrEnum):
    ...
    ``PAUSED`` represents the API workflow effect. On the Crew Agent wire
                    ...
                    RunSucceededEvent(
                        ...
                        data=RunSucceededEventData(
                            deferred_tool_call=DeferredToolCallPayload(
                                ...
                                metadata={"layer_type": "crew.ask_human", "schema_version": 1},
                            ),
                            session_snapshot=CompositorSessionSnapshot(layers=[]),
                        ),
                    ),
