"""Build ``crew-agent`` run requests from API-side product concepts.

This module is intentionally an adapter, not a wire DTO package. The emitted
object is always ``crew_agent.protocol.CreateRunRequest`` so the Agent backend
...
schemas land in later phases. Crew-owned execution identifiers are emitted as an
explicit ``crew.execution_context`` layer so the run request stays fully
composition-driven.
"""
...
from crew_agent.layers.ask_human import CREW_ASK_HUMAN_LAYER_TYPE_ID, CrewAskHumanLayerConfig
from crew_agent.layers.config import CREW_CONFIG_LAYER_TYPE_ID, CrewConfigLayerConfig
from crew_agent.layers.crew_core_tools import CREW_CORE_TOOLS_LAYER_TYPE_ID, CrewCoreToolsLayerConfig
from crew_agent.layers.crew_plugin import (
    CREW_PLUGIN_LLM_LAYER_TYPE_ID,
    CREW_PLUGIN_TOOLS_LAYER_TYPE_ID,
    CrewPluginCredentialValue,
    CrewPluginLLMLayerConfig,
    CrewPluginToolsLayerConfig,
)
from crew_agent.layers.drive import CREW_DRIVE_LAYER_TYPE_ID, CrewDriveLayerConfig
from crew_agent.layers.execution_context import (
    CREW_EXECUTION_CONTEXT_LAYER_TYPE_ID,
    CrewExecutionContextLayerConfig,
)
from crew_agent.layers.knowledge import CREW_KNOWLEDGE_BASE_LAYER_TYPE_ID, CrewKnowledgeBaseLayerConfig
from crew_agent.layers.output import CREW_OUTPUT_LAYER_TYPE_ID, CrewOutputLayerConfig
from crew_agent.layers.shell import CREW_SHELL_LAYER_TYPE_ID, CrewShellLayerConfig
from crew_agent.protocol import (
    CREW_AGENT_HISTORY_LAYER_ID,
    CREW_AGENT_MODEL_LAYER_ID,
    CREW_AGENT_OUTPUT_LAYER_ID,
    CreateRunRequest,
...
CREW_EXECUTION_CONTEXT_LAYER_ID = "execution_context"
CREW_CONFIG_LAYER_ID = "config"
CREW_DRIVE_LAYER_ID = "drive"
CREW_PLUGIN_TOOLS_LAYER_ID = "tools"
CREW_CORE_TOOLS_LAYER_ID = "core_tools"
CREW_KNOWLEDGE_BASE_LAYER_ID = "knowledge"
CREW_ASK_HUMAN_LAYER_ID = "ask_human"
CREW_SHELL_LAYER_ID = "shell"
...
def _shell_layer_deps() -> dict[str, str]:
    return {"execution_context": CREW_EXECUTION_CONTEXT_LAYER_ID}


def _drive_layer_deps() -> dict[str, str]:
    return {"shell": CREW_SHELL_LAYER_ID}
...
def _shell_config_with_drive_ref(
    shell_config: CrewShellLayerConfig | None,
    drive_config: CrewDriveLayerConfig | None,
) -> CrewShellLayerConfig:
    config = shell_config or CrewShellLayerConfig()
...
- Use the installed `crew-agent` CLI when you need to inspect or persist Agent configuration."""
...
class AgentBackendModelConfig(BaseModel):
    """API-side model/plugin selection before it is converted to Crew Agent layers."""

    plugin_id: str
    model_provider: str
    model: str
    credentials: dict[str, CrewPluginCredentialValue] = Field(default_factory=dict)
    model_settings: dict[str, JsonValue] = Field(default_factory=dict)

    model_config: ClassVar[ConfigDict] = ConfigDict(extra="forbid")


# ``CrewPluginLLMLayerConfig.model_settings`` is pydantic_ai's ``ModelSettings``
...
class AgentBackendOutputConfig(BaseModel):
    ...
    ``crew_agent.layers.output`` so callers only control the JSON Schema plus
...
class AgentBackendWorkflowNodeRunInput(BaseModel):
    ...
    tools: CrewPluginToolsLayerConfig | None = None
    core_tools: CrewCoreToolsLayerConfig | None = None
    knowledge: CrewKnowledgeBaseLayerConfig | None = None
    config_layer_config: CrewConfigLayerConfig | None = None
    # Drive Skills & Files declaration (crew.drive) — an index the agent pulls
    # through the back proxy, never inline content.
    drive_config: CrewDriveLayerConfig | None = None
    # Human-in-the-loop ask_human deferred tool (crew.ask_human). Present only when
    ...
    ask_human_config: CrewAskHumanLayerConfig | None = None
    # Inject the sandboxed shell layer (crew.shell). Requires the agent backend
    # to be wired with a shellctl entrypoint; see configs AGENT_SHELL_ENABLED.
    include_shell: bool = False
    shell_config: CrewShellLayerConfig | None = None
...
class AgentBackendRunRequestBuilder:
    """Converts API product state into the public ``crew-agent`` run protocol."""
    ...
    def build_for_agent_app(self, run_input: AgentBackendAgentAppRunInput) -> CreateRunRequest:
        ...
        layers.extend(
            [
                ...
                RunLayerSpec(
                    name=CREW_EXECUTION_CONTEXT_LAYER_ID,
                    type=CREW_EXECUTION_CONTEXT_LAYER_TYPE_ID,
        ...
        if include_shell:
            # Sandboxed bash workspace (crew.shell). It enters before config/drive
            # so eager pulls materialize content in the same filesystem used by
            # model commands.
            layers.append(
                RunLayerSpec(
                    name=CREW_SHELL_LAYER_ID,
                    type=CREW_SHELL_LAYER_TYPE_ID,
        ...
        if run_input.config_layer_config is not None:
            layers.append(
                RunLayerSpec(
                    name=CREW_CONFIG_LAYER_ID,
                    type=CREW_CONFIG_LAYER_TYPE_ID,
        ...
        if run_input.drive_config is not None:
            # Drive Skills & Files declaration (crew.drive): the catalog plus
            # prompt-mentioned entries eagerly pulled through the shell layer.
            layers.append(
                RunLayerSpec(
                    name=CREW_DRIVE_LAYER_ID,
                    type=CREW_DRIVE_LAYER_TYPE_ID,
        ...
        if run_input.include_history:
            layers.append(
                RunLayerSpec(
                    name=CREW_AGENT_HISTORY_LAYER_ID,
        ...
        layers.append(
            RunLayerSpec(
                name=CREW_AGENT_MODEL_LAYER_ID,
                type=CREW_PLUGIN_LLM_LAYER_TYPE_ID,
                deps={"execution_context": CREW_EXECUTION_CONTEXT_LAYER_ID},
                metadata=run_input.metadata,
                config=CrewPluginLLMLayerConfig(
        ...
        if run_input.tools is not None and run_input.tools.tools:
            plugin_tool_deps = {"execution_context": CREW_EXECUTION_CONTEXT_LAYER_ID}
            if include_shell:
                plugin_tool_deps["shell"] = CREW_SHELL_LAYER_ID
            layers.append(
                RunLayerSpec(
                    name=CREW_PLUGIN_TOOLS_LAYER_ID,
                    type=CREW_PLUGIN_TOOLS_LAYER_TYPE_ID,
        ...
        if run_input.core_tools is not None and run_input.core_tools.tools:
            layers.append(
                RunLayerSpec(
                    name=CREW_CORE_TOOLS_LAYER_ID,
                    type=CREW_CORE_TOOLS_LAYER_TYPE_ID,
                    deps={"execution_context": CREW_EXECUTION_CONTEXT_LAYER_ID},
                    metadata=run_input.metadata,
        ...
        if run_input.knowledge is not None and run_input.knowledge.sets:
            layers.append(
                RunLayerSpec(
                    name=CREW_KNOWLEDGE_BASE_LAYER_ID,
                    type=CREW_KNOWLEDGE_BASE_LAYER_TYPE_ID,
                    deps={"execution_context": CREW_EXECUTION_CONTEXT_LAYER_ID},
                    metadata=run_input.metadata,
        ...
        if run_input.ask_human_config is not None:
            # Human-in-the-loop ask_human deferred tool (crew.ask_human). A call ends
            ...
            layers.append(
                RunLayerSpec(
                    name=CREW_ASK_HUMAN_LAYER_ID,
                    type=CREW_ASK_HUMAN_LAYER_TYPE_ID,
                    metadata=run_input.metadata,
        ...
        if run_input.output is not None:
            layers.append(
                RunLayerSpec(
                    name=CREW_AGENT_OUTPUT_LAYER_ID,
                    type=CREW_OUTPUT_LAYER_TYPE_ID,
                    metadata=run_input.metadata,
                    config=CrewOutputLayerConfig(
    ...
    def build_cleanup_request(
        ...
        *,
        ...
        (``crew.plugin.llm``, ``crew.plugin.tools``) are excluded from both the
    ...
    def build_for_workflow_node(self, run_input: AgentBackendWorkflowNodeRunInput) -> CreateRunRequest:
        ...
        if include_shell:
            # Sandboxed bash workspace (crew.shell). It enters before drive so
            # drive can materialize mentioned targets with `crew-agent drive pull`
