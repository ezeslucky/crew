...
def migrate_knowledge_vector_database():
    ...
    skipped_count = 0
    total_count = 0
    vector_type = crew_config.VECTOR_STORE
...
def add_qdrant_index(field: str):
    ...
    try:
        ...
        from crew_vdb_qdrant.qdrant_vector import PathQdrantParams, QdrantConfig
        from qdrant_client.http.exceptions import UnexpectedResponse
        from qdrant_client.http.models import PayloadSchemaType

        for binding in bindings:
            if crew_config.QDRANT_URL is None:
                raise ValueError("Qdrant URL is required.")
            qdrant_config = QdrantConfig(
                endpoint=crew_config.QDRANT_URL,
                api_key=crew_config.QDRANT_API_KEY,
                root_path=current_app.root_path,
                timeout=crew_config.QDRANT_CLIENT_TIMEOUT,
                grpc_port=crew_config.QDRANT_GRPC_PORT,
                prefer_grpc=crew_config.QDRANT_GRPC_ENABLED,
            )
            try:
