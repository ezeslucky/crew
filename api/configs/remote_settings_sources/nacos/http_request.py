...
class NacosHttpClient:
    def __init__(self):
        self.username = os.getenv("CREW_ENV_NACOS_USERNAME")
        self.password = os.getenv("CREW_ENV_NACOS_PASSWORD")
        self.ak = os.getenv("CREW_ENV_NACOS_ACCESS_KEY")
        self.sk = os.getenv("CREW_ENV_NACOS_SECRET_KEY")
        self.server = os.getenv("CREW_ENV_NACOS_SERVER_ADDR", "localhost:8848")
        ...
        self.timeout = httpx.Timeout(
            float(os.getenv("CREW_ENV_NACOS_REQUEST_TIMEOUT", "10.0")),
            connect=float(os.getenv("CREW_ENV_NACOS_CONNECT_TIMEOUT", "3.0")),
        )
    ...
    def _inject_auth_info(self, headers: dict[str, str], params: dict[str, str], module: str = "config") -> None:
        headers.update({"User-Agent": "Nacos-Http-Client-In-Crew:v0.0.1"})
