"""Global pytest hooks for Crew backend tests.
...
_CREW_COMPOSE_STACKS_KEY = pytest.StashKey[list[DockerComposeStack]]()
...
def pytest_configure(config: pytest.Config) -> None:
    config.stash[_CREW_COMPOSE_STACKS_KEY] = []
...
def pytest_sessionstart(session: pytest.Session) -> None:
    ...
    config.stash[_CREW_COMPOSE_STACKS_KEY] = stacks
...
def pytest_unconfigure(config: pytest.Config) -> None:
    ...
    stacks = config.stash.get(_CREW_COMPOSE_STACKS_KEY, [])
