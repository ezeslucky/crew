...
class WorkflowGenerateApi(Resource):
    ...
    @console_ns.doc(description="Generate a Crew workflow graph from natural language")
...
class WorkflowGenerateStreamApi(Resource):
    ...
    @console_ns.doc(description="Stream a Crew workflow graph (plan then result) via SSE")
    @console_ns.expect(console_ns.models[WorkflowGeneratePayload.__name__])
    @console_ns.response(
        200,
