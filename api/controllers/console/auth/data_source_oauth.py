...
def get_oauth_providers():
    with current_app.app_context():
        notion_oauth = NotionOAuth(
            client_id=crew_config.NOTION_CLIENT_ID or "",
            client_secret=crew_config.NOTION_CLIENT_SECRET or "",
            redirect_uri=crew_config.CONSOLE_API_URL + "/console/api/oauth/data-source/callback/notion",
        )
...
class OAuthDataSource(Resource):
    ...
    def get(self, provider: str):
        ...
        if crew_config.NOTION_INTEGRATION_TYPE == "internal":
            internal_secret = crew_config.NOTION_INTERNAL_SECRET
...
class OAuthDataSourceCallback(Resource):
    ...
    def get(self, provider: str):
        ...
        if "code" in request.args:
            ...
            return redirect(f"{crew_config.CONSOLE_WEB_URL}?type=notion&code={code}")
        elif "error" in request.args:
            error = request.args.get("error")

            return redirect(f"{crew_config.CONSOLE_WEB_URL}?type=notion&error={error}")
        else:
            return redirect(f"{crew_config.CONSOLE_WEB_URL}?type=notion&error=Access denied")
