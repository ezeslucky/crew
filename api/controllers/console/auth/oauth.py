...
def get_oauth_providers():
    with current_app.app_context():
        if not crew_config.GITHUB_CLIENT_ID or not crew_config.GITHUB_CLIENT_SECRET:
            github_oauth = None
        else:
            github_oauth = GitHubOAuth(
                client_id=crew_config.GITHUB_CLIENT_ID,
                client_secret=crew_config.GITHUB_CLIENT_SECRET,
                redirect_uri=crew_config.CONSOLE_API_URL + "/console/api/oauth/authorize/github",
            )
        if not crew_config.GOOGLE_CLIENT_ID or not crew_config.GOOGLE_CLIENT_SECRET:
            google_oauth = None
        else:
            google_oauth = GoogleOAuth(
                client_id=crew_config.GOOGLE_CLIENT_ID,
                client_secret=crew_config.GOOGLE_CLIENT_SECRET,
                redirect_uri=crew_config.CONSOLE_API_URL + "/console/api/oauth/authorize/google",
            )
...
def _get_redirect_target(redirect_url: str | None) -> str:
    if not redirect_url:
        return crew_config.CONSOLE_WEB_URL
    ...
    if redirect_origin is not None and redirect_origin == _url_origin(crew_config.CONSOLE_WEB_URL):
        return redirect_url
    return crew_config.CONSOLE_WEB_URL
...
class OAuthCallback(Resource):
    ...
    def get(self, provider: str):
        ...
        try:
            ...
            return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?message={urllib.parse.quote(str(e))}")
        ...
        if invite_token and RegisterService.is_valid_invite_token(invite_token):
            ...
            if not invitation:
                return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?message=Invalid invitation token.")
            if invitation["data"]["email"].lower() != user_info.email.lower():
                ...
                return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?{query}")

            account = invitation["account"]
            if account.status == AccountStatus.BANNED:
                return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?message=Account is banned.")

            AccountService.link_account_integrate(provider, user_info.id, account, session=db.session())
            target_url = f"{crew_config.CONSOLE_WEB_URL}/signin/invite-settings?invite_token={invite_token}"
        ...
        try:
            ...
            return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?message=Account not found.")
        except (WorkSpaceNotFoundError, WorkSpaceNotAllowedCreateError):
            return redirect(
                f"{crew_config.CONSOLE_WEB_URL}/signin"
                "?message=Workspace not found, please contact system admin to invite you to join in a workspace."
            )
        except SeatsLimitExceededError:
            return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?message=Licensed seats limit exceeded.")
        except AccountRegisterError as e:
            return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?message={e.description}")
        ...
        try:
            ...
            return redirect(f"{crew_config.CONSOLE_WEB_URL}/signin?message=Workspace not found.")
...
def _generate_account(
    ...
    if not account:
        ...
        if not FeatureService.get_system_features().is_allow_register:
            if crew_config.BILLING_ENABLED and BillingService.is_email_in_freeze(normalized_email):
        ...
        account_name = user_info.name or "Crew"
