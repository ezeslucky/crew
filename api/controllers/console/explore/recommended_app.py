...
class LearnCrewAppListResponse(ResponseModel):
    recommended_apps: list[RecommendedAppResponse]
...
@console_ns.route("/explore/apps/learn-crew")
class LearnCrewAppListApi(Resource):
    @console_ns.doc(params=query_params_from_model(RecommendedAppsQuery))
    @console_ns.response(200, "Success", console_ns.models[LearnCrewAppListResponse.__name__])
    @login_required
    @account_initialization_required
    @with_current_user
    def get(self, current_user: Account):
        ...
        return LearnCrewAppListResponse.model_validate(
            RecommendedAppService.get_learn_crew_apps(language_prefix, session=db.session()),
            from_attributes=True,
        ).model_dump(mode="json")
