...
These raw Blueprint routes deliberately stay outside Crew's OpenAPI surface:
KnowledgeFS owns the wire contract consumed by the frontend. The catch-all path
avoids resource-specific Crew controllers, while the forwarding module consumes
only the operations explicitly enabled by Crew's product registry. The registry
...
trigger Crew browser-session recovery; resource-level 403 responses remain 403.
"""
...
def _knowledge_fs_enabled[**P](
    ...
    @wraps(view)
    def decorated(*args: P.args, **kwargs: P.kwargs) -> ResponseReturnValue:
        if not crew_config.KNOWLEDGE_FS_ENABLED:
            raise NotFound()
        return view(*args, **kwargs)
...
def _proxy_response(
    ...
    *,
    ...
    if upstream.status_code == HTTPStatus.UNAUTHORIZED:
        upstream.close()
        logger.error(
            "KnowledgeFS rejected the Crew server credential with HTTP %s for tenant_id=%s",
