...
def _list_hardcoded_builtin_tool_providers(tenant_id: str) -> list[dict[str, Any]]:
    ...
    for provider in ToolManager.list_hardcoded_providers():
        if is_filtered(
            include_set=crew_config.POSITION_TOOL_INCLUDES_SET,
            exclude_set=crew_config.POSITION_TOOL_EXCLUDES_SET,
            data=provider,
    ...
    def get(self, tenant_id: str):
        try:
            return {
                ...
                "host": crew_config.PLUGIN_REMOTE_INSTALL_HOST,
                "port": crew_config.PLUGIN_REMOTE_INSTALL_PORT,
            }
...
class PluginIconApi(Resource):
    ...
    def get(self):
        ...
        icon_cache_max_age = crew_config.TOOL_ICON_CACHE_MAX_AGE
...
class PluginUploadFromPkgApi(Resource):
    ...
    def post(self, tenant_id: str):
        ...
        content = _read_upload_content(file, crew_config.PLUGIN_MAX_PACKAGE_SIZE)
        try:
...
class PluginUploadFromBundleApi(Resource):
    ...
    def post(self, tenant_id: str):
        ...
        content = _read_upload_content(file, crew_config.PLUGIN_MAX_BUNDLE_SIZE)
        try:
...
class PluginChangeAutoUpgradeApi(Resource):
    ...
    @with_current_tenant_id
    def post(self, tenant_id: str, user: Account):
        if not crew_config.RBAC_ENABLED and not user.is_admin_or_owner:
            raise Forbidden()
