...
def _legacy_workspace_roles(
    ...
    Crew role model when enterprise RBAC is disabled.
    """
    legacy_roles = []
    for role_name in ("owner", "admin", "editor", "normal", "dataset_operator"):
        if not crew_config.DATASET_OPERATOR_ENABLED and role_name == "dataset_operator":
            continue
        legacy_roles.append(
            svc.RBACRole(
                id=role_name,
...
class RBACAppWhitelistApi(Resource):
    ...
    def put(self, app_id):
        ...
        if crew_config.RBAC_ENABLED and request.scope is RBACResourceWhitelistScope.ALL:
