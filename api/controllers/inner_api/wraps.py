...
def inner_api_only[**P, R](view: Callable[P, R]) -> Callable[P, R]:
    ...
    def decorated(*args: P.args, **kwargs: P.kwargs) -> R:
        if not crew_config.INNER_API:
            abort(404)

        inner_api_key = request.headers.get("X-Inner-Api-Key")
        if not inner_api_key or inner_api_key != crew_config.INNER_API_KEY:
...
def plugin_inner_api_only[**P, R](view: Callable[P, R]) -> Callable[P, R]:
    @wraps(view)
    def decorated(*args: P.args, **kwargs: P.kwargs) -> R:
        if not crew_config.PLUGIN_DAEMON_KEY:
            abort(404)

        # get header 'X-Inner-Api-Key'
        inner_api_key = request.headers.get("X-Inner-Api-Key")
        if not inner_api_key or inner_api_key != crew_config.INNER_API_KEY_FOR_PLUGIN:
            abort(404)
...
def agent_inner_api_only[**P, R](view: Callable[P, R]) -> Callable[P, R]:
    ...
    `crew-agent -> Crew API` transport credentials as the plugin inner bridge.
