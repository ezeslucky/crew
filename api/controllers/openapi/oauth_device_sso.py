...
_ALLOWED_SSO_ERRORS = {"sso_failed", "email_belongs_to_crew_account"}
...
def _trusted_origin() -> str:
    base = (crew_config.CONSOLE_API_URL or "").rstrip("/")
    if not base:
...
def _sso_complete_impl():
    ...
    if AccountService.has_active_account_with_email(claims.email, session=db.session()):
        _emit_external_rejection_audit(
            ...
            reason="email_belongs_to_crew_account",
        )
        return _device_error_redirect("email_belongs_to_crew_account", user_code)
...
def approve_external():
    ...
    if AccountService.has_active_account_with_email(claims.subject_email, session=db.session()):
        _emit_external_rejection_audit(state, claims, reason="email_belongs_to_crew_account")
        raise Forbidden("email_belongs_to_crew_account")
