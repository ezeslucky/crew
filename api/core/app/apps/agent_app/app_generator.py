...
``AgentAppRunner`` calling the crew-agent backend rather than an in-process
...
from core.app.llm.model_access import build_crew_model_access
...
def _append_prompt_file_mappings(query: str, prompt_file_mappings: Sequence[JsonValue]) -> str:
    ...
    return (
        f"{query}\n"
        "User provided files: use crew-agent file download with the listed transfer_method and reference/url "
...
def _canonical_file_reference(reference: str | None) -> str | None:
    if reference is None:
        return None
    if reference.startswith("crew-file-ref:"):
...
class AgentAppGenerator(MessageBasedAppGenerator):
        ...
        *,
        ...
        with preserve_flask_contexts(flask_app, context_vars=context):
            try:
                ...
                runner = self._build_runner(crew_context)
                runner.run(
    ...
    @staticmethod
    def _build_runner(crew_context: CrewRunContext) -> AgentAppRunner:
        credentials_provider, _ = build_crew_model_access(crew_context)
        return AgentAppRunner(
            ...
            text_delta_debounce_seconds=crew_config.AGENT_APP_TEXT_DELTA_DEBOUNCE_SECONDS,
        )
