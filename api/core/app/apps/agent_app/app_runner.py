...
from crew_agent.layers.ask_human import AskHumanToolArgs
from crew_agent.protocol import DeferredToolResultsPayload
...
from core.app.entities.app_invoke_entities import AgentRuntimeExitIntent, CrewRunContext
...
class _AgentProcessRecorder:
    ...
    def _create_thought(
        ...
        *,
        ...
        row = MessageAgentThought(
            ...
            created_by=self._crew_context.user_id,
        )
        self._next_position += 1
        db.session.add(row)
        db.session.commit()
    ...
    def _created_by_role(self) -> CreatorUserRole:
        if self._crew_context.invoke_from.runs_as_account():
...
class AgentAppRunner:
    ...
    def _build_session_scope(
        ...
        *,
        ...
        return AgentAppSessionScope(
            tenant_id=crew_context.tenant_id,
            app_id=crew_context.app_id,
            conversation_id=conversation_id,
            agent_id=agent_id,
    ...
    def _build_runtime(
        ...
        *,
        ...
        deferred_tool_results = (
            self._resolve_pending_ask_human(stored=stored, crew_context=crew_context, message_id=message_id)
    ...
    def _pause_for_ask_human(
        ...
        *,
        ...
        """End the chat turn on a crew.ask_human call: create a conversation-owned
        ...
        try:
            created = create_ask_human_form(
                ...
                repository=self._build_form_repository(crew_context),
                conversation_id=conversation_id,
            )
    ...
    def _build_form_repository(self, crew_context: CrewRunContext) -> HumanInputFormRepository:
        invoke_source = crew_context.invoke_from.value
        return HumanInputFormRepositoryImpl(
            tenant_id=crew_context.tenant_id,
            app_id=crew_context.app_id,
            workflow_execution_id=None,
            invoke_source=invoke_source,
            submission_actor_id=crew_context.user_id if invoke_source in {"debugger", "explore"} else None,
        )
