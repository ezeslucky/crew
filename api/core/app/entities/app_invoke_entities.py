...
class CrewRunContext(BaseModel):
    tenant_id: str
    app_id: str
    user_id: str
    user_from: UserFrom
...
def build_crew_run_context(
    *,
    ...
    """
    Build graph run_context with the reserved Crew runtime payload.

    `extra_context` can carry user-defined context keys. The reserved `_crew`
    ...
    run_context[CREW_RUN_CONTEXT_KEY] = CrewRunContext(
        tenant_id=tenant_id,
        app_id=app_id,
        user_id=user_id,
        user_from=user_from,
...
class AgentAppGenerateEntity(ChatAppGenerateEntity):
    ...
    accepted-entity union. The answer is produced by the crew-agent backend
