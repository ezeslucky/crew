...
class PauseStatePersistenceLayer(GraphEngineLayer):
    ...
    def _get_repo(self) -> APIWorkflowRunRepository:
        return CrewAPIRepositoryFactory.create_api_workflow_run_repository(self._session_maker)
    ...
    def on_event(self, event: GraphEngineEvent) -> None:
        ...
        # NOTE(QuantumGhost): Crew owns the pause-reason semantics that cross the
        # persistence boundary. Graphon session ids are translated back to form ids
        # here so repository/model layers only handle Crew-owned pause reasons.
