...
class CrewWorkflowFileRuntime(WorkflowFileRuntimeProtocol):
    ...
    def multimodal_send_format(self) -> str:
        return crew_config.MULTIMODAL_SEND_FORMAT
    ...
    def verify_preview_signature(
        ...
        *,
        ...
        if sign != base64.urlsafe_b64encode(recalculated).decode():
            return False
        return int(time.time()) - int(timestamp) <= crew_config.FILES_ACCESS_TIMEOUT

    @staticmethod
    def _base_url(*, for_external: bool) -> str:
        if for_external:
            return crew_config.FILES_URL
        return crew_config.INTERNAL_FILES_URL or crew_config.FILES_URL

    @staticmethod
    def _secret_key() -> bytes:
        return crew_config.SECRET_KEY.encode()
...
def bind_crew_workflow_file_runtime() -> None:
    set_workflow_file_runtime(CrewWorkflowFileRuntime(file_access_controller=DatabaseFileAccessController()))
