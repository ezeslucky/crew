...
from extensions.otel.semconv import CrewSpanAttributes
...
class ObservabilityLayer(GraphEngineLayer):
    ...
    def _init_tracer(self) -> None:
        ...
        if not (crew_config.ENABLE_OTEL or is_instrument_flag_enabled()):
    ...
    def _record_abort_reason(self, *, reason: str) -> None:
        ...
        span.set_attribute(CrewSpanAttributes.WORKFLOW_ABORT_REASON, reason)
        span.add_event(
            "crew.workflow.aborted",
            attributes={
                CrewSpanAttributes.WORKFLOW_ABORT_REASON: reason,
            },
        )
