"""Unified remote-file retrieval with Crew signed file URL resolution.
...
requests can resolve Crew-signed file URLs locally through DB + storage before
...
stay as real network requests and should not reinterpret Crew file URLs as stored
files.
"""
...
def make_request(method: str, url: str, max_retries: int = SSRF_DEFAULT_MAX_RETRIES, **kwargs: Any) -> httpx.Response:
    ...
    GET and HEAD requests for Crew-owned signed file URLs are served from local
    storage. Every other request is delegated unchanged to the SSRF proxy.
    """

    normalized_method = method.upper()
    if normalized_method == "GET":
        response = _resolve_crew_signed_file_url("GET", url)
        if response is not None:
            return response
    if normalized_method == "HEAD":
        response = _resolve_crew_signed_file_url("HEAD", url)
        if response is not None:
            return response
...
class GraphonRemoteFileFetcher:
    ...
    Graphon requires method-specific HTTP client methods, while regular Crew
...
def _resolve_crew_signed_file_url(method: Literal["GET", "HEAD"], url: str) -> httpx.Response | None:
    parsed_url = urllib.parse.urlparse(url)
    if not _is_crew_file_origin(parsed_url):
        return None
...
def _is_crew_file_origin(parsed_url: urllib.parse.ParseResult) -> bool:
    ...
    allowed_origins = {
        origin
        for configured_url in [crew_config.FILES_URL, crew_config.INTERNAL_FILES_URL]
...
def _verify_signed_file_url(
    *,
    ...
    if current_time - signed_at > crew_config.FILES_ACCESS_TIMEOUT:
        return False

    payload = f"{signed_file_url.preview_kind}|{signed_file_url.file_id}|{timestamp}|{nonce}"
    recalculated = hmac.new(crew_config.SECRET_KEY.encode(), payload.encode(), hashlib.sha256).digest()
