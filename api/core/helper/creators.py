...
creators_platform_api_url = URL(str(crew_config.CREATORS_PLATFORM_API_URL))
...
def get_redirect_url(user_account_id: str, claim_code: str) -> str:
    base_url = str(crew_config.CREATORS_PLATFORM_API_URL).rstrip("/")
    params: dict[str, str] = {"dsl_claim_code": claim_code}
    client_id = str(crew_config.CREATORS_PLATFORM_OAUTH_CLIENT_ID or "")
    if client_id:
