...
marketplace_api_url = URL(str(crew_config.MARKETPLACE_API_URL))
...
def download_plugin_pkg(plugin_unique_identifier: str) -> bytes:
    return download_with_size_limit(get_plugin_pkg_url(plugin_unique_identifier), crew_config.PLUGIN_MAX_PACKAGE_SIZE)
...
def batch_fetch_plugin_manifests(plugin_ids: list[str]) -> Sequence[MarketplacePluginDeclaration]:
    ...
    response = httpx.post(
        ...
        headers={"X-Crew-Version": crew_config.project.version},
...
def fetch_global_plugin_manifest(cache_key_prefix: str, cache_ttl: int) -> None:
    ...
    response = httpx.get(url, headers={"X-Crew-Version": crew_config.project.version}, timeout=MARKETPLACE_TIMEOUT)
    response.raise_for_status()
