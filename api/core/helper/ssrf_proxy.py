...
and metadata checks should use `core.file.remote_fetcher` instead so Crew-signed
...
_SSRF_CLIENT_LIMITS = httpx.Limits(
    max_connections=crew_config.SSRF_POOL_MAX_CONNECTIONS,
    max_keepalive_connections=crew_config.SSRF_POOL_MAX_KEEPALIVE_CONNECTIONS,
    keepalive_expiry=crew_config.SSRF_POOL_KEEPALIVE_EXPIRY,
)
...
def _create_proxy_mounts(verify: bool) -> dict[str, httpx.HTTPTransport]:
    ...
    return {
        "http://": httpx.HTTPTransport(
            proxy=crew_config.SSRF_PROXY_HTTP_URL,
            verify=verify,
        ),
        "https://": httpx.HTTPTransport(
            proxy=crew_config.SSRF_PROXY_HTTPS_URL,
            verify=verify,
        ),
    }
...
def _build_ssrf_client(verify: bool) -> httpx.Client:
    if crew_config.SSRF_PROXY_ALL_URL:
        return httpx.Client(
            proxy=crew_config.SSRF_PROXY_ALL_URL,
            verify=verify,
            limits=_SSRF_CLIENT_LIMITS,
        )

    if crew_config.SSRF_PROXY_HTTP_URL and crew_config.SSRF_PROXY_HTTPS_URL:
        return httpx.Client(
    ...
    **kwargs: Any,
    ...
    if "timeout" not in kwargs:
        kwargs["timeout"] = httpx.Timeout(
            timeout=crew_config.SSRF_DEFAULT_TIME_OUT,
            connect=crew_config.SSRF_DEFAULT_CONNECT_TIME_OUT,
            read=crew_config.SSRF_DEFAULT_READ_TIME_OUT,
            write=crew_config.SSRF_DEFAULT_WRITE_TIME_OUT,
        )

    # prioritize per-call option, which can be switched on and off inside the HTTP node on the web UI
    verify_option = kwargs.pop("ssl_verify", crew_config.HTTP_REQUEST_NODE_SSL_VERIFY)
