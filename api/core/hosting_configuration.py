...
class HostingConfiguration:
    ...
    def init_azure_openai() -> HostingProvider:
        ...
        if crew_config.HOSTED_AZURE_OPENAI_ENABLED:
            credentials = {
                "openai_api_key": crew_config.HOSTED_AZURE_OPENAI_API_KEY,
                "openai_api_base": crew_config.HOSTED_AZURE_OPENAI_API_BASE,
                "base_model_name": "gpt-35-turbo",
            }

            quotas: list[HostingQuota] = []
            hosted_quota_limit = crew_config.HOSTED_AZURE_OPENAI_QUOTA_LIMIT
    ...
    def init_openai(self) -> HostingProvider:
        ...
        if crew_config.HOSTED_OPENAI_TRIAL_ENABLED:
        ...
        if crew_config.HOSTED_OPENAI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "openai_api_key": crew_config.HOSTED_OPENAI_API_KEY,
            }

            if crew_config.HOSTED_OPENAI_API_BASE:
                credentials["openai_api_base"] = crew_config.HOSTED_OPENAI_API_BASE

            if crew_config.HOSTED_OPENAI_API_ORGANIZATION:
                credentials["openai_organization"] = crew_config.HOSTED_OPENAI_API_ORGANIZATION
    ...
    def init_gemini(self) -> HostingProvider:
        ...
        if crew_config.HOSTED_GEMINI_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trial_models = self.parse_restrict_models_from_env("HOSTED_GEMINI_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trial_models)
            quotas.append(trial_quota)

        if crew_config.HOSTED_GEMINI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "google_api_key": crew_config.HOSTED_GEMINI_API_KEY,
            }

            if crew_config.HOSTED_GEMINI_API_BASE:
                credentials["google_base_url"] = crew_config.HOSTED_GEMINI_API_BASE
    ...
    def init_anthropic(self) -> HostingProvider:
        ...
        if crew_config.HOSTED_ANTHROPIC_TRIAL_ENABLED:
            hosted_quota_limit = 0
        ...
        if crew_config.HOSTED_ANTHROPIC_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "anthropic_api_key": crew_config.HOSTED_ANTHROPIC_API_KEY,
            }

            if crew_config.HOSTED_ANTHROPIC_API_BASE:
                credentials["anthropic_api_url"] = crew_config.HOSTED_ANTHROPIC_API_BASE
    ...
    def init_tongyi(self) -> HostingProvider:
        ...
        if crew_config.HOSTED_TONGYI_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trail_models = self.parse_restrict_models_from_env("HOSTED_TONGYI_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trail_models)
            quotas.append(trial_quota)

        if crew_config.HOSTED_TONGYI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "dashscope_api_key": crew_config.HOSTED_TONGYI_API_KEY,
                # SNP-494: keep temporary compatibility with tongyi plugin string credential checks.
                "use_international_endpoint": str(crew_config.HOSTED_TONGYI_USE_INTERNATIONAL_ENDPOINT).lower(),
            }
    ...
    def init_xai(self) -> HostingProvider:
        ...
        if crew_config.HOSTED_XAI_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trail_models = self.parse_restrict_models_from_env("HOSTED_XAI_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trail_models)
            quotas.append(trial_quota)

        if crew_config.HOSTED_XAI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "api_key": crew_config.HOSTED_XAI_API_KEY,
            }

            if crew_config.HOSTED_XAI_API_BASE:
                credentials["endpoint_url"] = crew_config.HOSTED_XAI_API_BASE
    ...
    def init_deepseek(self) -> HostingProvider:
        ...
        if crew_config.HOSTED_DEEPSEEK_TRIAL_ENABLED:
            hosted_quota_limit = 0
            trail_models = self.parse_restrict_models_from_env("HOSTED_DEEPSEEK_TRIAL_MODELS")
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit, restrict_models=trail_models)
            quotas.append(trial_quota)

        if crew_config.HOSTED_DEEPSEEK_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "api_key": crew_config.HOSTED_DEEPSEEK_API_KEY,
            }

            if crew_config.HOSTED_DEEPSEEK_API_BASE:
                credentials["endpoint_url"] = crew_config.HOSTED_DEEPSEEK_API_BASE
    ...
    def init_minimax() -> HostingProvider:
        ...
        if crew_config.HOSTED_MINIMAX_ENABLED:
    ...
    def init_spark() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if crew_config.HOSTED_SPARK_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_zhipuai() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if crew_config.HOSTED_ZHIPUAI_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_moderation_config() -> HostedModerationConfig:
        if crew_config.HOSTED_MODERATION_ENABLED and crew_config.HOSTED_MODERATION_PROVIDERS:
            providers = crew_config.HOSTED_MODERATION_PROVIDERS.split(",")
            hosted_providers = []
    ...
    def parse_restrict_models_from_env(env_var: str) -> list[RestrictModel]:
        models_str = crew_config.model_dump().get(env_var)
