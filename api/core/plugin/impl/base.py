...
plugin_daemon_inner_api_baseurl = URL(str(crew_config.PLUGIN_DAEMON_URL))
_plugin_daemon_timeout_config = cast(
    float | httpx.Timeout | None,
    getattr(crew_config, "PLUGIN_DAEMON_TIMEOUT", 600.0),
)
...
class BasePluginClient:
    ...
    def _prepare_request(
        ...
        prepared_headers["X-Api-Key"] = crew_config.PLUGIN_DAEMON_KEY
    ...
    def _inject_trace_headers(self, headers: dict[str, str]) -> None:
        ...
        """
        if not crew_config.ENABLE_OTEL:
            return
