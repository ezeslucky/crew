...
TENANT_SCOPE_SCHEMA_CACHE_USER_ID = "__CREW_TS__"


# TODO(-LAN-): Move native structured-output invocation into Graphon's LLM node.
# TODO(-LAN-): Remove this Crew-side adapter once Graphon owns structured output end-to-end.
...
class PluginModelRuntime(ModelRuntime):
    ...
    def get_model_schema(
        ...
        *,
        ...
        if schema:
            try:
                redis_client.setex(cache_key, crew_config.PLUGIN_MODEL_SCHEMA_CACHE_TTL, schema.model_dump_json())
            except (RedisError, RuntimeError) as exc:
                logger.warning(
    ...
    def get_llm_num_tokens(
        ...
        *,
        ...
        if not crew_config.PLUGIN_BASED_TOKEN_COUNTING_ENABLED:
            return 0
