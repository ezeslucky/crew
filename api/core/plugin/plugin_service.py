...
class PluginService:
    ...
    PLUGIN_MODEL_PROVIDERS_CACHE_COMPRESSION_PREFIX = b"\x00crew-plugin-model-providers-zstd-v1:"
    ...
    def _store_cached_plugin_model_providers(
        ...
        try:
            ...
            redis_client.setex(cache_key, crew_config.PLUGIN_MODEL_PROVIDERS_CACHE_TTL, payload)
        except (RedisError, RuntimeError):
    ...
    def _store_cached_remote_model_plugin_marker(cls, tenant_id: str, marker: str | None) -> None:
        ...
        try:
            if marker is None:
                ...
                redis_client.setex(cache_key, crew_config.PLUGIN_MODEL_PROVIDERS_CACHE_TTL, marker)
        except (RedisError, RuntimeError):
    ...
    def fetch_latest_plugin_version(plugin_ids: Sequence[str]) -> Mapping[str, LatestPluginCache | None]:
        ...
        try:
            ...
            if cache_not_exists:
                if not crew_config.MARKETPLACE_ENABLED:
                    logger.info(
    ...
    def get_plugin_icon_url(cls, tenant_id: str, filename: str) -> str:
        url_prefix = (
            URL(crew_config.CONSOLE_API_URL or "/") / "console" / "api" / "workspaces" / "current" / "plugin" / "icon"
        )
    ...
    def upload_pkg_from_github(
        ...
        pkg = download_with_size_limit(
            f"https://github.com/{repo}/releases/download/{version}/{package}", crew_config.PLUGIN_MAX_PACKAGE_SIZE
        )
        features = FeatureService.get_system_features()
    ...
    def uninstall(tenant_id: str, plugin_installation_id: str) -> bool:
        ...
        if crew_config.ENTERPRISE_ENABLED:
