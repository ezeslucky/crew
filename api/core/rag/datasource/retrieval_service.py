...
class RetrievalService:
    ...
    def retrieve(
        ...
        with ThreadPoolExecutor(max_workers=crew_config.RETRIEVAL_SERVICE_EXECUTORS) as executor:  # type: ignore
            futures = []
    ...
    def _deduplicate_documents(cls, documents: list[Document]) -> list[Document]:
        ...
        for doc in documents:
            ...
            if doc_id:
                key = (doc.provider or "crew", doc_id)
                ...
                # Content-based dedup for non-crew or crew without doc_id
                content_key = (doc.provider or "crew", doc.page_content)
