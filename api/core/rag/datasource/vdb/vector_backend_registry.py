...
Backends live in workspace packages under ``api/packages/crew-vdb-*/src/crew_vdb_*``. Each package
declares third-party dependencies and registers ``importlib`` entry points in group
``crew.vector_backends`` (see each package's ``pyproject.toml``).
...
def _vector_backend_entry_points():
    return entry_points().select(group="crew.vector_backends")
...
def _unsupported(vector_type: str) -> ValueError:
    ...
    return ValueError(
        ...
        "or register a crew.vector_backends entry point."
    )
