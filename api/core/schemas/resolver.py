...
_CREW_SCHEMA_PATTERN = re.compile(r"^https://crew\.ai/schemas/(v\d+)/(.+)\.json$")
...
class SchemaResolver:
    """Resolver for Crew schema references with caching and optimizations"""
    ...
    def _process_dict(self, queue: deque, item: QueueItem) -> None:
        ...
        if ref_uri and _is_crew_schema_ref(ref_uri):
...
def resolve_crew_schema_refs(
    schema: SchemaType, registry: SchemaRegistry | None = None, max_depth: int = 30
) -> SchemaType:
    """
    Resolve $ref references in Crew schema to actual schema content
...
def _is_crew_schema_ref(ref_uri: Any) -> bool:
    """
    Check if the reference URI is a Crew schema reference

    Args:
        ref_uri: URI to check

    Returns:
        True if it's a Crew schema reference
    """
    if not isinstance(ref_uri, str):
        return False

    # Use pre-compiled pattern for better performance
    return bool(_CREW_SCHEMA_PATTERN.match(ref_uri))


def _has_crew_refs_recursive(schema: SchemaType) -> bool:
    """
    Recursively check if a schema contains any Crew $ref references
    ...
    Returns:
        True if any Crew $ref is found, False otherwise
    """
    match schema:
        case dict():
            ...
            for value in schema.values():
                if _has_crew_refs_recursive(value):
                    return True

        case list():
            # Check each item in the list
            for item in schema:
                if _has_crew_refs_recursive(item):
                    return True
...
def _has_crew_refs_hybrid(schema: SchemaType) -> bool:
    """
    ...
    try:
        ...
        if "https://crew.ai/schemas/" not in schema_str:
            return False
        ...
        return _has_crew_refs_recursive(schema)
...
def _has_crew_refs(schema: SchemaType) -> bool:
    """
    Check if a schema contains any Crew $ref references
    ...
    Returns:
        True if any Crew $ref is found, False otherwise
    """
    return _has_crew_refs_hybrid(schema)


def parse_crew_schema_uri(uri: str) -> tuple[str, str]:
    """
    Parse a Crew schema URI to extract version and schema name
    ...
    """
    match = _CREW_SCHEMA_PATTERN.match(uri)
    if not match:
        return "", ""
