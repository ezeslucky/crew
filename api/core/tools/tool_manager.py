...
class ToolManager:
        ...
        match provider_type:
            case ToolProviderType.BUILT_IN:
                ...
                if builtin_provider.expires_at != -1 and (builtin_provider.expires_at - 60) < int(time.time()):
                    ...
                    redirect_uri = f"{crew_config.CONSOLE_API_URL}/console/api/oauth/plugin/{provider_id}/tool/callback"
    ...
    def list_default_builtin_providers(cls, tenant_id: str) -> list[BuiltinToolProvider]:
        ...
        if crew_config.SQLALCHEMY_DATABASE_URI_SCHEME == "postgresql":
    ...
    def generate_builtin_tool_icon_url(cls, provider_id: str) -> str:
        return str(
            URL(crew_config.CONSOLE_API_URL or "/")
            / "console"
            / "api"
            / "workspaces"
            / "current"
