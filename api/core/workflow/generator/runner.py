...
def _node_builder_max_workers() -> int:
    return crew_config.WORKFLOW_GENERATOR_NODE_BUILDER_MAX_WORKERS
...
class WorkflowGenerator:
    """
    Generates a Crew workflow graph from a natural-language instruction.
    ...
    def _postprocess_graph(cls, *, graph: GraphDict, mode: WorkflowGenerationMode) -> GraphDict:
        ...
        # Defensive ID remap: Crew's run-time placeholder regex only accepts
        ...
        # which Crew's edge renderer needs.
    ...
    # Crew's workflow runtime uses (see
    ...
    def _sanitize_node_ids(cls, *, nodes: list[dict[str, Any]], edges: list[dict[str, Any]]) -> None:
        ...
        Crew's run-time ``VARIABLE_PATTERN`` accepts only ``[a-zA-Z0-9_]`` in
    ...
    def _collect_dangling_id_refs(
        ...
        because Crew's schema puts it on the wrapper (ReactFlow convention)
