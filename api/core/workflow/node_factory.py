...
from core.app.entities.app_invoke_entities import CREW_RUN_CONTEXT_KEY, CrewRunContext
from core.app.llm.model_access import build_crew_model_access, fetch_model_config
...
from core.workflow.node_runtime import (
    CrewFileReferenceFactory,
    CrewHumanInputNodeRuntime,
    CrewPreparedLLM,
    CrewPreparedPollingLLM,
    CrewPromptMessageSerializer,
    CrewRetrieverAttachmentLoader,
    CrewToolFileManager,
    CrewToolNodeRuntime,
    build_crew_llm_file_saver,
)
...
from core.workflow.nodes.agent_v2 import CrewAgentNode
...
from core.workflow.nodes.human_input.callback import CrewHITLCallback
from core.workflow.nodes.human_input.entities import HumanInputNodeData as CrewHumanInputNodeData
...
class CrewGraphInitContext:
    """Explicit graph-init values owned by the workflow layer.

    Crew is gradually removing direct `GraphInitParams` construction from its
...
@final
class CrewNodeFactory(NodeFactory):
    """
    ...
    def from_graph_init_context(
        cls,
        *,
        graph_init_context: CrewGraphInitContext,
        graph_runtime_state: "GraphRuntimeState",
    ) -> "CrewNodeFactory":
        """Bridge Crew's explicit init context into the current `graphon` API."""
        return cls(
    ...
    def __init__(
        ...
        self._crew_context = self._resolve_crew_context(graph_init_params.run_context)
        self._code_executor: CodeExecutorProtocol = DefaultWorkflowCodeExecutor()
        self._code_limits = CodeNodeLimits(
            max_string_length=crew_config.CODE_MAX_STRING_LENGTH,
            max_number=crew_config.CODE_MAX_NUMBER,
            min_number=crew_config.CODE_MIN_NUMBER,
            max_precision=crew_config.CODE_MAX_PRECISION,
            max_depth=crew_config.CODE_MAX_DEPTH,
            max_number_array_length=crew_config.CODE_MAX_NUMBER_ARRAY_LENGTH,
            max_string_array_length=crew_config.CODE_MAX_STRING_ARRAY_LENGTH,
            max_object_array_length=crew_config.CODE_MAX_OBJECT_ARRAY_LENGTH,
        )
        self._jinja2_template_renderer = CodeExecutorJinja2TemplateRenderer()
        self._template_transform_max_output_length = crew_config.TEMPLATE_TRANSFORM_MAX_LENGTH
        ...
        self._bound_tool_file_manager_factory = lambda: CrewToolFileManager(
            self._crew_context,
            conversation_id_getter=self._conversation_id,
        )
        self._file_reference_factory = CrewFileReferenceFactory(self._crew_context)
        self._prompt_message_serializer = CrewPromptMessageSerializer()
        self._retriever_attachment_loader = CrewRetrieverAttachmentLoader(
            file_reference_factory=self._file_reference_factory,
        )
        self._llm_file_saver = build_crew_llm_file_saver(
            run_context=self._crew_context,
            http_client=self._remote_file_http_client,
            conversation_id_getter=self._conversation_id,
        )
        self._human_input_runtime = CrewHumanInputNodeRuntime(
            self._crew_context,
        ...
        self._tool_runtime = CrewToolNodeRuntime(self._crew_context)
        self._http_request_file_manager = file_manager
        self._document_extractor_unstructured_api_config = UnstructuredApiConfig(
            api_url=crew_config.UNSTRUCTURED_API_URL,
            api_key=crew_config.UNSTRUCTURED_API_KEY or "",
        )
        self._http_request_config = build_http_request_config(
            max_connect_timeout=crew_config.HTTP_REQUEST_MAX_CONNECT_TIMEOUT,
            max_read_timeout=crew_config.HTTP_REQUEST_MAX_READ_TIMEOUT,
            max_write_timeout=crew_config.HTTP_REQUEST_MAX_WRITE_TIMEOUT,
            max_binary_size=crew_config.HTTP_REQUEST_NODE_MAX_BINARY_SIZE,
            max_text_size=crew_config.HTTP_REQUEST_NODE_MAX_TEXT_SIZE,
            ssl_verify=crew_config.HTTP_REQUEST_NODE_SSL_VERIFY,
            ssrf_default_max_retries=crew_config.SSRF_DEFAULT_MAX_RETRIES,
        )

        self._llm_credentials_provider, self._llm_model_factory = build_crew_model_access(self._crew_context)
    ...
    @staticmethod
    def _resolve_crew_context(run_context: Mapping[str, Any]) -> CrewRunContext:
        raw_ctx = run_context.get(CREW_RUN_CONTEXT_KEY)
        if raw_ctx is None:
            raise ValueError(f"run_context missing required key: {CREW_RUN_CONTEXT_KEY}")
        if isinstance(raw_ctx, CrewRunContext):
            return raw_ctx
        return CrewRunContext.model_validate(raw_ctx)
        ...
        node_init_kwargs_factories: Mapping[NodeType, Callable[[], dict[str, object]]] = {
            ...
            BuiltinNodeTypes.HUMAN_INPUT: lambda: {
                "hitl_callback": self._build_human_input_callback(
                    node_data=CrewHumanInputNodeData.model_validate(adapted_node_config["data"])
                ),
            },
    ...
    def _build_agent_node_init_kwargs(self, *, node_class: type[Node]) -> dict[str, object]:
        if issubclass(node_class, CrewAgentNode):
            ...
            return {
                ...
                "agent_backend_client": create_agent_backend_run_client(
                    base_url=crew_config.AGENT_BACKEND_BASE_URL,
                    use_fake=crew_config.AGENT_BACKEND_USE_FAKE,
                    fake_scenario=crew_config.AGENT_BACKEND_FAKE_SCENARIO,
                    stream_read_timeout_seconds=crew_config.AGENT_BACKEND_STREAM_READ_TIMEOUT_SECONDS,
                    stream_max_reconnects=crew_config.AGENT_BACKEND_STREAM_MAX_RECONNECTS,
                    stream_run_timeout_seconds=crew_config.AGENT_BACKEND_RUN_TIMEOUT_SECONDS,
                ),
    ...
    def _build_human_input_callback(
        self,
        *,
        node_data: CrewHumanInputNodeData,
    ) -> CrewHITLCallback:
        return CrewHITLCallback(
        ...
        node_init_kwargs: dict[str, object] = {
            ...
            "model_instance": (
                self._wrap_model_instance_for_node(
                    ...
                    request_metadata={"app_id": self._crew_context.app_id},
                )
    ...
    def _wrap_model_instance_for_node(
        *,
        ...
        if node_data.type == BuiltinNodeTypes.LLM and CrewNodeFactory._supports_plugin_llm_polling(model_instance):
            return CrewPreparedPollingLLM(model_instance, request_metadata=request_metadata)
        return CrewPreparedLLM(model_instance, request_metadata=request_metadata)

    @staticmethod
    def _supports_plugin_llm_polling(model_instance: ModelInstance) -> bool:
        return model_instance.get_model_schema().support_polling

    def _build_retriever_attachment_loader(self, node_data: LLMNodeData) -> CrewRetrieverAttachmentLoader:
        return CrewRetrieverAttachmentLoader(
            file_reference_factory=self._file_reference_factory,
    ...
    def _build_memory_for_llm_node(
        ...
        *,
        ...
        return fetch_memory(
            ...
            app_id=self._crew_context.app_id,
