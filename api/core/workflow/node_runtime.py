...
from core.callback_handler.workflow_tool_callback_handler import CrewWorkflowCallbackHandler
...
def resolve_crew_run_context(run_context: Mapping[str, Any] | CrewRunContext) -> CrewRunContext:
    if isinstance(run_context, CrewRunContext):
        return run_context
...
def apply_crew_debug_email_recipient(
    method: DeliveryChannelConfig,
    *,
    enabled: bool,
    actor_id: str | None,
) -> DeliveryChannelConfig:
    """Apply the Crew debugger-specific email recipient override outside `graphon`."""
    if not enabled:
        return method
...
class CrewFileReferenceFactory(FileReferenceFactoryProtocol):
    def __init__(self, run_context: Mapping[str, Any] | CrewRunContext) -> None:
        self._run_context = resolve_crew_run_context(run_context)
...
class CrewPreparedLLM(LLMProtocol):
...
class CrewPreparedPollingLLM(CrewPreparedLLM, LLMPollingCapableProtocol):
...
class CrewPromptMessageSerializer(PromptMessageSerializerProtocol):
    @override
    def serialize(
        self,
        *,
...
class CrewRetrieverAttachmentLoader(RetrieverAttachmentLoaderProtocol):
    """Resolve retriever attachments through Crew persistence and return graph file references."""
...
class CrewToolFileManager(ToolFileManagerProtocol):
    ...
    def __init__(
        self,
        run_context: Mapping[str, Any] | CrewRunContext,
        *,
...
class CrewToolNodeRuntime(ToolNodeRuntimeProtocol):
    def __init__(
        self,
        run_context: Mapping[str, Any] | CrewRunContext,
        ...
        self._run_context = resolve_crew_run_context(run_context)
        self._file_reference_factory = CrewFileReferenceFactory(self._run_context)
    ...
    def invoke(
        ...
        *,
        ...
        callback = CrewWorkflowCallbackHandler()
    ...
    def _tool_from_handle(tool_runtime: ToolRuntimeHandle) -> Tool:
        return CrewToolNodeRuntime._binding_from_handle(tool_runtime).tool
...
class CrewHumanInputNodeRuntime:
    def __init__(
        self,
        run_context: Mapping[str, Any] | CrewRunContext,
        *,
    ...
    def _resolve_delivery_methods(self, *, node_data: HumanInputNodeData) -> Sequence[DeliveryChannelConfig]:
        ...
        return [
            apply_crew_debug_email_recipient(
                method,
    ...
    def with_form_repository(self, form_repository: HumanInputFormRepository) -> CrewHumanInputNodeRuntime:
        return CrewHumanInputNodeRuntime(
            self._run_context,
...
def build_crew_llm_file_saver(
    *,
    ...
    return FileSaverImpl(
        tool_file_manager=CrewToolFileManager(run_context, conversation_id_getter=conversation_id_getter),
        file_reference_factory=CrewFileReferenceFactory(run_context),
