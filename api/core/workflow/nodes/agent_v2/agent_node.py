...
from crew_agent.protocol import CancelRunRequest
...
from .entities import CrewAgentNodeData
...
class CrewAgentNode(Node[CrewAgentNodeData]):
    node_type = BuiltinNodeTypes.AGENT

    def __init__(
        self,
        node_id: str,
        data: CrewAgentNodeData,
        *,
    ...
    def _run(self) -> Generator[NodeEventBase, None, None]:
        crew_ctx = CrewRunContext.model_validate(self.require_run_context_value(CREW_RUN_CONTEXT_KEY))
        ...
        while True:
            ...
            if isinstance(terminal_event, AgentBackendDeferredToolCallInternalEvent):
                # ENG-636: a crew.ask_human deferred call pauses the *outer*
                ...
                try:
                    pause_request = build_ask_human_pause_reason(
                        ...
                        repository=self._build_human_input_form_repository(
                            crew_ctx=crew_ctx, workflow_run_id=workflow_run_id
                        ),
                    )
    ...
    def _build_human_input_form_repository(
        ...
        *,
        ...
        """
        invoke_source = crew_ctx.invoke_from.value
        return HumanInputFormRepositoryImpl(
            tenant_id=crew_ctx.tenant_id,
            app_id=crew_ctx.app_id,
            workflow_execution_id=workflow_run_id,
            invoke_source=invoke_source,
            submission_actor_id=crew_ctx.user_id if invoke_source in {"debugger", "explore"} else None,
        )
