"""Map a ``crew.ask_human`` deferred tool call onto the existing HITL form path.

ENG-636. When an Agent backend run ends with a deferred ``crew.ask_human`` tool
...
plus Crew delivery configs and the enriched Crew-side pause reason payload.
The Graphon ``HitlRequired`` conversion stays at the node boundary.

The agent-side ``crew.ask_human`` contract is richer than the workflow form
...
from crew_agent.layers.ask_human import (
    AskHumanAction,
    AskHumanField,
    AskHumanFileField,
...
from crew_agent.protocol import DeferredToolCallPayload
