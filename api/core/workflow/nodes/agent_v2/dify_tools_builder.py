...
from crew_agent.layers.crew_core_tools import CrewCoreToolConfig, CrewCoreToolProviderType, CrewCoreToolsLayerConfig
from crew_agent.layers.crew_plugin import (
    CrewPluginCredentialValue,
    CrewPluginToolConfig,
    CrewPluginToolCredentialType,
    CrewPluginToolParameter,
    CrewPluginToolParameterForm,
    CrewPluginToolParameterType,
...
from models.agent_config_entities import AgentSoulCrewToolConfig, AgentSoulToolsConfig
from models.provider_ids import ToolProviderID
from models.tools import WorkflowToolProvider
from services.tools.mcp_tools_manage_service import MCPToolManageService

_CORE_TOOL_PROVIDER_TYPES: Final[dict[ToolProviderType, CrewCoreToolProviderType]] = {
...
class WorkflowAgentCrewToolsBuildError(ValueError):
...
class WorkflowAgentToolLayers:
    plugin_tools: CrewPluginToolsLayerConfig | None = None
...
class WorkflowAgentCrewToolLayersBuilder(Protocol):
    def build_layers(
        self,
        *,
        tenant_id: str,
...
class WorkflowAgentCrewToolsBuilder:
    """Prepare Agent Soul Crew tools for Agent backend run-layer configs.

    Plugin tools keep their existing direct daemon path. Core-routed tools
    (`builtin`/`api`/`workflow`/`mcp`) are emitted as `crew.core.tools`.
    """
    ...
    def build_layers(
        ...
        *,
        ...
        """Resolve user-selected Crew tools into direct/core Agent backend DTOs.
        ...
        """
        enabled_tools = [tool for tool in tools.crew_tools if tool.enabled]
        if not enabled_tools:
            return WorkflowAgentToolLayers()

        prepared_plugin: list[CrewPluginToolConfig] = []
        prepared_core: list[CrewCoreToolConfig] = []
        ...
        for tool_config in self._expand_provider_entries(tenant_id=tenant_id, enabled_tools=enabled_tools):
            ...
            if exposed_name in seen_names:
                raise WorkflowAgentCrewToolsBuildError(
                    "agent_tool_name_duplicated",
                    f"Duplicate Crew Tool name {exposed_name!r}.",
                )
        ...
        return WorkflowAgentToolLayers(
            plugin_tools=CrewPluginToolsLayerConfig(tools=prepared_plugin) if prepared_plugin else None,
            core_tools=CrewCoreToolsLayerConfig(tools=prepared_core) if prepared_core else None,
        )
    ...
    def _expand_provider_entries(
        ...
        *,
        tenant_id: str,
        enabled_tools: list[AgentSoulCrewToolConfig],
    ) -> list[AgentSoulCrewToolConfig]:
        ...
        expanded: list[AgentSoulCrewToolConfig] = []
        for tool_config in enabled_tools:
            if tool_config.tool_name is not None:
            ...
            try:
                ...
                raise WorkflowAgentCrewToolsBuildError(
                    ...
                    f"Crew Tool provider {provider_id!r} declaration not found: {exc}",
                ) from exc
            if not tool_names:
                raise WorkflowAgentCrewToolsBuildError(
                    "agent_tool_declaration_not_found",
                    f"Crew Tool provider {provider_id!r} declares no tools.",
                )
    ...
    def _normalized_tool_config(
        ...
        *,
        tenant_id: str,
        tool_config: AgentSoulCrewToolConfig,
    ) -> AgentSoulCrewToolConfig:
    ...
    def _fetch_tool_runtime(
        ...
        *,
        ...
        try:
            ...
            raise WorkflowAgentCrewToolsBuildError(
                "agent_tool_declaration_not_found",
                f"Crew Tool {tool_config.tool_name!r} declaration not found: {exc}",
            ) from exc
        except ToolProviderCredentialValidationError as exc:
            raise WorkflowAgentCrewToolsBuildError(
                "agent_tool_credential_invalid",
                f"Crew Tool {tool_config.tool_name!r} credential validation failed: {exc}",
            ) from exc
        except ValueError as exc:
            raise WorkflowAgentCrewToolsBuildError(
                "agent_tool_config_invalid",
                f"Crew Tool {tool_config.tool_name!r} runtime construction failed: {exc}",
            ) from exc

    @staticmethod
    def _to_agent_tool_entity(tool_config: AgentSoulCrewToolConfig) -> AgentToolEntity:
        assert tool_config.tool_name is not None
        return AgentToolEntity(
            provider_type=tool_config.provider_type,
            provider_id=WorkflowAgentCrewToolsBuilder._provider_id(tool_config),
    ...
    @staticmethod
    def _provider_id(tool_config: AgentSoulCrewToolConfig) -> str:
    ...
    @staticmethod
    def _provider_key(tool_config: AgentSoulCrewToolConfig) -> tuple[ToolProviderType, str]:
        return (tool_config.provider_type, WorkflowAgentCrewToolsBuilder._provider_id(tool_config))

    @staticmethod
    def _tool_layer_destination(tool_config: AgentSoulCrewToolConfig) -> Literal["plugin", "core"]:
        provider_type = tool_config.provider_type
        ...
        raise WorkflowAgentCrewToolsBuildError(
            "agent_tool_provider_not_supported",
            f"Crew Tool provider type {provider_type.value!r} is not supported in Agent tool layers.",
        )

    @staticmethod
    def _exposed_tool_name(tool_config: AgentSoulCrewToolConfig) -> str:
        assert tool_config.tool_name is not None
    ...
    def _to_plugin_backend_tool_config(
        ...
        if runtime is None:
            raise WorkflowAgentCrewToolsBuildError(
                "agent_tool_config_invalid",
                f"Crew Tool {tool_config.tool_name!r} has no runtime.",
            )
    ...
    @staticmethod
    def _plugin_provider(tool_config: AgentSoulCrewToolConfig, provider_id: str) -> tuple[str, str]:
    ...
    @staticmethod
    def _prepared_parameters(tool_runtime: Tool) -> list[CrewPluginToolParameter]:
        return [
            CrewPluginToolParameter.model_validate(parameter.model_dump(mode="json"))
            for parameter in tool_runtime.get_merged_runtime_parameters()
        ]

    @staticmethod
    def _description(tool_config: AgentSoulCrewToolConfig, tool_runtime: Tool) -> str | None:
    ...
    def _plugin_parameters_json_schema(
        tool_runtime: Tool,
        parameters: list[CrewPluginToolParameter],
    ) -> dict[str, Any]:
        ...
        if not isinstance(properties, dict) or not isinstance(required, list):
            raise WorkflowAgentCrewToolsBuildError(
                ...
                f"Crew Plugin Tool {tool_runtime.entity.identity.name!r} has invalid parameter schema.",
            )

        for parameter in parameters:
            if parameter.form is not CrewPluginToolParameterForm.LLM:
                continue
            if parameter.type is CrewPluginToolParameterType.FILE:
                properties[parameter.name] = _plugin_file_input_schema(parameter.llm_description or "")
            elif parameter.type in {
                CrewPluginToolParameterType.FILES,
                CrewPluginToolParameterType.SYSTEM_FILES,
            }:
    ...
    def _runtime_parameters(
        ...
        if missing:
            ...
            raise WorkflowAgentCrewToolsBuildError(
                ...
                f"Crew Tool {tool_runtime.entity.identity.name!r} is missing runtime parameters: {names}.",
            )
    ...
    def _normalize_credentials(
        ...
        *,
        tool_name: str,
    ) -> dict[str, CrewPluginCredentialValue]:
        normalized: dict[str, CrewPluginCredentialValue] = {}
        for key, value in credentials.items():
            ...
            raise WorkflowAgentCrewToolsBuildError(
                ...
                (
                    f"Crew Plugin Tool {tool_name!r} credential {key!r} has a non-scalar value "
