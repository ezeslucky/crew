...
from crew_agent.agent_stub.protocol import AgentStubFileMapping
from crew_agent.layers.ask_human import CrewAskHumanLayerConfig
from crew_agent.layers.config import (
    CrewConfigFileConfig,
    CrewConfigLayerConfig,
    CrewConfigSkillConfig,
    CrewConfigVersionConfig,
)
from crew_agent.layers.execution_context import (
    CrewExecutionContextInvokeFrom,
    CrewExecutionContextLayerConfig,
    CrewExecutionContextUserFrom,
)
from crew_agent.layers.knowledge import (
    CrewKnowledgeBaseLayerConfig,
    CrewKnowledgeDatasetConfig,
    CrewKnowledgeMetadataFilteringConfig,
    CrewKnowledgeModelConfig,
    CrewKnowledgeQueryConfig,
    CrewKnowledgeRerankingModelConfig,
    CrewKnowledgeRetrievalConfig,
    CrewKnowledgeSetConfig,
)
from crew_agent.layers.shell import (
    CrewShellCliToolConfig,
    CrewShellEnvVarConfig,
    CrewShellLayerConfig,
    CrewShellSandboxConfig,
    CrewShellSecretRefConfig,
)
from crew_agent.protocol import CreateRunRequest, DeferredToolResultsPayload
...
from core.app.entities.app_invoke_entities import CrewRunContext, InvokeFrom
...
from .crew_tools_builder import (
    WorkflowAgentCrewToolLayersBuilder,
    WorkflowAgentCrewToolsBuilder,
    WorkflowAgentCrewToolsBuildError,
...
_CANONICAL_CREW_FILE_REFERENCE_PATTERN = r"^crew-file-ref:eyJyZWNvcmRfaWQiOi[A-Za-z0-9_-]+={0,2}$"
...
class WorkflowAgentRuntimeRequestBuilder:
    """Build public Crew Agent run requests from workflow Agent v2 runtime state."""
    ...
    def __init__(
        ...
        *,
        ...
        crew_tools_builder: WorkflowAgentCrewToolLayersBuilder | None = None,
    ) -> None:
        self._credentials_provider = credentials_provider
        self._request_builder = request_builder or AgentBackendRunRequestBuilder()
        self._crew_tools_builder = crew_tools_builder or WorkflowAgentCrewToolsBuilder()
    ...
    def build(self, context: WorkflowAgentRuntimeBuildContext) -> WorkflowAgentRuntimeRequest:
        ...
        try:
            tool_layers = self._build_tool_layers(
                tenant_id=context.crew_context.tenant_id,
                app_id=context.crew_context.app_id,
                user_id=context.crew_context.user_id,
                tools=agent_soul.tools,
                invoke_from=context.crew_context.invoke_from,
            )
        except WorkflowAgentCrewToolsBuildError as error:
        ...
        if tool_layers.plugin_tools is not None or tool_layers.core_tools is not None or agent_soul.tools.cli_tools:
            metadata["agent_tools"] = {
                "crew_tool_count": len(tool_layers.exposed_tool_names()),
                "crew_tool_names": tool_layers.exposed_tool_names(),
        ...
        request = self._request_builder.build_for_workflow_node(
            AgentBackendWorkflowNodeRunInput(
                ...
                # carrier for Crew tenant/user/run identifiers. ``user_id`` and
                ...
                execution_context=CrewExecutionContextLayerConfig(
                    tenant_id=context.crew_context.tenant_id,
                    user_id=context.crew_context.user_id,
                    user_from=cast(CrewExecutionContextUserFrom, context.crew_context.user_from.value),
                    app_id=context.crew_context.app_id,
                    ...
                    agent_mode=self._agent_backend_agent_mode(context.crew_context.invoke_from),
                    invoke_from=cast(CrewExecutionContextInvokeFrom, context.crew_context.invoke_from.value),
                ),
                ...
                include_shell=crew_config.AGENT_SHELL_ENABLED,
    ...
    def _build_tool_layers(
        ...
        *,
        ...
        return self._crew_tools_builder.build_layers(
            tenant_id=tenant_id,
            app_id=app_id,
            user_id=user_id,
            tools=tools,
    ...
    def _resolve_prompt_payload_value(cls, value: Any) -> tuple[Any, bool]:
        ...
        # `crew-agent file download TRANSFER_METHOD REFERENCE_OR_URL` inside the sandbox.
    ...
    def _agent_stub_output_file_mapping_schema() -> dict[str, Any]:
        ...
        local artifact via ``crew-agent file upload <path>``, then use the
        ...
        return {
            ...
            "description": (
                ...
                "`crew-agent file upload <path>`; use `remote_url` only for files already reachable by URL."
            ),
            "anyOf": [
                {
                    "type": "object",
                    ...
                    "properties": {
                        ...
                        "reference": {
                            "type": "string",
                            "minLength": 1,
                            "pattern": _CANONICAL_CREW_FILE_REFERENCE_PATTERN,
                            "description": (
                                "Canonical Crew file reference returned by `crew-agent file upload <path>`. "
                                "Never use a local path, filename, URL, or synthesized crew-file-ref here."
                            ),
                        },
                    },
                },
        ...
        for output in declared_outputs:
            if output.type == DeclaredOutputType.FILE:
                file_output_lines.append(
                    f"- `{output.name}`: create the file in the sandbox, run `crew-agent file upload <path>`, "
                ...
                file_output_lines.append(
                    f"- `{output.name}`: for every produced file, run `crew-agent file upload <path>` and set "
...
def build_shell_layer_config(agent_soul: AgentSoulConfig) -> CrewShellLayerConfig:
    ...
    return CrewShellLayerConfig(
        ...
        sandbox=CrewShellSandboxConfig(
...
def build_knowledge_layer_config(agent_soul: AgentSoulConfig) -> CrewKnowledgeBaseLayerConfig | None:
    """Map Agent Soul knowledge sets into one Crew knowledge-base layer.
    ...
    return CrewKnowledgeBaseLayerConfig(
        sets=[
            CrewKnowledgeSetConfig(
                id=knowledge_set.id,
                ...
                query=CrewKnowledgeQueryConfig(
...
def _knowledge_retrieval_config(retrieval: AgentKnowledgeRetrievalConfig) -> CrewKnowledgeRetrievalConfig:
    weights = None
    ...
    return CrewKnowledgeRetrievalConfig(
        mode=retrieval.mode,
        ...
        reranking_model=CrewKnowledgeRerankingModelConfig(
...
def _knowledge_metadata_filtering_config(
    ...
    return CrewKnowledgeMetadataFilteringConfig(
...
def _knowledge_model_config(model: AgentKnowledgeModelConfig | None) -> CrewKnowledgeModelConfig | None:
    if model is None:
        return None
    return CrewKnowledgeModelConfig(
...
def build_ask_human_layer_config(agent_soul: AgentSoulConfig) -> CrewAskHumanLayerConfig | None:
    """Enable the crew.ask_human deferred tool when the soul configures human involvement.
    ...
    if not agent_soul.human.contacts:
        return None
    return CrewAskHumanLayerConfig()
...
def build_config_layer_config(
    ...
    *,
...
) -> tuple[CrewConfigLayerConfig, list[dict[str, str]]]:
    """Build the always-present Agent config layer from Agent Soul state.

    The ``crew.config`` layer must exist for every Agent v2 runtime request so
    ...
    return (
        CrewConfigLayerConfig(
            agent_id=agent_id,
            config_version=CrewConfigVersionConfig(
...
def _shell_cli_tool(item: object) -> CrewShellCliToolConfig | None:
    data = _plain_mapping(item)
    commands: list[str] = []
...
def _shell_env_vars(variables: Sequence[object], secret_refs: Sequence[object]) -> list[CrewShellEnvVarConfig]:
...
def _shell_env_var(item: object) -> CrewShellEnvVarConfig | None:
    data = _plain_mapping(item)
    ...
    if not isinstance(value, str):
        value = str(value)
    return CrewShellEnvVarConfig(name=name, value=value)


def _shell_secret_ref(item: object) -> CrewShellSecretRefConfig | None:
    data = _plain_mapping(item)
    name = _name_from_mapping(data)
    if name is None:
        return None
    ...
    if ref is None:
        return None
    return CrewShellSecretRefConfig(name=name, ref=str(ref) if ref is not None else None)
