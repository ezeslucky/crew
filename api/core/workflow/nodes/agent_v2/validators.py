...
class WorkflowAgentNodeValidator:
    ...
    def _validate_node_schema(*, node_id: str, node_data: Mapping[str, Any]) -> None:
        try:
            CrewAgentNodeData.model_validate(node_data)
    ...
    def _validate_agent_soul_tools(
        ...
        *,
        ...
        for tool in agent_soul.tools.crew_tools:
            if not tool.enabled:
                continue
            ...
            if exposed_name in exposed_names:
                raise WorkflowAgentNodeValidationError(
                    f"Workflow Agent node {binding.node_id} has duplicate Crew Plugin Tool name {exposed_name}."
                )
