...
from core.app.entities.app_invoke_entities import InvokeFrom, UserFrom, build_crew_run_context
from core.app.file_access import DatabaseFileAccessController
from core.app.workflow.layers.llm_quota import LLMQuotaLayer
from core.app.workflow.layers.observability import ObservabilityLayer
from core.workflow.node_factory import (
    CrewGraphInitContext,
    CrewNodeFactory,
    is_start_node_type,
...
def iter_crew_graph_engine_events(
    engine: GraphEngine,
    response_stream_filter: ResponseStreamFilter | None = None,
) -> Generator[GraphEngineEvent, None, None]:
    """
    Apply Crew's response streaming compatibility filter to GraphEngine events.

    Graphon v0.5.0 emits raw variable stream chunks and requires callers to opt
    into the legacy response-ordered stream behavior that Crew exposes to its
...
class WorkflowEntry:
    def __init__(
        ...
        """
        # check call depth
        workflow_call_max_depth = crew_config.WORKFLOW_CALL_MAX_DEPTH
        ...
        self.graph_engine = GraphEngine(
            ...
            command_channel=command_channel,
            config=GraphEngineConfig(
                min_workers=crew_config.GRAPH_ENGINE_MIN_WORKERS,
                max_workers=crew_config.GRAPH_ENGINE_MAX_WORKERS,
                scale_up_threshold=crew_config.GRAPH_ENGINE_SCALE_UP_THRESHOLD,
                scale_down_idle_time=crew_config.GRAPH_ENGINE_SCALE_DOWN_IDLE_TIME,
            ),
            child_engine_builder=self._child_engine_builder,
        )

        # Add debug logging layer when in debug mode
        if crew_config.DEBUG:
        ...
        limits_layer = ExecutionLimitsLayer(
            max_steps=crew_config.WORKFLOW_MAX_EXECUTION_STEPS, max_time=crew_config.WORKFLOW_MAX_EXECUTION_TIME
        )
        self.graph_engine.layer(limits_layer)
        self.graph_engine.layer(LLMQuotaLayer(tenant_id=tenant_id))

        # Add observability layer when OTel is enabled
        if crew_config.ENABLE_OTEL or is_instrument_flag_enabled():
    ...
    def run(self) -> Generator[GraphEngineEvent, None, None]:
        ...
        try:
            ...
            generator = iter_crew_graph_engine_events(graph_engine, self._response_stream_filter)
    ...
    def single_step_run(
        ...
        *,
        ...
        run_context = build_crew_run_context(
        ...
        node_factory = CrewNodeFactory.from_graph_init_context(
