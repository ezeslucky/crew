"""Validate Crew Console KnowledgeFS declarations against a pinned OpenAPI document.

The OpenAPI document is exported only during explicit development validation. Runtime declarations live with Crew
...
class ContractDeclaration(TypedDict):
    """KnowledgeFS transport contract declared by one Crew Console registry entry."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="crew-knowledge-fs-contract-") as directory:
...
def validate_declarations(document: dict[str, Any], declarations: tuple[ContractDeclaration, ...]) -> None:
    """Validate Crew Console declarations against matching pinned OpenAPI operations."""
    ...
    for declaration in declarations:
        ...
        if operation_id in declared_ids:
            raise ValueError(f"Crew Console registry has duplicate operationId: {operation_id}")
