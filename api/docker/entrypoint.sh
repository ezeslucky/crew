...
if [[ "${MODE}" == "worker" ]]; then
  ...
  if [[ $# -eq 0 ]]; then
    ...
    echo ""
    echo "  Docker:"
    echo "    docker run -e MODE=job crew-api create-tenant --email admin@example.com"
    echo ""
  ...
  if [[ "${DEBUG}" == "true" ]]; then
    export HOST=${CREW_BIND_ADDRESS:-0.0.0.0}
    export PORT=${CREW_PORT:-5001}
    exec python -m app
  else
    exec gunicorn \
      --no-control-socket \
      --bind "${CREW_BIND_ADDRESS:-0.0.0.0}:${CREW_PORT:-5001}" \
