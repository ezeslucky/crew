...
All token metrics (crew.tokens.input, crew.tokens.output, crew.tokens.total) use the
...
class EnterpriseOtelTrace:
    ...
    def _common_attrs(self, trace_info: BaseTraceInfo) -> dict[str, Any]:
        ...
        return {
            "crew.trace_id": trace_info.resolved_trace_id,
            "crew.tenant_id": tenant_id,
            "crew.app_id": app_id,
            "crew.app.name": metadata.get("app_name"),
            "crew.workspace.name": metadata.get("workspace_name"),
            "gen_ai.user.id": user_id,
            "crew.message.id": trace_info.message_id,
        }
    ...
    def _workflow_trace(self, info: WorkflowTraceInfo) -> None:
        ...
        span_attrs: dict[str, Any] = {
            "crew.trace_id": info.resolved_trace_id,
            "crew.tenant_id": tenant_id,
            "crew.app_id": app_id,
            "crew.workflow.id": info.workflow_id,
            "crew.workflow.run_id": info.workflow_run_id,
            "crew.workflow.status": info.workflow_run_status,
            "crew.workflow.error": info.error,
            "crew.workflow.elapsed_time": info.workflow_run_elapsed_time,
            "crew.invoke_from": metadata.get("triggered_from"),
            "crew.conversation.id": info.conversation_id,
            "crew.message.id": info.message_id,
            "crew.invoked_by": info.invoked_by,
        ...
        if isinstance(parent_ctx, dict):
            ...
            span_attrs["crew.parent.trace_id"] = parent_ctx_dict.get("trace_id")
            span_attrs["crew.parent.node.execution_id"] = parent_ctx_dict.get("parent_node_execution_id")
            span_attrs["crew.parent.workflow.run_id"] = parent_ctx_dict.get("parent_workflow_run_id")
            span_attrs["crew.parent.app.id"] = parent_ctx_dict.get("parent_app_id")
        ...
        log_attrs.update(
            {
                ...
                "gen_ai.user.id": user_id,
                "gen_ai.usage.total_tokens": info.total_tokens,
                "crew.workflow.version": info.workflow_run_version,
            }
        )

        ref = f"ref:workflow_run_id={info.workflow_run_id}"
        log_attrs["crew.workflow.inputs"] = self._content_or_ref(info.workflow_run_inputs, ref)
        log_attrs["crew.workflow.outputs"] = self._content_or_ref(info.workflow_run_outputs, ref)
        log_attrs["crew.workflow.query"] = self._content_or_ref(info.query, ref)
    ...
    def _emit_node_execution_trace(
        ...
        span_attrs: dict[str, Any] = {
            ...
            "crew.workflow.run_id": info.workflow_run_id,
            "crew.message.id": info.message_id,
            "crew.conversation.id": metadata.get("conversation_id"),
            "crew.node.execution_id": info.node_execution_id,
            "crew.node.id": info.node_id,
            "crew.node.type": info.node_type,
            "crew.node.title": info.title,
            "crew.node.status": info.status,
            "crew.node.error": info.error,
            "crew.node.elapsed_time": info.elapsed_time,
            "crew.node.index": info.index,
            "crew.node.predecessor_node_id": info.predecessor_node_id,
            "crew.node.iteration_id": info.iteration_id,
            "crew.node.loop_id": info.loop_id,
            "crew.node.parallel_id": info.parallel_id,
            "crew.node.invoked_by": info.invoked_by,
        ...
        log_attrs.update(
            {
                "crew.app.name": metadata.get("app_name"),
                "crew.workspace.name": metadata.get("workspace_name"),
                "crew.invoke_from": metadata.get("invoke_from"),
                "gen_ai.user.id": user_id,
                "gen_ai.usage.total_tokens": info.total_tokens,
                "crew.node.total_price": info.total_price,
                "crew.node.currency": info.currency,
                "gen_ai.provider.name": info.model_provider,
                "gen_ai.request.model": info.model_name,
                "gen_ai.tool.name": info.tool_name,
                "crew.node.iteration_index": info.iteration_index,
                "crew.node.loop_index": info.loop_index,
                "crew.plugin.name": metadata.get("plugin_name"),
                "crew.credential.name": metadata.get("credential_name"),
                "crew.credential.id": metadata.get("credential_id"),
                "crew.dataset.ids": self._maybe_json(metadata.get("dataset_ids")),
                "crew.dataset.names": self._maybe_json(metadata.get("dataset_names")),
            }
        )

        ref = f"ref:node_execution_id={info.node_execution_id}"
        log_attrs["crew.node.inputs"] = self._content_or_ref(info.node_inputs, ref)
        log_attrs["crew.node.outputs"] = self._content_or_ref(info.node_outputs, ref)
        log_attrs["crew.node.process_data"] = self._content_or_ref(info.process_data, ref)
    ...
    def _message_trace(self, info: MessageTraceInfo) -> None:
        ...
        attrs.update(
            {
                "crew.invoke_from": metadata.get("from_source"),
                "crew.conversation.id": metadata.get("conversation_id"),
                "crew.conversation.mode": info.conversation_mode,
                ...
                "gen_ai.usage.total_tokens": info.total_tokens,
                "crew.message.status": metadata.get("status"),
                "crew.message.error": info.error,
                "crew.message.from_source": metadata.get("from_source"),
                "crew.message.from_end_user_id": metadata.get("from_end_user_id"),
                "crew.message.from_account_id": metadata.get("from_account_id"),
                "crew.streaming": info.is_streaming_request,
                "crew.message.time_to_first_token": info.gen_ai_server_time_to_first_token,
                "crew.message.streaming_duration": info.llm_streaming_time_to_generate,
                "crew.workflow.run_id": metadata.get("workflow_run_id"),
            }
        )

        if info.start_time and info.end_time:
            attrs["crew.message.duration"] = (info.end_time - info.start_time).total_seconds()

        node_execution_id = metadata.get("node_execution_id")
        if node_execution_id:
            attrs["crew.node.execution_id"] = node_execution_id
        ...
        attrs["crew.message.inputs"] = self._content_or_ref(inputs, ref)
        attrs["crew.message.outputs"] = self._content_or_ref(outputs, ref)
    ...
    def _tool_trace(self, info: ToolTraceInfo) -> None:
        ...
        attrs.update(
            {
                "crew.tool.name": info.tool_name,
                "crew.tool.duration": float(info.time_cost),
                "crew.tool.status": "failed" if info.error else "succeeded",
                "crew.tool.error": info.error,
                "crew.workflow.run_id": metadata.get("workflow_run_id"),
            }
        )
        node_execution_id = metadata.get("node_execution_id")
        ...
        ref = f"ref:message_id={info.message_id}"
        attrs["crew.tool.inputs"] = self._content_or_ref(info.tool_inputs, ref)
        attrs["crew.tool.outputs"] = self._content_or_ref(info.tool_outputs, ref)
        attrs["crew.tool.parameters"] = self._content_or_ref(info.tool_parameters, ref)
        attrs["crew.tool.config"] = self._content_or_ref(info.tool_config, ref)
    ...
    def _moderation_trace(self, info: ModerationTraceInfo) -> None:
        ...
        attrs.update(
            {
                "crew.moderation.flagged": info.flagged,
                "crew.moderation.action": info.action,
                "crew.moderation.preset_response": info.preset_response,
                "crew.moderation.type": metadata.get("moderation_type", "input"),
                "crew.moderation.categories": self._maybe_json(metadata.get("moderation_categories", [])),
                "crew.workflow.run_id": metadata.get("workflow_run_id"),
            }
        )
        node_execution_id = metadata.get("node_execution_id")
        ...
        attrs["crew.moderation.query"] = self._content_or_ref(
            info.query,
    ...
    def _suggested_question_trace(self, info: SuggestedQuestionTraceInfo) -> None:
        ...
        attrs.update(
            {
                "gen_ai.usage.total_tokens": info.total_tokens,
                "crew.suggested_question.status": status,
                "crew.suggested_question.error": error,
                "crew.suggested_question.duration": duration,
                "gen_ai.provider.name": info.model_provider,
                "gen_ai.request.model": info.model_id,
                "crew.suggested_question.count": len(info.suggested_question),
                "crew.workflow.run_id": metadata.get("workflow_run_id"),
            }
        )
        node_execution_id = metadata.get("node_execution_id")
        ...
        attrs["crew.suggested_question.questions"] = self._content_or_ref(
    ...
    def _dataset_retrieval_trace(self, info: DatasetRetrievalTraceInfo) -> None:
        ...
        tenant_id, app_id, user_id = self._context_ids(info, metadata)
        attrs = self._common_attrs(info)
        attrs["crew.retrieval.error"] = info.error
        attrs["crew.retrieval.status"] = "failed" if info.error else "succeeded"
        if info.start_time and info.end_time:
            attrs["crew.retrieval.duration"] = (info.end_time - info.start_time).total_seconds()
        attrs["crew.workflow.run_id"] = metadata.get("workflow_run_id")
        node_execution_id = metadata.get("node_execution_id")
        if node_execution_id:
            attrs["crew.node.execution_id"] = node_execution_id
        ...
        attrs["crew.dataset.id"] = self._maybe_json(dataset_ids)
        attrs["crew.dataset.name"] = self._maybe_json(dataset_names)
        attrs["crew.retrieval.document_count"] = len(docs)
            ...
            for ds_info in embedding_models.values():
                if isinstance(ds_info, dict):
                    ...
                    if m and m not in models:
                        models.append(m)
            attrs["crew.dataset.embedding_providers"] = self._maybe_json(providers)
            attrs["crew.dataset.embedding_models"] = self._maybe_json(models)
        ...
        if rerank_provider or rerank_model:
            attrs["crew.retrieval.rerank_provider"] = rerank_provider
            attrs["crew.retrieval.rerank_model"] = rerank_model

        ref = f"ref:message_id={info.message_id}"
        retrieval_inputs = self._safe_payload_value(info.inputs)
        attrs["crew.retrieval.query"] = self._content_or_ref(retrieval_inputs, ref)
        attrs["crew.dataset.documents"] = self._content_or_ref(structured_docs, ref)
    ...
    def _generate_name_trace(self, info: GenerateNameTraceInfo) -> None:
        ...
        tenant_id, app_id, user_id = self._context_ids(info, metadata)
        attrs = self._common_attrs(info)
        attrs["crew.conversation.id"] = info.conversation_id
        node_execution_id = metadata.get("node_execution_id")
        if node_execution_id:
            attrs["crew.node.execution_id"] = node_execution_id
        ...
        attrs["crew.generate_name.duration"] = duration
        attrs["crew.generate_name.status"] = status
        attrs["crew.generate_name.error"] = error

        ref = f"ref:conversation_id={info.conversation_id}"
        inputs = self._safe_payload_value(info.inputs)
        outputs = self._safe_payload_value(info.outputs)
        attrs["crew.generate_name.inputs"] = self._content_or_ref(inputs, ref)
        attrs["crew.generate_name.outputs"] = self._content_or_ref(outputs, ref)
    ...
    def _prompt_generation_trace(self, info: PromptGenerationTraceInfo) -> None:
        ...
        attrs = {
            ...
            "crew.tenant_id": tenant_id,
            "gen_ai.user.id": user_id,
            "crew.app_id": app_id or "",
            "crew.app.name": metadata.get("app_name"),
            "crew.workspace.name": metadata.get("workspace_name"),
            "crew.prompt_generation.operation_type": info.operation_type,
            "gen_ai.provider.name": info.model_provider,
            "gen_ai.request.model": info.model_name,
            "gen_ai.usage.input_tokens": info.prompt_tokens,
            "gen_ai.usage.output_tokens": info.completion_tokens,
            "gen_ai.usage.total_tokens": info.total_tokens,
            "crew.prompt_generation.duration": info.latency,
            "crew.prompt_generation.status": "failed" if info.error else "succeeded",
            "crew.prompt_generation.error": info.error,
        }
        node_execution_id = metadata.get("node_execution_id")
        if node_execution_id:
            attrs["crew.node.execution_id"] = node_execution_id
        ...
        if info.total_price is not None:
            attrs["crew.prompt_generation.total_price"] = info.total_price
            attrs["crew.prompt_generation.currency"] = info.currency

        ref = f"ref:trace_id={info.trace_id}"
        outputs = self._safe_payload_value(info.outputs)
        attrs["crew.prompt_generation.instruction"] = self._content_or_ref(info.instruction, ref)
        attrs["crew.prompt_generation.output"] = self._content_or_ref(outputs, ref)
