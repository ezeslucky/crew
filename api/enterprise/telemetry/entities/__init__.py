...
class EnterpriseTelemetrySpan(StrEnum):
    WORKFLOW_RUN = "crew.workflow.run"
    NODE_EXECUTION = "crew.node.execution"
    DRAFT_NODE_EXECUTION = "crew.node.execution.draft"


class EnterpriseTelemetryEvent(StrEnum):
    """Event names for enterprise telemetry logs."""

    APP_CREATED = "crew.app.created"
    APP_UPDATED = "crew.app.updated"
    APP_DELETED = "crew.app.deleted"
    FEEDBACK_CREATED = "crew.feedback.created"
    WORKFLOW_RUN = "crew.workflow.run"
    MESSAGE_RUN = "crew.message.run"
    TOOL_EXECUTION = "crew.tool.execution"
    MODERATION_CHECK = "crew.moderation.check"
    SUGGESTED_QUESTION_GENERATION = "crew.suggested_question.generation"
    DATASET_RETRIEVAL = "crew.dataset.retrieval"
    GENERATE_NAME_EXECUTION = "crew.generate_name.execution"
    PROMPT_GENERATION_EXECUTION = "crew.prompt_generation.execution"
    REHYDRATION_FAILED = "crew.telemetry.rehydration_failed"
...
class TokenMetricLabels(BaseModel):
    """Unified label structure for all crew.token.* metrics.

    All token counters (crew.tokens.input, crew.tokens.output, crew.tokens.total) MUST
