...
class EnterpriseMetricHandler:
        ...
        if not payload:
            ...
            emit_metric_only_event(
                ...
                attributes={
                    ...
                    "crew.telemetry.error": f"Payload rehydration failed for event_id={envelope.event_id}",
                    "crew.telemetry.payload_type": envelope.case,
                    "crew.telemetry.correlation_id": envelope.event_id,
                },
                tenant_id=envelope.tenant_id,
            )
    ...
    def _on_app_created(self, envelope: TelemetryEnvelope) -> None:
        ...
        attrs = {
            "crew.app_id": payload.get("app_id"),
            "crew.tenant_id": envelope.tenant_id,
            "crew.event.id": envelope.event_id,
            "crew.app.mode": payload.get("mode"),
            "crew.app.created_at": datetime.now(UTC).isoformat(),
        }
    ...
    def _on_app_updated(self, envelope: TelemetryEnvelope) -> None:
        ...
        attrs = {
            ...
            "crew.tenant_id": envelope.tenant_id,
            "crew.event.id": envelope.event_id,
            "crew.app.updated_at": datetime.now(UTC).isoformat(),
        }
    ...
    def _on_app_deleted(self, envelope: TelemetryEnvelope) -> None:
        ...
        attrs = {
            ...
            "crew.tenant_id": envelope.tenant_id,
            "crew.event.id": envelope.event_id,
            "crew.app.deleted_at": datetime.now(UTC).isoformat(),
        }
    ...
    def _on_feedback_created(self, envelope: TelemetryEnvelope) -> None:
        ...
        attrs: dict[str, Any] = {
            "crew.message.id": payload.get("message_id"),
            "crew.tenant_id": envelope.tenant_id,
            "crew.event.id": envelope.event_id,
            "crew.app_id": payload.get("app_id"),
            "crew.conversation.id": payload.get("conversation_id"),
            "gen_ai.user.id": payload.get("from_end_user_id") or payload.get("from_account_id"),
            "crew.feedback.rating": payload.get("rating"),
            "crew.feedback.from_source": payload.get("from_source"),
            "crew.feedback.created_at": datetime.now(UTC).isoformat(),
        }
        if include_content:
            attrs["crew.feedback.content"] = payload.get("content")
