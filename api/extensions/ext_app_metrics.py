...
def init_app(app: CrewApp):
    ...
    def after_request(response):
        ...
        response.headers.add("X-Version", crew_config.project.version)
        response.headers.add("X-Env", crew_config.DEPLOY_ENV)
        return response

    @app.route("/health")
    def health():
        return Response(
            json.dumps({"pid": os.getpid(), "status": "ok", "version": crew_config.project.version}),
            status=200,
