...
def _apply_cors_once(bp, /, **cors_kwargs):
    ...
    if getattr(bp, "_crew_cors_applied", False):
        return

    from flask_cors import CORS

    CORS(bp, **cors_kwargs)
    bp._crew_cors_applied = True
...
def init_app(app: CrewApp):
    ...
    if crew_config.OPENAPI_ENABLED:
        ...
        _apply_cors_once(
            openapi_bp,
            resources={r"/*": {"origins": crew_config.OPENAPI_CORS_ALLOW_ORIGINS}},
    ...
    _apply_cors_once(
        ...
        resources={
            ...
            r"^/chat-messages$": {
                "origins": crew_config.WEB_API_CORS_ALLOW_ORIGINS,
    ...
    _apply_cors_once(
        console_app_bp,
        resources={r"/*": {"origins": crew_config.CONSOLE_CORS_ALLOW_ORIGINS}},
        supports_credentials=True,
        allow_headers=list(AUTHENTICATED_HEADERS),
        methods=["GET", "PUT", "POST", "DELETE", "OPTIONS", "PATCH"],
        expose_headers=list(EXPOSED_HEADERS),
