...
def get_celery_ssl_options() -> CelerySSLOptionsDict | None:
    ...
    if not crew_config.BROKER_USE_SSL:
        return None

    # Check if Celery is actually using Redis
    broker_is_redis = crew_config.CELERY_BROKER_URL and (
        crew_config.CELERY_BROKER_URL.startswith("redis://") or crew_config.CELERY_BROKER_URL.startswith("rediss://")
    )
    ...
    ssl_cert_reqs = cert_reqs_map.get(crew_config.REDIS_SSL_CERT_REQS, ssl.CERT_NONE)

    return CelerySSLOptionsDict(
        ssl_cert_reqs=ssl_cert_reqs,
        ssl_ca_certs=crew_config.REDIS_SSL_CA_CERTS,
        ssl_certfile=crew_config.REDIS_SSL_CERTFILE,
        ssl_keyfile=crew_config.REDIS_SSL_KEYFILE,
    )
...
def get_celery_broker_transport_options() -> CelerySentinelTransportDict | dict[str, Any]:
    ...
    if crew_config.CELERY_USE_SENTINEL:
        transport_options = CelerySentinelTransportDict(
            master_name=crew_config.CELERY_SENTINEL_MASTER_NAME,
            sentinel_kwargs=_CelerySentinelKwargsDict(
                socket_timeout=crew_config.CELERY_SENTINEL_SOCKET_TIMEOUT,
                password=crew_config.CELERY_SENTINEL_PASSWORD,
            ),
        )
    else:
        transport_options = {}
...
def get_celery_redis_global_keyprefix() -> str | None:
    ...
    normalized_prefix = normalize_redis_key_prefix(crew_config.REDIS_KEY_PREFIX)
    if not normalized_prefix:
        return None
    return f"{normalized_prefix}:"


def init_app(app: CrewApp) -> Celery:
    ...
    celery_app = Celery(
        app.name,
        task_cls=FlaskTask,
        broker=crew_config.CELERY_BROKER_URL,
        backend=crew_config.CELERY_BACKEND,
    )

    celery_app.conf.update(
        result_backend=crew_config.CELERY_RESULT_BACKEND,
        broker_transport_options=broker_transport_options,
        broker_connection_retry_on_startup=True,
        worker_log_format=crew_config.LOG_FORMAT,
        worker_task_log_format=crew_config.LOG_FORMAT,
        worker_hijack_root_logger=False,
        timezone=pytz.timezone(crew_config.LOG_TZ or "UTC"),
        task_ignore_result=True,
        task_annotations=crew_config.CELERY_TASK_ANNOTATIONS,
    )

    if crew_config.CELERY_BACKEND == "redis":
        celery_app.conf.update(
    ...
    if ssl_options:
        celery_app.conf.update(
            ...
            redis_backend_use_ssl=ssl_options if crew_config.CELERY_BACKEND == "redis" else None,
        )

    if crew_config.LOG_FILE:
        celery_app.conf.update(
            worker_logfile=crew_config.LOG_FILE,
        )
    ...
    day = crew_config.CELERY_BEAT_SCHEDULER_TIME

    # if you add a new task, please add the switch to CeleryScheduleTasksConfig
    beat_schedule: dict[str, CeleryBeatScheduleEntry] = {}
    if crew_config.ENABLE_CLEAN_EMBEDDING_CACHE_TASK:
    ...
    if crew_config.ENABLE_CLEAN_UNUSED_DATASETS_TASK:
    ...
    if crew_config.ENABLE_CREATE_TIDB_SERVERLESS_TASK:
    ...
    if crew_config.ENABLE_UPDATE_TIDB_SERVERLESS_STATUS_TASK:
    ...
    if crew_config.ENABLE_CLEAN_MESSAGES:
    ...
    if crew_config.ENABLE_MAIL_CLEAN_DOCUMENT_NOTIFY_TASK:
    ...
    if crew_config.ENABLE_DATASETS_QUEUE_MONITOR:
        ...
        beat_schedule["datasets-queue-monitor"] = {
            ...
            "schedule": timedelta(minutes=crew_config.QUEUE_MONITOR_INTERVAL or 30),
        }
    if crew_config.ENABLE_HUMAN_INPUT_TIMEOUT_TASK:
        ...
        beat_schedule["human_input_form_timeout"] = {
            ...
            "schedule": timedelta(minutes=crew_config.HUMAN_INPUT_TIMEOUT_TASK_INTERVAL),
        }
    if crew_config.ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK and crew_config.MARKETPLACE_ENABLED:
    ...
    if crew_config.WORKFLOW_LOG_CLEANUP_ENABLED:
        # 2:00 AM every day
    ...
    if crew_config.ENABLE_WORKFLOW_RUN_CLEANUP_TASK:
        # for saas only
    ...
    if crew_config.ENABLE_CLEAN_OAUTH_ACCESS_TOKENS_TASK:
    ...
    if crew_config.ENABLE_WORKFLOW_SCHEDULE_POLLER_TASK:
        ...
        beat_schedule["workflow_schedule_task"] = {
            ...
            "schedule": timedelta(minutes=crew_config.WORKFLOW_SCHEDULE_POLLER_INTERVAL),
        }
    if crew_config.ENABLE_TRIGGER_PROVIDER_REFRESH_TASK:
        ...
        beat_schedule["trigger_provider_refresh"] = {
            ...
            "schedule": timedelta(minutes=crew_config.TRIGGER_PROVIDER_REFRESH_INTERVAL),
        }

    if crew_config.ENABLE_API_TOKEN_LAST_USED_UPDATE_TASK:
        ...
        beat_schedule["batch_update_api_token_last_used"] = {
            ...
            "schedule": timedelta(minutes=crew_config.API_TOKEN_LAST_USED_UPDATE_INTERVAL),
        }

    if crew_config.ENTERPRISE_ENABLED and crew_config.ENTERPRISE_TELEMETRY_ENABLED:
