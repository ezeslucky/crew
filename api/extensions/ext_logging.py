"""Logging extension for Crew Flask application."""
...
def init_app(app: CrewApp):
    ...
    if log_file:
        ...
        log_handlers.append(
            RotatingFileHandler(
                filename=log_file,
                maxBytes=crew_config.LOG_FILE_MAX_SIZE * 1024 * 1024,
                backupCount=crew_config.LOG_FILE_BACKUP_COUNT,
            )
        )
...
def apply_request_id_formatter():
    ...
    for handler in logging.root.handlers:
        if handler.formatter:
            handler.formatter = RequestIdFormatter(crew_config.LOG_FORMAT, crew_config.LOG_DATEFORMAT)
