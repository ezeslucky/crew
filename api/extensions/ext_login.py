...
class CrewLoginManager(flask_login.LoginManager):
    """Project-specific Flask-Login manager with a stable unauthorized contract.

    Crew registers `unauthorized_handler` below to always return a JSON `Response`.
...
def _load_user_from_request(request_from_flask_login: Request, session: Session) -> LoginUser | None:
    ...
    if crew_config.SWAGGER_UI_ENABLED and request.path.endswith((crew_config.SWAGGER_UI_PATH, "/swagger.json")):
        return None

    auth_token = extract_access_token(request)

    # Check for admin API key authentication first
    if crew_config.ADMIN_API_KEY_ENABLE and auth_token:
        admin_api_key = crew_config.ADMIN_API_KEY
