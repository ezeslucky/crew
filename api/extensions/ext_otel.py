...
def init_app(app: CrewApp):
    ...
    resource = Resource(
        attributes={
            SERVICE_NAME: crew_config.APPLICATION_NAME,
            SERVICE_VERSION: f"crew-{crew_config.project.version}-{crew_config.COMMIT_SHA}",
            PROCESS_PID: os.getpid(),
            DEPLOYMENT_ENVIRONMENT_NAME: f"{crew_config.DEPLOY_ENV}-{crew_config.EDITION}",
            HOST_NAME: socket.gethostname(),
            HOST_ARCH: platform.machine(),
            "custom.deployment.git_commit": crew_config.COMMIT_SHA,
    ...
    sampler = ParentBasedTraceIdRatio(crew_config.OTEL_SAMPLING_RATE)
    ...
    protocol = (crew_config.OTEL_EXPORTER_OTLP_PROTOCOL or "").lower()
    if crew_config.OTEL_EXPORTER_TYPE == "otlp":
        if protocol == "grpc":
            # Auto-detect TLS: https:// uses secure, everything else is insecure
            endpoint = crew_config.OTLP_BASE_ENDPOINT
            insecure = not endpoint.startswith("https://")

            # Header field names must consist of lowercase letters, check RFC7540
            grpc_headers = (
                (("authorization", f"Bearer {crew_config.OTLP_API_KEY}"),) if crew_config.OTLP_API_KEY else ()
            )
            ...
            headers = {"Authorization": f"Bearer {crew_config.OTLP_API_KEY}"} if crew_config.OTLP_API_KEY else None

            trace_endpoint = crew_config.OTLP_TRACE_ENDPOINT
            if not trace_endpoint:
                trace_endpoint = crew_config.OTLP_BASE_ENDPOINT + "/v1/traces"
            exporter = HTTPSpanExporter(
                endpoint=trace_endpoint,
                headers=headers,
            )

            metric_endpoint = crew_config.OTLP_METRIC_ENDPOINT
            if not metric_endpoint:
                metric_endpoint = crew_config.OTLP_BASE_ENDPOINT + "/v1/metrics"
    ...
    provider.add_span_processor(
        BatchSpanProcessor(
            exporter,
            max_queue_size=crew_config.OTEL_MAX_QUEUE_SIZE,
            schedule_delay_millis=crew_config.OTEL_BATCH_EXPORT_SCHEDULE_DELAY,
            max_export_batch_size=crew_config.OTEL_MAX_EXPORT_BATCH_SIZE,
            export_timeout_millis=crew_config.OTEL_BATCH_EXPORT_TIMEOUT,
        )
    )
    reader = PeriodicExportingMetricReader(
        metric_exporter,
        export_interval_millis=crew_config.OTEL_METRIC_EXPORT_INTERVAL,
        export_timeout_millis=crew_config.OTEL_METRIC_EXPORT_TIMEOUT,
    )
...
def is_enabled():
    return crew_config.ENABLE_OTEL
