...
class RedisClientWrapper:
    ...
    def _get_prefix(self) -> str:
        return crew_config.REDIS_KEY_PREFIX
...
def _get_ssl_configuration() -> tuple[type[Union[Connection, SSLConnection]], dict[str, Any]]:
    ...
    if not crew_config.REDIS_USE_SSL:
        return Connection, {}
...
def _get_cache_configuration() -> CacheConfig | None:
    ...
    if not crew_config.REDIS_ENABLE_CLIENT_SIDE_CACHE:
        return None

    resp_protocol = crew_config.REDIS_SERIALIZATION_PROTOCOL
    if resp_protocol < 3:
...
def _get_retry_policy() -> Retry:
    ...
    return Retry(
        backoff=ExponentialWithJitterBackoff(
            base=crew_config.REDIS_RETRY_BACKOFF_BASE,
            cap=crew_config.REDIS_RETRY_BACKOFF_CAP,
        ),
        retries=crew_config.REDIS_RETRY_RETRIES,
        supported_errors=(
            ConnectionError,
            TimeoutError,
            BrokenPipeError,
...
def _get_connection_health_params() -> RedisHealthParamsDict:
    ...
    if sys.platform == "linux":
        socket_keepalive_options[socket.TCP_KEEPIDLE] = crew_config.REDIS_KEEPALIVE_IDLE
        socket_keepalive_options[socket.TCP_KEEPINTVL] = crew_config.REDIS_KEEPALIVE_INTERVAL
        socket_keepalive_options[socket.TCP_KEEPCNT] = crew_config.REDIS_KEEPALIVE_COUNT
    elif sys.platform == "darwin":
        socket_keepalive_options[socket.TCP_KEEPALIVE] = crew_config.REDIS_KEEPALIVE_IDLE

    return RedisHealthParamsDict(
        retry=_get_retry_policy(),
        socket_timeout=crew_config.REDIS_SOCKET_TIMEOUT,
        socket_connect_timeout=crew_config.REDIS_SOCKET_CONNECT_TIMEOUT,
        health_check_interval=crew_config.REDIS_HEALTH_CHECK_INTERVAL,
        socket_keepalive=crew_config.REDIS_KEEPALIVE,
...
def _get_base_redis_params() -> RedisBaseParamsDict:
    ...
    return RedisBaseParamsDict(
        username=crew_config.REDIS_USERNAME,
        password=crew_config.REDIS_PASSWORD or None,
        db=crew_config.REDIS_DB,
        encoding="utf-8",
        encoding_errors="strict",
        decode_responses=False,
        protocol=crew_config.REDIS_SERIALIZATION_PROTOCOL,
...
def _create_sentinel_client(redis_params: RedisBaseParamsDict) -> Union[redis.Redis, RedisCluster]:
    ...
    if not crew_config.REDIS_SENTINELS:
        raise ValueError("REDIS_SENTINELS must be set when REDIS_USE_SENTINEL is True")

    if not crew_config.REDIS_SENTINEL_SERVICE_NAME:
        raise ValueError("REDIS_SENTINEL_SERVICE_NAME must be set when REDIS_USE_SENTINEL is True")

    sentinel_hosts = [(node.split(":")[0], int(node.split(":")[1])) for node in crew_config.REDIS_SENTINELS.split(",")]

    health_params = _get_connection_health_params()

    sentinel_kwargs = {
        "socket_timeout": crew_config.REDIS_SENTINEL_SOCKET_TIMEOUT,
        "username": crew_config.REDIS_SENTINEL_USERNAME,
        "password": crew_config.REDIS_SENTINEL_PASSWORD,
        "socket_keepalive": health_params["socket_keepalive"],
        "socket_keepalive_options": health_params["socket_keepalive_options"],
    }

    if crew_config.REDIS_MAX_CONNECTIONS:
        sentinel_kwargs["max_connections"] = crew_config.REDIS_MAX_CONNECTIONS
    ...
    master: redis.Redis = sentinel.master_for(crew_config.REDIS_SENTINEL_SERVICE_NAME, **params)
    return master


def _create_cluster_client() -> Union[redis.Redis, RedisCluster]:
    """Create Redis cluster client."""
    if not crew_config.REDIS_CLUSTERS:
        raise ValueError("REDIS_CLUSTERS must be set when REDIS_USE_CLUSTERS is True")

    nodes = [
        ClusterNode(host=node.split(":")[0], port=int(node.split(":")[1]))
        for node in crew_config.REDIS_CLUSTERS.split(",")
    ]

    cluster_kwargs: dict[str, Any] = {
        "startup_nodes": nodes,
        "password": crew_config.REDIS_CLUSTERS_PASSWORD,
        "protocol": crew_config.REDIS_SERIALIZATION_PROTOCOL,
        "cache_config": _get_cache_configuration(),
        **_get_cluster_connection_health_params(),
    }
    if crew_config.REDIS_MAX_CONNECTIONS:
        cluster_kwargs["max_connections"] = crew_config.REDIS_MAX_CONNECTIONS
...
def _create_standalone_client(redis_params: RedisBaseParamsDict) -> Union[redis.Redis, RedisCluster]:
    ...
    params: dict[str, Any] = {
        **redis_params,
        "host": crew_config.REDIS_HOST,
        "port": crew_config.REDIS_PORT,
        "connection_class": connection_class,
    }

    if crew_config.REDIS_MAX_CONNECTIONS:
        params["max_connections"] = crew_config.REDIS_MAX_CONNECTIONS
...
def _create_pubsub_client(pubsub_url: str, use_clusters: bool) -> redis.Redis | RedisCluster:
    max_conns = crew_config.REDIS_MAX_CONNECTIONS
...
def init_app(app: CrewApp):
    ...
    if crew_config.normalized_pubsub_redis_url:
        _pubsub_redis_client = _create_pubsub_client(
            crew_config.normalized_pubsub_redis_url, crew_config.PUBSUB_REDIS_USE_CLUSTERS
        )
...
def get_pubsub_broadcast_channel() -> BroadcastChannelProtocol:
    ...
    if crew_config.PUBSUB_REDIS_CHANNEL_TYPE == "sharded":
        return ShardedRedisBroadcastChannel(_pubsub_redis_client)
    if crew_config.PUBSUB_REDIS_CHANNEL_TYPE == "streams":
        return StreamsBroadcastChannel(
            _pubsub_redis_client,
            retention_seconds=crew_config.PUBSUB_STREAMS_RETENTION_SECONDS,
        )
