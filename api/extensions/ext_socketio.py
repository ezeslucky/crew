...
def _get_ssl_cert_reqs() -> ssl.VerifyMode:
    ...
    return cert_reqs_map.get(crew_config.REDIS_SSL_CERT_REQS, ssl.CERT_NONE)
...
def _build_redis_options(redis_url: str) -> dict[str, Any]:
    ...
    if crew_config.REDIS_MAX_CONNECTIONS:
        options["max_connections"] = crew_config.REDIS_MAX_CONNECTIONS
...
def create_socketio_client_manager() -> Any:
    ...
    pub/sub channel. The channel name follows Crew's Redis key prefix to keep independent deployments isolated.
    """
    redis_url = crew_config.normalized_pubsub_redis_url
...
sio = cast(
    ...
    socketio.Server(
        ...
        cors_allowed_origins=crew_config.CONSOLE_CORS_ALLOW_ORIGINS,
    ),
)
