...
class SupportsFlaskInstrumentor(Protocol):
    def instrument_app(
        self, app: CrewApp, response_hook: Callable[[Span, str, list], None] | None = None, **kwargs: object
    ) -> None: ...
...
def init_flask_instrumentor(app: CrewApp) -> None:
    meter = get_meter("http_metrics", version=crew_config.project.version)
...
def init_sqlalchemy_instrumentor(app: CrewApp) -> None:
...
def init_instruments(app: CrewApp) -> None:
