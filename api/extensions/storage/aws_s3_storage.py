...
class AwsS3Storage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        self.bucket_name = crew_config.S3_BUCKET_NAME
        if crew_config.S3_USE_AWS_MANAGED_IAM:
            logger.info("Using AWS managed IAM role for S3")

            session = boto3.Session()
            region_name = crew_config.S3_REGION
            ...
            self.client = boto3.client(
                "s3",
                aws_secret_access_key=crew_config.S3_SECRET_KEY,
                aws_access_key_id=crew_config.S3_ACCESS_KEY,
                endpoint_url=crew_config.S3_ENDPOINT,
                region_name=crew_config.S3_REGION,
                config=Config(s3={"addressing_style": crew_config.S3_ADDRESS_STYLE}),
            )
        # create bucket
        try:
