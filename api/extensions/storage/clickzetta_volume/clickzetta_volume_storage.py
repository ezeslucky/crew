...
class ClickZettaVolumeConfig(BaseModel):
    ...
    crew_prefix: str = "crew_km"  # Directory prefix for User Volume
    ...
    def validate_config(cls, values: dict[str, Any]):
        ...
        values.setdefault("schema_name", get_env_with_fallback("CLICKZETTA_VOLUME_SCHEMA", "CLICKZETTA_SCHEMA", "crew"))
        ...
        values.setdefault("crew_prefix", os.getenv("CLICKZETTA_VOLUME_CREW_PREFIX", "crew_km"))
...
class ClickZettaVolumeStorage(BaseStorage):
    ...
    def _get_volume_path(self, filename: str, dataset_id: str | None = None) -> str:
        ...
        if self._config.volume_type == "user":
            # Add crew prefix for User Volume to organize files
            return f"{self._config.crew_prefix}/{filename}"
            ...
            if dataset_id in ["upload_files", "temp", "cache", "tools", "website_files", "privkeys"]:
                # Use User Volume with crew prefix for special directories
                return f"{self._config.crew_prefix}/{filename}"
    ...
    def _get_volume_sql_prefix(self, dataset_id: str | None = None) -> str:
        ...
        if self._config.volume_type == "user":
            ...
            # For crew's current file storage pattern, most files are stored in
    ...
    def save(self, filename: str, data: bytes):
        ...
        try:
            ...
            # Get the actual volume path (may include crew_km prefix)
            volume_path = self._get_volume_path(filename, dataset_id)

            # For User Volume, use the full path with crew_km prefix
    ...
    def scan(self, path: str, files: bool = True, directories: bool = False) -> list[str]:
        ...
        try:
            ...
            # For User Volume, add crew prefix to path
            if volume_prefix == "USER VOLUME":
                if path:
                    scan_path = f"{self._config.crew_prefix}/{path}"
                    sql = f"LIST {volume_prefix} SUBDIRECTORY '{scan_path}'"
                else:
                    sql = f"LIST {volume_prefix} SUBDIRECTORY '{self._config.crew_prefix}'"
            else:
                if path:
            ...
            if rows:
                for row in rows:
                    ...
                    # For User Volume, remove crew prefix from results
                    crew_prefix_with_slash = f"{self._config.crew_prefix}/"
                    if volume_prefix == "USER VOLUME" and file_path.startswith(crew_prefix_with_slash):
                        file_path = file_path[len(crew_prefix_with_slash) :]  # Remove prefix
