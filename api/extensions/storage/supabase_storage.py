...
class SupabaseStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        if crew_config.SUPABASE_URL is None:
            raise ValueError("SUPABASE_URL is not set")
        if crew_config.SUPABASE_API_KEY is None:
            raise ValueError("SUPABASE_API_KEY is not set")
        if crew_config.SUPABASE_BUCKET_NAME is None:
            raise ValueError("SUPABASE_BUCKET_NAME is not set")

        self.bucket_name = crew_config.SUPABASE_BUCKET_NAME
        self.client = Client(supabase_url=crew_config.SUPABASE_URL, supabase_key=crew_config.SUPABASE_API_KEY)
        self.create_bucket(id=crew_config.SUPABASE_BUCKET_NAME, bucket_name=crew_config.SUPABASE_BUCKET_NAME)
