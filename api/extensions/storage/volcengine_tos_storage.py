...
class VolcengineTosStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        if not crew_config.VOLCENGINE_TOS_ACCESS_KEY:
            raise ValueError("VOLCENGINE_TOS_ACCESS_KEY is not set")
        if not crew_config.VOLCENGINE_TOS_SECRET_KEY:
            raise ValueError("VOLCENGINE_TOS_SECRET_KEY is not set")
        if not crew_config.VOLCENGINE_TOS_ENDPOINT:
            raise ValueError("VOLCENGINE_TOS_ENDPOINT is not set")
        if not crew_config.VOLCENGINE_TOS_REGION:
            raise ValueError("VOLCENGINE_TOS_REGION is not set")
        self.bucket_name = crew_config.VOLCENGINE_TOS_BUCKET_NAME
        self.client = tos.TosClientV2(
            ak=crew_config.VOLCENGINE_TOS_ACCESS_KEY,
            sk=crew_config.VOLCENGINE_TOS_SECRET_KEY,
            endpoint=crew_config.VOLCENGINE_TOS_ENDPOINT,
            region=crew_config.VOLCENGINE_TOS_REGION,
        )
