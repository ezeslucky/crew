# AGENTS.md — crewctl (TypeScript CLI)

TypeScript port of crewctl. Stack: custom CLI framework (`src/framework/`), Node 22+, ESM, ky for HTTP, Vitest, and Vite+ formatting and linting.
...
- **Every leaf command extends `CrewCommand`.** Add `static agentGuide` string when command benefits from agent workflow docs — see `src/commands/AGENTS.md`.

## Layering

| Layer     | Path                             | Role                                                                                                                                  |
| --------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| commands  | `src/commands/`                  | Command class shells (extend `CrewCommand`). Only place framework imports run.                                                        |
...
- Behavior tests run against real Hono mock at `test/fixtures/crew-mock/`. No `nock`, `msw`, or `fetchMock` — every test exercises real HTTP.
...
- `test/fixtures/crew-mock/` public surface (endpoints, JSON shapes, status codes, scenario names) — that's the crew-api contract.
