...
Onboarding ref for `crew/cli/` contributors. Cover canonical patterns, layer contracts, scaffolding recipe, dev workflow, anti-patterns. Read before adding command or touching shared infra.
...
| File       | Responsibility                                                                          |
| ---------- | --------------------------------------------------------------------------------------- |
| `index.ts` | `CrewCommand` subclass. Flag/arg declaration + `run()` wiring only. No business logic.  |
...
- [ ] `index.ts` extends `CrewCommand`
- [ ] Authed command calls `this.authedCtx()`; non-authed skips
- [ ] No try/catch in `run()` — `CrewCommand.catch()` handles `BaseError`
...
## CrewCommand base class

All commands extend `CrewCommand`, not `Command`.

```typescript
export default class MyCommand extends CrewCommand {
...
throw new BaseError({
  ...
  hint: "pass --workspace or run 'crewctl use workspace <id>'",
})
```

`ErrorCode` exhaustive const object — never use raw strings. `exitFor(code)` maps to exit codes auto. `CrewCommand.catch()` calls `formatErrorForCli` with `outputFormat` so JSON/YAML consumers get machine-readable error output.
...
Tests live in `run.test.ts` alongside command. Test `run.ts` direct — never the `CrewCommand` class.
...
### crew-mock fixture server

`test/fixtures/crew-mock/server.ts` mirrors `/openapi/v1/*`. Each test starts isolated instance:

```typescript
import { startMock } from '../../../test/fixtures/crew-mock/server.js'
...
| `try { ... } catch (e) { if (isBaseError(e)) ... }` in every command | Throw `BaseError`; `CrewCommand.catch()` handles                           |
