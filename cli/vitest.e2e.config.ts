...
export default defineConfig({
  ...
  pack: {
    ...
    target: 'node22',
    define: {
      __CREWCTL_VERSION__: JSON.stringify(buildInfo.version),
      __CREWCTL_COMMIT__: JSON.stringify(buildInfo.commit),
      __CREWCTL_BUILD_DATE__: JSON.stringify(buildInfo.buildDate),
      __CREWCTL_CHANNEL__: JSON.stringify(buildInfo.channel),
      __CREWCTL_MIN_CREW__: JSON.stringify(buildInfo.minCrew),
      __CREWCTL_MAX_CREW__: JSON.stringify(buildInfo.maxCrew),
    },
  },
  test: {
    environment: 'node',
    ...
    setupFiles: [],
    // CREW_E2E_INCLUDE: comma-separated glob patterns, e.g.
    //   CREW_E2E_INCLUDE="test/e2e/suites/run/run-app-basic.e2e.ts"
    //   CREW_E2E_INCLUDE="test/e2e/suites/run/**/*.e2e.ts"
    //   CREW_E2E_INCLUDE="test/e2e/suites/discovery/**/*.e2e.ts,test/e2e/suites/run/run-app-basic.e2e.ts"
    // Deprecated alias: CREW_E2E_SINGLE_FILE (single path only, kept for back-compat)
    include:
      (() => {
        const raw = process.env.CREW_E2E_INCLUDE ?? process.env.CREW_E2E_SINGLE_FILE
        if (raw)
          return raw
            .split(',')
            .map((s) => s.trim())
      ...
      (process.env.CREW_E2E_MODE === 'local'
