...
const enterpriseServerDir = process.env.CREW_ENTERPRISE_SERVER
  ? path.resolve(process.env.CREW_ENTERPRISE_SERVER)
  : path.resolve(currentDir, '../../../crew-enterprise/server')
...
const stripSchemaNamePrefix = (schemaName: string) => {
  return schemaName
    .replace(/^crew\.enterprise\.api\.enterprise\./, '')
    .replace(/^crew\.enterprise\.api\.appdeploy\.v1\./, '')
    .replace(/^crew\.enterprise\.api\.appdeploy\./, '')
    .replace(/^pagination\./, '')
}
