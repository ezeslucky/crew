...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'CheckboxGroup primitive built on Base UI. It owns multi-checkbox array state, allValues, and parent checkbox semantics. Import from `@crew/crew-ui/checkbox-group` and compose with `Checkbox` from `@crew/crew-ui/checkbox`.',
      },
    },
  },
  tags: ['autodocs'],
...
export const DocumentSelection: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Matches Crew table/list selection patterns such as documents, segments, annotations, and install bundle items: CheckboxGroup owns the selected ID array, allValues defines the current selectable page, and the parent checkbox provides select-all plus mixed state.',
      },
    },
  },
}
  ...
  return (
    <Field name="allowed_file_types" className="flex w-80 flex-col gap-2">
      <FieldDescription className="body-xs-regular text-text-tertiary">
        This mirrors Crew dynamic form fields where checkbox options are controlled by schema and
...
export const DynamicFormField: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Matches Crew checkbox-list form usage in workflow node forms and base form rendering. Field and Fieldset provide group labeling; CheckboxGroup owns controlled array state.',
      },
    },
  },
}
