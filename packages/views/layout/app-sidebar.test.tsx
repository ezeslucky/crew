...
import { ApiError } from "@crew/core/api";
...
vi.mock("@crew/ui/components/ui/sidebar", () => ({
...
vi.mock("@crew/ui/components/ui/collapsible", () => ({
...
vi.mock("@crew/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@crew/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@crew/core/inbox/queries", () => ({
...
vi.mock("@crew/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@crew/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@crew/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@crew/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@crew/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@crew/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@crew/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@crew/core/workspace/queries", () => ({
