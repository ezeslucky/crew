...
vi.mock("@crew/core/auth", async () => {
  const actual =
    await vi.importActual<typeof import("@crew/core/auth")>(
      "@crew/core/auth",
    );
...
vi.mock("@crew/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@crew/core/onboarding")>(
      "@crew/core/onboarding",
    );
...
function wipeDismissCounters() {
  for (let i = window.localStorage.length - 1; i >= 0; i--) {
    ...
    if (k && k.startsWith("crew.source_backfill.dismiss.")) {
...
describe("SourceBackfillModal", () => {
  it("does not render when there is no user", () => {
    renderModal();
    expect(
      screen.queryByText(/How did you hear about crew/i),
  ...
  it("opens for an onboarded user with empty source", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about crew/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("crew.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
