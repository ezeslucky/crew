...
// self-hosted instances instead of the hardcoded `crew.ai`.
describe("StepWorkspace — workspace URL prefix", () => {
  it("shows the brand host when no app URL is configured", () => {
    renderStep({ existing: null, disabled: false });
    expect(screen.getByText("crew.ai/")).toBeInTheDocument();
  });
  ...
  it("shows the deployment host for self-hosted instances", () => {
    renderStep({
      existing: null,
      disabled: false,
      daemonAppUrl: "https://crew.example.com",
    });
    expect(screen.getByText("crew.example.com/")).toBeInTheDocument();
    expect(screen.queryByText("crew.ai/")).not.toBeInTheDocument();
  });
});
