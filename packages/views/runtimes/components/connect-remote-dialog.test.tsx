...
import { configStore } from "@crew/core/config";
...
describe("ConnectRemoteDialog", () => {
  it("uses cloud setup commands by default", () => {
    ...
    expect(baseElement).toHaveTextContent("crew setup");
    expect(baseElement).not.toHaveTextContent("crew setup self-host");
    expect(baseElement).toHaveTextContent(
      "crew config set server_url https://api.crew.ai",
    );
    expect(baseElement).toHaveTextContent(
      "crew config set app_url https://crew.ai",
    );
  });
  ...
  it("uses self-host daemon URLs from runtime config", () => {
    ...
    expect(baseElement).toHaveTextContent(
      "crew setup self-host --server-url https://api.example.com --app-url https://app.example.com",
    );
    expect(baseElement).toHaveTextContent(
      "crew config set server_url https://api.example.com",
    );
    expect(baseElement).toHaveTextContent(
      "crew config set app_url https://app.example.com",
    );
  });
  ...
  it("disables font ligatures in setup command code", () => {
    ...
    const setupCode = Array.from(baseElement.querySelectorAll("code")).find((node) =>
      node.textContent?.includes("crew setup"),
    );
  ...
  it("disables font ligatures in fallback token command code", () => {
    ...
    const tokenCode = Array.from(baseElement.querySelectorAll("code")).find((node) =>
      node.textContent?.includes("crew login --token <YOUR_TOKEN>"),
    );
