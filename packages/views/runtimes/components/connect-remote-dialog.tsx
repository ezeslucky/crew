...
import { runtimeKeys } from "@crew/core/runtimes/queries";
import { useWSEvent } from "@crew/core/realtime";
import { paths, useWorkspaceSlug } from "@crew/core/paths";
...
import { CODE_LIGATURE_CLASS } from "@crew/ui/lib/code-style";
...
const CLOUD_SERVER_URL = "https://api.crew.ai";
const CLOUD_APP_URL = "https://crew.ai";
...
function daemonCommands(serverUrl: string | undefined, appUrl: string | undefined) {
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `crew setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}`,
      tokenCmd: `crew config set server_url ${normalizedServerUrl}
crew config set app_url ${normalizedAppUrl}
crew login --token <YOUR_TOKEN>
crew daemon start`,
    };
  }
  ...
  return {
    setupCmd: "crew setup",
    tokenCmd: `crew config set server_url ${CLOUD_SERVER_URL}
crew config set app_url ${CLOUD_APP_URL}
crew login --token <YOUR_TOKEN>
crew daemon start`,
  };
}
...
export function ConnectRemoteDialog({ onClose }: { onClose: () => void }) {
  ...
  // `crew setup` is one blocking command that handles config + login
