...
import { Card, CardContent } from "@crew/ui/components/ui/card";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { larkInstallationsOptions, larkKeys } from "@crew/core/lark";
import { api, ApiError } from "@crew/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@crew/core/types";
...
// install path is per-agent (each Crew Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Crew Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Crew agent's
