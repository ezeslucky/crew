...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/ezeslucky/crew.git"
$RepoWebUrl    = "https://github.com/ezeslucky/crew"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".crew\server"
$InstallDir    = if ($env:CREW_INSTALL_DIR) { $env:CREW_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/ezeslucky/crew/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:CREW_SELFHOST_REF) {
        return $env:CREW_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = crew version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Crew CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Crew requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/ezeslucky/crew/releases/download/$latest/crew-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "crew-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "crew.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/ezeslucky/crew/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "crew.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "crew-cli-$version-windows-$arch.zip"
        $legacyAsset = "crew_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "crew.zip") -DestinationPath $tmpDir -Force
    ...
    $exeSrc = Join-Path $tmpDir "crew.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "crew.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "crew.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "crew.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Crew CLI installed to $binDir\crew.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "crew") {
        ...
        if ($isUpToDate) {
            Write-Ok "Crew CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Crew CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Crew CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "crew")) {
        Write-Fail "CLI installed but 'crew' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Crew self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:CREW_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Crew server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Crew repository..."
    ...
    Write-Info "Pulling official Crew images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Crew services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Crew server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Crew - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Crew CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     crew setup               " -NoNewline; Write-Host "# Connect to Crew Cloud (crew.ai)" -ForegroundColor DarkGray
    Write-Host "     crew setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:CREW_MODE="with-server"; irm https://raw.githubusercontent.com/ezeslucky/crew/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Crew - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Crew server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     crew setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:CREW_MODE="stop"; irm https://raw.githubusercontent.com/ezeslucky/crew/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Crew services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Crew installation found at $InstallDir"
    }

    if (Test-CommandExists "crew") {
        try {
            crew daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:CREW_MODE) { $env:CREW_MODE.ToLower() } else { "default" }
