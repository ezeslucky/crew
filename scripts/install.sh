#!/usr/bin/env bash
# crew installer — installs the CLI and optionally provisions a self-host server.
#
# Install / upgrade CLI only:
#   curl -fsSL https://raw.githubusercontent.com/ezeslucky/crew/main/scripts/install.sh | bash
#
# Install CLI + provision self-host server:
#   curl -fsSL https://raw.githubusercontent.com/ezeslucky/crew/main/scripts/install.sh | bash -s -- --with-server
#
# After installation, run `crew setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/ezeslucky/crew.git"
REPO_WEB_URL="https://github.com/ezeslucky/crew"  # without .git, for GitHub web APIs
INSTALL_DIR="${CREW_INSTALL_DIR:-$HOME/.crew/server}"
BREW_PACKAGE="crew-ai/tap/crew"
...
print_remote_server_token_hint() {
  ...
  printf "     1. On your local computer, open ${CYAN}https://crew.ai/settings?tab=tokens${RESET}\n"
  printf "        and create a token under ${BOLD}Settings > API Tokens${RESET}.\n"
  printf "     2. On this server, run:\n"
  printf "        ${CYAN}crew login --token <YOUR_TOKEN>${RESET}\n"
  printf "        ${CYAN}crew daemon start${RESET}\n"
  printf "\n"
}
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/ezeslucky/crew/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). Crew supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing Crew CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap crew-ai/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "Crew CLI already installed via Homebrew"
    else
      warn "Failed to install crew via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "Crew CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Crew CLI from GitHub Releases..."
  ...
  local url="https://github.com/ezeslucky/crew/releases/download/${latest}/crew-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/crew.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/crew.tar.gz" -C "$tmp_dir" crew

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with CREW_BIN_DIR.
  local bin_dir="${CREW_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/crew" "$bin_dir/crew"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/crew" "$bin_dir/crew"
  else
  ...
  rm -rf "$tmp_dir"
  ok "Crew CLI installed to $bin_dir/crew"
}
...
get_selfhost_ref() {
  if [ -n "${CREW_SELFHOST_REF:-}" ]; then
    printf '%s' "$CREW_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Crew CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Crew CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Crew CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists crew; then
    local current_ver
    # `crew version` outputs "crew 0.3.23 (commit: f46b929eb, built: 2026-06-16T10:11:56Z)" — extract just the version
    current_ver=$(crew version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Crew CLI is up to date ($current_ver)"
      return 0
    fi

    info "Crew CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(crew version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ok "Crew CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists crew; then
    fail "CLI installed but 'crew' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Crew self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  info "Pulling official Crew images..."
  pull_official_selfhost_images
  info "Starting Crew services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  Crew — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Crew CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}crew setup${RESET}                # Connect to Crew Cloud (crew.ai)\n"
  printf "     ${CYAN}crew setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  print_remote_server_token_hint
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/ezeslucky/crew/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Crew — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Crew server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}crew setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/ezeslucky/crew/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Crew installation found at $INSTALL_DIR"
  fi

  if command_exists crew; then
    crew daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Crew CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  CREW_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.crew/server)"
        echo "  CREW_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  CREW_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'crew setup' to configure your environment."
        exit 0
        ;;
