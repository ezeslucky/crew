...
require_config "$config" 'CREW_APP_URL: http://localhost:3100'
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "CREW_APP_URL=${CREW_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "CREW_SERVER_URL=${CREW_SERVER_URL}" \
...
require_env "$local_env" 'CREW_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'CREW_SERVER_URL=ws://localhost:9100/ws'
