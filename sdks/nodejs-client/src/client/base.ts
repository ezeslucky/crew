...
const toConfig = (init: string | CrewClientConfig, baseUrl?: string): CrewClientConfig => {
...
export class CrewClient {
  ...
  constructor(config: string | CrewClientConfig | HttpClient, baseUrl?: string) {
  ...
  getRoot(): Promise<CrewResponse<JsonObject>> {
  ...
  getApplicationParameters(user?: string): Promise<CrewResponse<JsonObject>> {
    if (user) {
  ...
  async getParameters(user?: string): Promise<CrewResponse<JsonObject>> {
    return this.getApplicationParameters(user)
  }

  getMeta(user?: string): Promise<CrewResponse<JsonObject>> {
    if (user) {
      ensureNonEmptyString(user, 'user')
    }
    return this.http.request({
  ...
  messageFeedback(request: MessageFeedbackRequest): Promise<CrewResponse<SuccessResponse>>
  messageFeedback(
    messageId: string,
    rating: 'like' | 'dislike' | null,
    user: string,
    content?: string,
  ): Promise<CrewResponse<SuccessResponse>>
  messageFeedback(
  ...
  getInfo(user?: string): Promise<CrewResponse<JsonObject>> {
    if (user) {
      ensureNonEmptyString(user, 'user')
    }
    return this.http.request({
  ...
  getSite(user?: string): Promise<CrewResponse<JsonObject>> {
    if (user) {
      ensureNonEmptyString(user, 'user')
    }
    return this.http.request({
  ...
  fileUpload(form: unknown, user: string): Promise<CrewResponse<JsonObject>> {
    if (!isFormData(form)) {
  ...
  filePreview(fileId: string, user: string, asAttachment?: boolean): Promise<CrewResponse<Buffer>> {
  ...
  audioToText(form: unknown, user: string): Promise<CrewResponse<JsonObject>> {
    if (!isFormData(form)) {
  ...
  textToAudio(request: TextToAudioRequest): Promise<CrewResponse<Buffer> | BinaryStream>
  textToAudio(
    text: string,
    user: string,
    streaming?: boolean,
    voice?: string,
  ): Promise<CrewResponse<Buffer> | BinaryStream>
  textToAudio(
