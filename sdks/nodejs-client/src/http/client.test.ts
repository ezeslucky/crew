...
describe('HttpClient', () => {
  ...
  it('builds requests with auth headers and JSON content type', async () => {
    ...
    expect(url).toBe('https://api.crew.ai/v1/chat-messages')
  ...
  it('logs requests and responses when enableLogging is true', async () => {
    ...
    expect(consoleInfo).toHaveBeenCalledWith(
      expect.stringContaining('crew-client-node response 200 GET'),
    )
  })
  ...
  it('logs retry attempts when enableLogging is true', async () => {
    ...
    expect(consoleInfo).toHaveBeenCalledWith(expect.stringContaining('crew-client-node retry'))
  })
})
