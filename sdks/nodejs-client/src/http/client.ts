...
const DEFAULT_USER_AGENT = 'crew-client-node'
...
export type HttpClientSettings = Required<Omit<CrewClientConfig, 'apiKey'>> & {
  apiKey: string
}
...
const normalizeSettings = (config: CrewClientConfig): HttpClientSettings => ({
...
const mapHttpError = (response: RawHttpResponse, path: string): CrewError => {
...
const mapTransportError = (error: unknown, timeoutContext: TimeoutContext): CrewError => {
...
export class HttpClient {
  ...
  constructor(config: CrewClientConfig) {
  ...
  async requestStream<T>(options: RequestOptions): Promise<CrewStream<T>> {
  ...
  async requestRaw<TResponseType extends HttpResponseType = 'json'>(
    ...
    if (enableLogging) {
      console.info(`crew-client-node request ${method} ${url}`)
    }
    ...
    while (true) {
      ...
      try {
        ...
        if (enableLogging) {
          console.info(`crew-client-node response ${response.status} ${method} ${url}`)
        }
        ...
        if (enableLogging) {
          console.info(`crew-client-node retry ${attempt + 1} in ${delay}ms for ${method} ${url}`)
        }
        attempt += 1
        await sleep(delay)
      } finally {
