...
import { BASE_URL, ChatClient, CrewClient, WorkflowClient, routes } from './index'
...
describe('Client', () => {
  ...
  it('creates a client with default settings', () => {
    const crewClient = new CrewClient('test')

    expect(crewClient.getHttpClient().getSettings()).toMatchObject({
      apiKey: 'test',
      baseUrl: BASE_URL,
      timeout: 60,
    })
  ...
  it('updates the api key', () => {
    const crewClient = new CrewClient('test')
    crewClient.updateApiKey('test2')

    expect(crewClient.getHttpClient().getSettings().apiKey).toBe('test2')
  })
})
...
describe('Send Requests', () => {
  ...
  it('makes a successful request to the application parameter route', async () => {
    ...
    const response = await crewClient.sendRequest(method, endpoint)
  ...
  it('uses the getMeta route configuration', async () => {
    ...
    await crewClient.getMeta('end-user')
...
describe('File uploads', () => {
  ...
  it('does not override multipart boundary headers for legacy FormData', async () => {
    ...
    await crewClient.fileUpload(form, 'end-user')
