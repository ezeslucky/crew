...
vi.mock('@/app/components/base/logo/crew-logo', () => ({
  __esModule: true,
  default: () => <div>crew-logo</div>,
}))
...
describe('Human input share form', () => {
  ...
  it('should render status cards for terminal fetch states', () => {
    ...
    cases.forEach(({ error, title, subtitle, submissionID }) => {
      ...
      expect(screen.getByText('crew-logo')).toBeInTheDocument()
      unmount()
    })
  })
  ...
  it('should hide branding when remove_webapp_brand is enabled', () => {
    ...
    expect(screen.queryByText('crew-logo')).not.toBeInTheDocument()
  })
