...
describe('ExternalAttributionRecorder', () => {
  ...
  it('seeds the utm_info cookie and create_app attribution from the landing url', async () => {
    setSearchParams('utm_source=crew_blog&slug=get-started-with-crew')

    render(<ExternalAttributionRecorder />)

    await waitFor(() => {
      expect(getUtmInfoCookie()).toEqual({
        utm_source: 'crew_blog',
        slug: 'get-started-with-crew',
      })
    })
    ...
    expect(firstArg?.searchParams?.get('slug')).toBe('get-started-with-crew')
  })

  it('seeds attribution from the redirect_url when auth redirects away from the landing url', async () => {
    setSearchParams(
      `redirect_url=${encodeURIComponent('/apps?utm_source=crew_blog&slug=buildaisupportassistantwithcrew')}`,
    )
    ...
    await waitFor(() => {
      expect(getUtmInfoCookie()).toEqual({
        utm_source: 'crew_blog',
        slug: 'buildaisupportassistantwithcrew',
      })
    })
    expect(mockRememberCreateAppExternalAttribution).toHaveBeenCalledTimes(1)
    const firstArg = mockRememberCreateAppExternalAttribution.mock.calls[0]?.[0]
    expect(firstArg?.searchParams?.get('utm_source')).toBe('crew_blog')
    expect(firstArg?.searchParams?.get('slug')).toBe('buildaisupportassistantwithcrew')
  })

  it('does nothing without a utm_source', () => {
    setSearchParams('slug=get-started-with-crew')
  ...
  it('does nothing for cross-origin redirect_url attribution params', () => {
    setSearchParams(
      `redirect_url=${encodeURIComponent('https://example.com/apps?utm_source=crew_blog&slug=get-started-with-crew')}`,
    )
