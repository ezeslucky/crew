import type { GetSystemFeaturesResponse } from '@crew/contracts/api/console/system-features/types.gen'
...
const mockLearnCrewApp = vi.hoisted(
  () =>
    ({
      app_id: 'learn-crew-template',
      app: {
        id: 'learn-crew-template-source',
        mode: 'chat',
        icon_type: 'emoji',
        icon: '🤖',
        icon_background: '#fff',
        icon_url: '',
        name: 'Learn Crew Template',
        description: 'Learn how to build with Crew',
...
vi.mock('@/app/components/explore/learn-crew', () => ({
  default: ({
    title,
    onCreate,
    onTry,
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        {
          type: 'button',
          onClick: () => onTry?.({ appId: mockLearnCrewApp.app_id, app: mockLearnCrewApp }),
        },
        'Preview Learn Crew template',
      ),
      React.createElement(
        'button',
        { type: 'button', onClick: () => onCreate?.(mockLearnCrewApp) },
        'Create Learn Crew template',
      ),
    ),
}))
...
type RenderListOptions = {
  onCreateLearnCrew?: (app: App) => void
  onTryLearnCrew?: (params: TryAppSelection) => void
  ...
  const rendered = renderWithNuqs(
    <ConsoleQueryWrapper>
      <JotaiProvider store={store}>
        <List
          onCreateLearnCrew={options.onCreateLearnCrew}
          onTryLearnCrew={options.onTryLearnCrew}
        />
      </JotaiProvider>
    </ConsoleQueryWrapper>,
    { searchParams },
...
describe('List', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render first empty state when there are no apps and no active filters', () => {
      ...
      expect(screen.getByText('app.firstEmpty.learnCrewTitle'))!.toBeInTheDocument()
      ...
      expect(
        screen
          .getByText('app.firstEmpty.learnCrewTitle')
          .closest('[data-step-by-step-tour-target]'),
      ).toHaveAttribute(
        'data-step-by-step-tour-target',
        STEP_BY_STEP_TOUR_TARGETS.studioEmptyLearnCrew,
      )
    })
    ...
    it('should hide learn crew in first empty state when learn app is disabled', () => {
      mockAppData = { pages: [{ data: [], total: 0 }] }

      renderList('', { systemFeatures: { enable_learn_app: false } })

      expect(screen.getByText('app.firstEmpty.title'))!.toBeInTheDocument()
      expect(screen.queryByText('app.firstEmpty.learnCrewTitle')).not.toBeInTheDocument()
    })
    ...
    it('should forward Learn Crew template interactions', async () => {
      const user = userEvent.setup()
      const onCreateLearnCrew = vi.fn()
      const onTryLearnCrew = vi.fn()
      mockAppData = { pages: [{ data: [], total: 0 }] }
      ...
      await user.click(screen.getByRole('button', { name: 'Preview Learn Crew template' }))
      expect(onTryLearnCrew).toHaveBeenCalledWith({
        appId: mockLearnCrewApp.app_id,
        app: mockLearnCrewApp,
      })

      await user.click(screen.getByRole('button', { name: 'Create Learn Crew template' }))
      expect(onCreateLearnCrew).toHaveBeenCalledWith(mockLearnCrewApp)
    })
