...
import LearnCrew from '@/app/components/explore/learn-crew'
...
function FirstEmptyState({
  ...
  return (
    <div className="flex grow flex-col overflow-hidden">
      ...
      {showLearnCrew && (
        <div data-step-by-step-tour-target={STEP_BY_STEP_TOUR_TARGETS.studioEmptyLearnCrew}>
          <LearnCrew
            canCreate
            className="px-4 pt-2 pb-0 [&_div.grid]:gap-3 [&>div]:mx-0 [&>div]:rounded-t-2xl [&>div]:rounded-b-none [&>div]:px-5 [&>div]:pt-4 [&>div]:pb-5"
            dismissible={false}
            itemLimit={4}
            onCreate={onCreateLearnCrew}
            onTry={onTryLearnCrew}
            showDescription
            title={t(($) => $['firstEmpty.learnCrewTitle'], { ns: 'app' })}
          />
        </div>
      )}
    </div>
