...
const Header: FC<IHeaderProps> = ({
  ...
  const handleMessageReceived = useCallback(
    (event: MessageEvent) => {
      ...
      if (!currentParentOrigin && event.data.type === 'crew-chatbot-config') {
        currentParentOrigin = event.origin
        setParentOrigin(event.origin)
      }
      if (event.origin !== currentParentOrigin) return
      if (event.data.type === 'crew-chatbot-config')
  ...
  useEffect(() => {
    ...
    window.parent.postMessage({ type: 'crew-chatbot-iframe-ready' }, targetOrigin)
