...
import { Checkbox } from '@crew/crew-ui/checkbox'
import { CheckboxGroup } from '@crew/crew-ui/checkbox-group'
import { cn } from '@crew/crew-ui/cn'
import { Field, FieldDescription, FieldItem, FieldLabel } from '@crew/crew-ui/field'
import { Fieldset, FieldsetLegend } from '@crew/crew-ui/fieldset'
...
export const CheckboxList = ({
  ...
  return (
    <Field name={name} className={cn('flex w-full flex-col gap-1', containerClassName)}>
      ...
      >
        {label && <FieldsetLegend className="mb-0">{label}</FieldsetLegend>}
        {description && (
            ...
            <div className="relative flex items-center gap-2 border-b border-divider-subtle px-3 py-2">
              {!searchQuery && showSelectAll && (
                <FieldItem disabled={disabled} className="shrink-0 gap-0">
                  <FieldLabel
                    className={cn('flex items-center p-0', !disabled && 'cursor-pointer')}
                  >
                    <Checkbox parent disabled={disabled} />
            ...
            {!filteredOptions.length ? (
              ...
              filteredOptions.map((option) => (
                ...
                >
                  ...
                  >
                    <Checkbox value={option.value} disabled={option.disabled || disabled} />
                    <span
