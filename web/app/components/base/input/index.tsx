...
/**
 * @deprecated Use `@crew/crew-ui/input` for primitive inputs and
 * `@crew/crew-ui/field` for form composition. Search inputs should use
    ...
    return (
      <div className={cn('relative w-full', wrapperClassName)}>
        {showLeftIcon && (
          <span
            className={cn(
              'absolute top-1/2 left-2 i-ri-search-line size-4 -translate-y-1/2 text-components-input-text-placeholder',
            )}
          />
        )}
        <input
        ...
        {!!(showClearIcon && value && !disabled && !destructive) && (
          <button
            ...
            className={cn(
              'group absolute top-1/2 right-2 -translate-y-1/2 cursor-pointer border-none bg-transparent p-px',
            )}
            onClick={onClear}
          >
            <span
