...
const ContextBlockComponent: FC<ContextBlockComponentProps> = ({
  ...
  return (
    <div
      className={`group inline-flex h-6 items-center rounded-[5px] border border-transparent bg-[#F4F3FF] pr-0.5 pl-1 text-[#6938EF] hover:bg-[#EBE9FE] ${open ? 'bg-[#EBE9FE]' : ''} ${isSelected && 'border-[#9B8AFB]!'} `}
      ref={ref}
    >
      <span
          ...
          <PopoverTrigger
            render={
              <button
                ...
                className={`flex h-[18px] w-[18px] cursor-pointer items-center justify-center rounded border-none p-0 text-[11px] font-semibold ${open ? 'bg-[#6938EF] text-white' : 'bg-white/50 group-hover:bg-white group-hover:shadow-xs'} `}
                ref={triggerRef}
