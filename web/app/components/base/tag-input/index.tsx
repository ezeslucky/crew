...
const TagInput = ({
  ...
  const inputPlaceholder =
    placeholder ||
    (isSpecialMode
      ? t(($) => $['model.params.stop_sequencesPlaceholder'], { ns: 'common' })
      : t(($) => $['segment.addKeyWord'], { ns: 'datasetDocuments' }))
  ...
  const handleNewTag = useCallback(
    (value: string) => {
      ...
      if (!valueTrimmed) {
        if (required) toast.error(t(($) => $['segment.keywordEmpty'], { ns: 'datasetDocuments' }))
        return
      }
    ...
    >
      {(items || []).map((item, index) => (
        <div
          key={item}
          className={cn(
            'mt-1 mr-1 flex items-center rounded-md border border-divider-deep bg-components-badge-white-to-dark py-1 pr-1 pl-1.5 system-xs-regular text-text-secondary',
          )}
        >
          {item}
          {!disableRemove && (
            ...
            >
              <span
                className="ml-0.5 i-ri-close-line size-3.5 text-text-tertiary"
                aria-hidden="true"
              />
            </button>
          )}
      ...
      {!disableAdd && (
        <div
          className={cn(
            'group/tag-add mt-1 flex items-center gap-x-0.5',
            !isSpecialMode ? 'rounded-md border border-dashed border-divider-deep px-1.5' : '',
          )}
        >
          {!isSpecialMode && !focused && (
            <span className="i-ri-add-line size-3.5 text-text-placeholder group-hover/tag-add:text-text-secondary" />
          )}
          <span
