...
export const useDocumentCreation = (options: UseDocumentCreationOptions) => {
  ...
  const validateParams = useCallback(
    (params: ValidationParams): boolean => {
      const {
        segmentationType,
        maxChunkLength,
        limitMaxChunkLength,
      ...
      if (!isSetting) {
        if (
          ...
          (!embeddingModel.model || !embeddingModel.provider)
        ) {
  ...
  const executeCreation = useCallback(
    async (
      ...
      if (datasetId && !isSetting && !canCreateDocument) return
  ...
  const validatePreviewParams = useCallback(
    (maxChunkLength: number): boolean => {
