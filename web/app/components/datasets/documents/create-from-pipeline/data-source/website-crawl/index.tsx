...
const WebsiteCrawl = ({
  ...
  const usePreProcessingParams = useRef(
    !isInPipeline ? usePublishedPipelinePreProcessingParams : useDraftPipelinePreProcessingParams,
  )
  ...
  const handleCheckedCrawlResultChange = useCallback(
  ...
  const handlePreview = useCallback(
    (website: CrawlResultItem, index: number) => {
      ...
      ssePost(
        ...
        },
        {
          ...
          onDataSourceNodeError: (error: DataSourceNodeErrorResponse) => {
            setCrawlErrorMessage(
              error.error || t(($) => $[`${I18N_PREFIX}.unknownError`], { ns: 'datasetCreation' }),
            )
            setStep(CrawlStep.finished)
          },
        },
  ...
  return (
    <div className="flex flex-col">
      <Header
        docTitle="Docs"
        docLink={docLink('/use-crew/knowledge/knowledge-pipeline/authorize-data-source')}
      ...
      {!isInit && (
        <div className="relative flex flex-col">
          {isRunning && <Crawling crawledNum={crawledNum} totalNum={totalNum} />}
          {showError && (
            <ErrorMessage
              className="mt-2"
