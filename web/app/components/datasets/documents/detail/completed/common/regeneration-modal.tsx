    ...
    <>
      <div className="pb-4">
        <AlertDialogTitle className="title-2xl-semi-bold text-text-primary">
          {t(($) => $['segment.regenerationConfirmTitle'], { ns: 'datasetDocuments' })}
        </AlertDialogTitle>
        <AlertDialogDescription className="system-md-regular text-text-secondary">
          {t(($) => $['segment.regenerationConfirmMessage'], { ns: 'datasetDocuments' })}
        </AlertDialogDescription>
      </div>
  ...
  ({ onClose }) => {
    ...
    return (
      <>
        <div className="pb-4">
          <span className="title-2xl-semi-bold text-text-primary">
            {t(($) => $['segment.regenerationSuccessTitle'], { ns: 'datasetDocuments' })}
          </span>
          <p className="system-md-regular text-text-secondary">
            {t(($) => $['segment.regenerationSuccessMessage'], { ns: 'datasetDocuments' })}
          </p>
        </div>
        <div className="flex justify-end pt-6">
...
const RegenerationModal: FC<IRegenerationModalProps> = ({
  ...
  eventEmitter?.useSubscription((v) => {
    ...
    if (v === 'update-segment-success') setUpdateSucceeded(true)
