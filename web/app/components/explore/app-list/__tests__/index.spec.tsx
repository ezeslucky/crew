...
import { LEARN_CREW_HIDDEN_STORAGE_KEY } from '../../learn-crew/storage'
...
describe('AppList', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should not render learn crew content while learn crew items are loading', () => {
      mockExploreData = {
        categories: ['Writing'],
        allList: [createApp()],
      }
    ...
    it('should not show the learn crew placeholder when the section is hidden', () => {
      mockExploreData = {
        categories: ['Writing'],
        allList: [createApp()],
      }
    ...
    it('should render learn crew templates without badges or template metadata', () => {
      mockExploreData = {
        categories: ['Writing'],
        allList: [createApp()],
      }
      ...
      expect(
        screen.queryByRole('link', { name: 'explore.learnCrew.moreTemplates' }),
      ).not.toBeInTheDocument()
    ...
    it('should hide learn crew templates when learn app is disabled', () => {
      mockExploreData = {
        categories: ['Writing'],
        allList: [createApp()],
      }
    ...
    it('should collapse learn crew and persist hidden state when hide is clicked', async () => {
      mockExploreData = {
        categories: ['Writing'],
        allList: [createApp()],
      }
      ...
      fireEvent.click(screen.getByRole('button', { name: 'explore.learnCrew.hide' }))

      const learnCrewSection = screen
        .getByRole('heading', { name: 'explore.learnCrew.title' })
        .closest('section')
      expect(learnCrewSection).toHaveClass('z-50', 'opacity-20')
      expect(learnCrewSection).toHaveStyle({ transform: 'scale(0.08)' })
      ...
      expect(localStorage.getItem(LEARN_CREW_HIDDEN_STORAGE_KEY)).toBe('true')
    })
  })
  ...
  describe('User Interactions', () => {
    ...
    it('should open create flow from learn crew item card click', async () => {
      vi.useRealTimers()
    ...
    it('should advance the Learn Crew tour to the create button after a lesson opens', async () => {
      vi.useRealTimers()
      const user = userEvent.setup()
      mockExploreData = {
        categories: ['Writing'],
    ...
    it('should close the Learn Crew detail when the home action guide is skipped', async () => {
      vi.useRealTimers()
      const user = userEvent.setup()
      mockExploreData = {
        categories: ['Writing'],
    ...
    it('should complete the Learn Crew tour when a no-create user opens a lesson detail', async () => {
      vi.useRealTimers()
      const user = userEvent.setup()
      mockExploreData = {
        categories: ['Writing'],
    ...
    it('should complete the Learn Crew tour only after the app is created from details', async () => {
      vi.useRealTimers()
      const user = userEvent.setup()
      mockExploreData = {
        categories: ['Writing'],
    ...
    it('should clear the Learn Crew session and provenance when completion persistence fails', async () => {
      vi.useRealTimers()
      const user = userEvent.setup()
      mockExploreData = {
        categories: ['Writing'],
    ...
    it('should skip redirect after confirming a pending Learn Crew tour create', async () => {
      vi.useRealTimers()
      const user = userEvent.setup()
      mockExploreData = {
        categories: ['Writing'],
    ...
    it('should hide the Learn Crew tour target while the create modal is open and abandon on cancel', async () => {
      vi.useRealTimers()
      const user = userEvent.setup()
      mockExploreData = {
        categories: ['Writing'],
