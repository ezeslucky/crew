...
const Apps = ({ onSuccess }: { onSuccess?: () => void }) => {
  ...
  const currentCreateAppTrackingRef = useRef<Pick<
    ...
    'source' | 'templateId'
  > | null>(null)
  const isCurrentTryAppFromLearnCrewRef = useRef(false)
  ...
  const shouldForceShowLearnCrewForTour =
  ...
  const completeHomeTourAfterCreate = useCallback(() => {
    ...
    resetStepByStepTourSession()
    isCurrentTryAppFromLearnCrewRef.current = false
  ...
  const abandonHomeTourCreate = useCallback(() => {
    if (!isCurrentTryAppFromLearnCrewRef.current || isSubmittingHomeTourCreateRef.current) return
  ...
  const hideTryAppPanel = useCallback(() => {
    ...
    // oxlint-disable-next-line eslint-react/set-state-in-effect -- Also called from the tour-state sync effect when the Learn Crew action guide is skipped.
    setCurrentTryApp(undefined)
  }, [abandonHomeTourCreate])
  const homeTryAppCreateGuideActive =
    ...
    !completedStepByStepTourTaskIds.includes(HOME_STEP_BY_STEP_TOUR_TASK_ID)
  useEffect(() => {
    if (!isCurrentTryAppFromLearnCrewRef.current || !currentTryApp || isShowCreateModal) {
  ...
  const handleTryAppFromLearnCrew = useCallback(
    (params: TryAppSelection) => {
      isCurrentTryAppFromLearnCrewRef.current = true
      setCurrentTryApp(params)
  ...
  const handleShowFromTryApp = useCallback(() => {
    ...
    shouldCompleteHomeTourOnCreateRef.current =
      isCurrentTryAppFromLearnCrewRef.current &&
      activeStepByStepTourTaskId === HOME_STEP_BY_STEP_TOUR_TASK_ID &&
      !completedStepByStepTourTaskIds.includes(HOME_STEP_BY_STEP_TOUR_TASK_ID) &&
      activeStepByStepTourGuideIndex === 1
  ...
  const handleCreateFromLearnCrew = useCallback((app: App) => {
    setCurrApp(app)
    setIsShowCreateModal(true)
  }, [])
        ...
        {homeQueries.isPending ? (
          ...
          <>
            ...
            <ExploreRecommendations
              ...
              forceShowLearnCrew={shouldForceShowLearnCrewForTour}
              onCreate={handleCreateFromLearnCrew}
              onTry={handleTryAppFromLearnCrew}
            />
      ...
      {isShowTryAppPanel && (
        <TryApp
          ...
          createButtonStepByStepTourTarget={
            canCreateApp && isCurrentTryAppFromLearnCrewRef.current && !isShowCreateModal
