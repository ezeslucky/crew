...
describe('LearnCrewItem', () => {
  ...
  it('should not render hover action buttons', () => {
    render(<LearnCrewItem canCreate item={createApp()} onCreate={vi.fn()} onTry={vi.fn()} />)
  ...
  it('should create app when card is clicked outside cloud edition', () => {
    ...
    render(<LearnCrewItem canCreate item={app} onCreate={onCreate} onTry={vi.fn()} />)

    fireEvent.click(screen.getByRole('button', { name: 'Learn Crew App' }))
  ...
  it('should not make the card clickable outside cloud edition when create is unavailable', () => {
    ...
    render(<LearnCrewItem canCreate={false} item={createApp()} onTry={vi.fn()} />)

    expect(screen.queryByRole('button', { name: 'Learn Crew App' })).not.toBeInTheDocument()
  })
  ...
  it('should open detail when card is clicked in cloud edition', () => {
    ...
    render(<LearnCrewItem canCreate={false} item={app} onTry={onTry} />)
  ...
  it('should run the card action when Enter is pressed', () => {
    ...
    fireEvent.keyDown(screen.getByRole('button', { name: 'Learn Crew App' }), { key: 'Enter' })
