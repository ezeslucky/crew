...
describe('InvitationLink', () => {
  ...
  it('should copy absolute url as is', async () => {
    const user = userEvent.setup()
    const absoluteValue = { ...value, url: 'https://crew.ai/invite/123' }

    render(<InvitationLink value={absoluteValue} />)

    await user.click(screen.getByRole('button', { name: 'https://crew.ai/invite/123' }))

    expect(copy).toHaveBeenCalledWith('https://crew.ai/invite/123')
  })
})
