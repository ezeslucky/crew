...
import { Avatar } from '@crew/crew-ui/avatar'
...
const MemberDetailsModal = ({
  ...
  const { data: rolesOfMember, isLoading: isLoadingRolesOfMember } = useRolesOfMember(
    member.id,
    language,
  )
  ...
  const builtinRoles = useMemo(
    () => selectedRoles.filter((role) => role.is_builtin),
    [selectedRoles],
  )
  ...
  const handleRemove = useCallback(
    (roleId: string) => {
      if (!allowMultipleRoles && selectedRoles.length <= 1) return
