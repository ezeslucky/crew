...
import { Dialog, DialogContent, DialogTitle } from '@crew/crew-ui/dialog'
...
import { LEARN_CREW_HIDDEN_STORAGE_KEY } from '@/app/components/explore/learn-crew/storage'
...
const activeGradientMaskClassName = 'aria-[current=page]:crew-blue-glass-surface'
...
vi.mock('@crew/crew-ui/toast', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@crew/crew-ui/toast')>()
  return {
    ...actual,
    toast: {
      ...actual.toast,
...
describe('MainNav', () => {
  ...
  it('aligns the global navigation spacing with the main sidebar design', async () => {
    ...
    const logoLink = screen.getByLabelText('Crew')
  ...
  it('shows Learn Crew switch in help menu and restores it from localStorage', async () => {
    ...
    const learnCrewItem = await screen.findByRole('menuitemcheckbox', {
      name: 'common.mainNav.help.learnCrew',
    })
    expect(learnCrewItem).toHaveAttribute('aria-checked', 'false')

    fireEvent.click(learnCrewItem)

    await waitFor(() => {
      expect(localStorage.getItem(LEARN_CREW_HIDDEN_STORAGE_KEY)).toBe('false')
    })
  ...
  it('hides Learn Crew switch in help menu when learn app is disabled', async () => {
    ...
    await screen.findByText('common.mainNav.help.docs')
    expect(
      screen.queryByRole('menuitemcheckbox', { name: 'common.mainNav.help.learnCrew' }),
    ).not.toBeInTheDocument()
  })
