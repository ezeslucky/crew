...
import { Switch } from '@crew/crew-ui/switch'
import { useSuspenseQuery } from '@tanstack/react-query'
import { useAtomValue, useSetAtom } from 'jotai'
import { useState } from 'react'
import { useTranslation } from 'react-i18next'
import {
  useLearnCrewHiddenValue,
  useSetLearnCrewHidden,
} from '@/app/components/explore/learn-crew/storage'
...
const HelpMenu = ({ triggerIcon = defaultTriggerIcon, triggerClassName }: HelpMenuProps) => {
  ...
  const setLearnCrewHidden = useSetLearnCrewHidden()
  ...
  const shouldShowLearnCrewSwitch = systemFeatures.enable_learn_app
        ...
        >
          <>
            <DropdownMenuGroup className="p-1">
              <DropdownMenuLinkItem
                href={docLink('/use-crew/getting-started/introduction')}
                target="_blank"
              ...
              {shouldShowLearnCrewSwitch && (
                <DropdownMenuCheckboxItem
                  checked={!learnCrewHidden}
                  closeOnClick={false}
                  className="mx-0 h-8 gap-1 px-0 py-1 pr-2 pl-3"
                  onCheckedChange={(checked) => setLearnCrewHidden(!checked)}
                >
                  <span
                    aria-hidden
                  ...
                  <span className="min-w-0 flex-1 truncate px-1 py-0.5 system-md-regular text-text-secondary">
                    {t(($) => $['mainNav.help.learnCrew'], { ns: 'common' })}
                  </span>
                  <MenuSwitchIndicator checked={!learnCrewHidden} />
                </DropdownMenuCheckboxItem>
              )}
            ...
            <DropdownMenuGroup className="p-1">
              <DropdownMenuLinkItem
                href="https://github.com/ezeslucky/crew"
                target="_blank"
                rel="noopener noreferrer"
