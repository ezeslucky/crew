...
const createMockFile = (name: string = 'test-plugin.crewpkg'): File => {
  return new File(['test content'], name, { type: 'application/octet-stream' })
}

const createMockBundleFile = (): File => {
  return new File(['bundle content'], 'test-bundle.crewbndl', { type: 'application/octet-stream' })
}
...
describe('InstallFromLocalPackage', () => {
  ...
  describe('Rendering', () => {
    it('should render modal with uploading step initially', () => {
      ...
      expect(screen.getByTestId('file-name')).toHaveTextContent('test-plugin.crewpkg')
    })
  ...
  // ================================
  describe('Edge Cases', () => {
    it('should handle file with .crewpkg extension as package', () => {
      const pkgFile = createMockFile('my-plugin.crewpkg')
      render(<InstallFromLocalPackage {...defaultProps} file={pkgFile} />)

      expect(screen.getByTestId('is-bundle')).toHaveTextContent('false')
    })

    it('should handle file with .crewbndl extension as bundle', () => {
      const bundleFile = createMockFile('my-bundle.crewbndl')
    ...
    it('should display correct file name in uploading step', () => {
      const customFile = createMockFile('custom-plugin-name.crewpkg')
      render(<InstallFromLocalPackage {...defaultProps} file={customFile} />)

      expect(screen.getByTestId('file-name')).toHaveTextContent('custom-plugin-name.crewpkg')
    })
  ...
  describe('Prop Variations', () => {
    it('should work with different file names', () => {
      const files = [
        createMockFile('plugin-a.crewpkg'),
        createMockFile('plugin-b.crewpkg'),
        createMockFile('bundle-c.crewbndl'),
      ]
    ...
    it('should handle different file types correctly', () => {
      ...
      const { rerender } = render(
        <InstallFromLocalPackage {...defaultProps} file={createMockFile('test.crewpkg')} />,
      )
      expect(screen.getByTestId('is-bundle')).toHaveTextContent('false')
...
describe('Uploading Step', () => {
  ...
  describe('Rendering', () => {
    it('should render uploading state with file name', () => {
      const defaultProps = {
        file: createMockFile('my-custom-plugin.crewpkg'),
        onClose: vi.fn(),
        onSuccess: vi.fn(),
      }
      ...
      expect(screen.getByTestId('uploading-step')).toBeInTheDocument()
      expect(screen.getByTestId('file-name')).toHaveTextContent('my-custom-plugin.crewpkg')
    })
  ...
  describe('File Type Detection', () => {
    it('should detect .crewpkg as package', () => {
      const defaultProps = {
        file: createMockFile('test.crewpkg'),
        onClose: vi.fn(),
        onSuccess: vi.fn(),
      }
    ...
    it('should detect .crewbndl as bundle', () => {
      const defaultProps = {
        file: createMockFile('test.crewbndl'),
        onClose: vi.fn(),
        onSuccess: vi.fn(),
      }
