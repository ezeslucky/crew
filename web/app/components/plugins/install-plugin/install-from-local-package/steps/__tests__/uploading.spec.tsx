...
describe('Uploading', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render card with file name', () => {
      const file = createMockFile('my-plugin.crewpkg')
      render(<Uploading {...defaultProps} file={file} />)

      expect(screen.getByTestId('card-name')).toHaveTextContent('my-plugin.crewpkg')
      expect(screen.getByTestId('card-loading-filename')).toHaveTextContent('my-plugin.crewpkg')
    })
  ...
  describe('File Name Display', () => {
    it('should display correct file name for package file', () => {
      const file = createMockFile('custom-plugin.crewpkg')
      render(<Uploading {...defaultProps} file={file} />)

      expect(screen.getByTestId('card-name')).toHaveTextContent('custom-plugin.crewpkg')
    })

    it('should display correct file name for bundle file', () => {
      const file = createMockFile('custom-bundle.crewbndl')
      render(<Uploading {...defaultProps} file={file} isBundle />)

      expect(screen.getByTestId('card-name')).toHaveTextContent('custom-bundle.crewbndl')
    })

    it('should display file name in uploading message', () => {
      const file = createMockFile('special-plugin.crewpkg')
      render(<Uploading {...defaultProps} file={file} />)
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle file with special characters in name', () => {
      const file = createMockFile('my plugin (v1.0).crewpkg')
      render(<Uploading {...defaultProps} file={file} />)

      expect(screen.getByTestId('card-name')).toHaveTextContent('my plugin (v1.0).crewpkg')
    })
  })
  ...
  describe('Props Variations', () => {
    it('should work with different file types', () => {
      const files = [
        ...
        createMockFile('bundle.crewbndl'),
      ]
