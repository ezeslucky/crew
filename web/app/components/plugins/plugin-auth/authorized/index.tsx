...
import type { Placement } from '@crew/crew-ui/popover'
...
import { Popover, PopoverContent, PopoverTrigger } from '@crew/crew-ui/popover'
...
const Authorized = ({
  ...
  const oAuthCredentials = credentials.filter(
    (credential) => credential.credential_type === CredentialTypeEnum.OAUTH2,
  )
  const apiKeyCredentials = credentials.filter(
    (credential) => credential.credential_type === CredentialTypeEnum.API_KEY,
  )
  ...
  const openConfirm = useCallback(
    (credentialId?: string) => {
      ...
      if (credentialId) pendingOperationCredentialIdRef.current = credentialId
  ...
  const handleConfirm = useCallback(async () => {
    if (doingActionRef.current || !canManageCredential) return
  ...
  const handleApiKeyModalOpenChange = useCallback((open: boolean) => {
    ...
    if (!open) pendingOperationCredentialIdRef.current = null
  }, [])
  ...
  const handleSetDefault = useCallback(
    async (id: string) => {
      if (doingActionRef.current || !canUseCredential) return
      try {
        handleSetDoingAction(true)
  ...
  const unavailableCredential = credentials.find(
    (credential) => credential.not_allowed_to_use && credential.is_default,
  )
