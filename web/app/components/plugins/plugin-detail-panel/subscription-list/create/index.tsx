...
import {
  ...
  SelectItemIndicator,
  SelectTrigger,
} from '@crew/crew-ui/select'
import { toast } from '@crew/crew-ui/toast'
import { Tooltip, TooltipContent, TooltipTrigger } from '@crew/crew-ui/tooltip'
...
export const CreateSubscriptionButton = ({
  ...
  const supportedMethods = useMemo(
    () => providerInfo?.supported_creation_methods || [],
    [providerInfo?.supported_creation_methods],
  )
  const { data: oauthConfig, refetch: refetchOAuthConfig } = useTriggerOAuthConfig(
    detail?.provider || '',
    supportedMethods.includes(SupportedCreationMethods.OAUTH),
  )
  ...
  const [
    ...
    { setTrue: showClientSettingsModal, setFalse: hideClientSettingsModal },
  ] = useBoolean(false)

  const buttonTextMap = useMemo(() => {
    return {
      [SupportedCreationMethods.OAUTH]: t(($) => $['subscription.createButton.oauth'], {
        ns: 'pluginTrigger',
      }),
      [SupportedCreationMethods.APIKEY]: t(($) => $['subscription.createButton.apiKey'], {
        ns: 'pluginTrigger',
      }),
      [SupportedCreationMethods.MANUAL]: t(($) => $['subscription.createButton.manual'], {
        ns: 'pluginTrigger',
      }),
  ...
  const handleClientSettingsOpenChange = useCallback(
    (open: boolean) => {
      if (open) {
        showClientSettingsModal()
        return
    ...
    return [
      {
        ...
        tag: !showCustomBadge ? null : (
          <Badge className="mr-0.5 ml-1">{t(($) => $['auth.custom'], { ns: 'plugin' })}</Badge>
        ),
        extra: (
          <Tooltip>
            <TooltipTrigger
            ...
            />
            <TooltipContent>
              {t(($) => $['subscription.addType.options.oauth.clientSettings'], {
                ns: 'pluginTrigger',
              })}
            </TooltipContent>
          </Tooltip>
      ...
      },
      {
        ...
        label: t(($) => $['subscription.addType.options.manual.description'], {
          ns: 'pluginTrigger',
        }),
        extra: (
          <Infotip
  ...
  const shouldAllowSelect =
    methodType === DEFAULT_METHOD ||
    (methodType === SupportedCreationMethods.OAUTH && supportedMethods.length === 1)

  const showCreateModal = useCallback(
    (createInfo: { type: SupportedCreationMethods; builder?: TriggerSubscriptionBuilder }) => {
  ...
  if (!supportedMethods.length) return null
    ...
    <>
      ...
      >
        <SelectTrigger
          ...
          nativeButton={false}
          className={cn(
            'h-8 border-0 bg-transparent px-0 hover:bg-transparent focus-visible:bg-transparent [&>*:last-child]:hidden',
            buttonType === CreateButtonType.FULL_BUTTON && 'grow',
          )}
        >
          {buttonType === CreateButtonType.FULL_BUTTON ? (
            <Button
            ...
            >
              <div className="flex flex-1 items-center justify-center">
                ...
                {methodType === SupportedCreationMethods.OAUTH &&
                  oauthConfig?.custom_enabled &&
                  oauthConfig?.custom_configured && (
                    <Badge className="mr-0.5 ml-1 border-text-primary-on-surface bg-components-badge-bg-dimm text-text-primary-on-surface">
                    ...
                    <TooltipTrigger
                      render={
                        <div onClick={onClickClientSettings} className="p-2">
                          <span
                            aria-hidden
                            className="i-ri-equalizer-2-line size-4 text-components-button-primary-text"
                          />
                        </div>
                      }
                    />
