...
vi.mock('@/config', async (importOriginal) => {
  ...
  return {
    ...actual,
    SUPPORT_INSTALL_LOCAL_FILE_EXTENSIONS: '.crewpkg,.zip',
  }
})
...
vi.mock('@crew/crew-ui/dropdown-menu', async () => {
...
describe('InstallPluginDropdown', () => {
  ...
  it('keeps the trigger visible but disabled when install is unavailable', () => {
    ...
    fireEvent.change(container.querySelector('input[type="file"]')!, {
      target: {
        files: [new File(['content'], 'plugin.crewpkg')],
      },
    })
  ...
  it('opens the local package installer when a file is selected', () => {
    ...
    expect(screen.getByText('plugin.crewpkg')).toBeInTheDocument()
  })
