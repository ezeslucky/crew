...
const PluginsPicker: FC<Props> = ({ updateMode, value, onChange, integrationCategory }) => {
  ...
  const isCurrentCategoryPlugin = useCallback(
    (pluginId: string) => {
      if (!integrationCategory) return true
  ...
  const handleVisiblePluginsChange = useCallback(
    (newVisiblePlugins: string[]) => {
      onChange(
        integrationCategory
          ? [
              ...hiddenPlugins,
              ...newVisiblePlugins.filter((plugin) => !hiddenPlugins.includes(plugin)),
            ]
          : newVisiblePlugins,
      )
    },
  ...
  return (
    <div className="mt-2 flex w-full flex-col gap-2 rounded-[10px] bg-background-section-burn p-2.5">
      ...
      {hasSelected && <PluginsSelected className="h-6 w-full gap-1" plugins={visiblePlugins} />}
