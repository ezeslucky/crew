...
export const useFieldList = ({
  ...
  const handleSubmitField = useCallback(
    (data: InputVar, moreInfo?: MoreInfo) => {
      const isDuplicate = allVariableNames.some(
        (name) =>
          name === data.variable &&
          name !== inputFieldsRef.current[editingFieldIndex.current]?.variable,
      )
      if (isDuplicate) {
        toast.error(
          t(($) => $['inputFieldPanel.error.variableDuplicate'], { ns: 'datasetPipeline' }),
        )
        return
      }
      ...
      if (moreInfo?.type === ChangeType.changeVarName)
        handleInputVarRename(
          nodeId,
          [VARIABLE_PREFIX, nodeId, moreInfo.payload?.beforeKey || ''],
          [VARIABLE_PREFIX, nodeId, moreInfo.payload?.afterKey || ''],
        )
