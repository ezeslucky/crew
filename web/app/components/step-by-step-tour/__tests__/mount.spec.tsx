...
} from '@crew/contracts/api/console/onboarding/types.gen'
...
vi.mock('react-i18next', async () => {
  ...
  return {
    ...
    ...createReactI18nextMock({
      'common.stepByStepTour.title': 'Get to know Crew',
      ...
      'common.stepByStepTour.guides.home.noCreate.description':
        'You can review lessons and see how Crew works here. Creating an app from a lesson requires a workspace where you have create permission, or help from an admin.',
      'common.stepByStepTour.guides.home.noCreate.title': 'Browse Learn Crew',
      ...
      'common.stepByStepTour.guides.knowledge.empty.create.description':
        'Fastest way to get going. Upload documents and Crew handles chunking, indexing, and embedding for you. You can switch to custom anytime.',
      ...
      'common.stepByStepTour.guides.studio.empty.dsl.description':
        'Got a Crew DSL file? Import it here to restore an app you have shared or backed up.',
      'common.stepByStepTour.guides.studio.empty.dsl.title': 'Import a DSL file',
      'common.stepByStepTour.guides.studio.empty.learnCrew.description':
        'New to Crew? Walk through a guided lesson first.',
      'common.stepByStepTour.guides.studio.empty.learnCrew.title': 'Or start with Learn Crew',
      'common.stepByStepTour.guides.studio.empty.template.description':
        'Browse Crew templates and pick one that matches what you want to build.',
      ...
      'common.stepByStepTour.tasks.home.title': 'Try a Learn Crew lesson',
      'common.stepByStepTour.tasks.home.description':
        'Open a hands-on lesson from Learn Crew to see Crew in action.',
      'common.stepByStepTour.tasks.home.noCreate.title': 'Browse Learn Crew',
...
describe('StepByStepTourMount', () => {
  ...
  it('does not render the checklist when the Step-by-step Tour feature is disabled', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByRole('region', { name: 'Get to know Crew' })).not.toBeInTheDocument()
    })
  })
  ...
  it('renders the checklist for an eligible workspace', async () => {
    ...
    expect(await screen.findByRole('region', { name: 'Get to know Crew' })).toBeInTheDocument()
  })
  ...
  it('restores the checklist after Skip fails and allows retry', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('region', { name: 'Get to know Crew' })).toBeInTheDocument()
      expect(
        screen.queryByRole('region', { name: 'Step-by-step Tour recovery tip' }),
      ).not.toBeInTheDocument()
    })
  ...
  it('uses a three-step checklist when Learn Crew is disabled', async () => {
    ...
    expect(await screen.findByRole('region', { name: 'Get to know Crew' })).toBeInTheDocument()
    expect(screen.queryByText('Try a Learn Crew lesson')).not.toBeInTheDocument()
  ...
  it('treats the three-step tour as complete when Learn Crew is disabled', async () => {
    mockEnableLearnApp.value = false
    setStepByStepTourTestState({
      manuallyEnabledWorkspaceIds: ['workspace-1'],
      manuallyDisabledWorkspaceIds: [],
  ...
  it('starts the home guide against the Learn Crew target', async () => {
    setStepByStepTourTestState({
      manuallyEnabledWorkspaceIds: ['workspace-1'],
      manuallyDisabledWorkspaceIds: [],
      minimized: false,
  ...
  it('uses no-create Learn Crew copy when the workspace cannot create apps', async () => {
    mockWorkspacePermissionKeys.value = []
    setStepByStepTourTestState({
      manuallyEnabledWorkspaceIds: ['workspace-1'],
      manuallyDisabledWorkspaceIds: [],
    ...
    try {
      ...
      expect(await screen.findByText('Browse Learn Crew')).toBeInTheDocument()
      expect(
        screen.getByText(
          'You can review lessons and see how Crew works here. Creating an app from a lesson requires a workspace where you have create permission, or help from an admin.',
        ),
      ...
      expect(mockRouterPush).toHaveBeenCalledWith('/')
      expect(await screen.findByRole('region', { name: 'Browse Learn Crew' })).toBeInTheDocument()
      expect(
        screen.getByText(
          'You can review lessons and see how Crew works here. Creating an app from a lesson requires a workspace where you have create permission, or help from an admin.',
        ),
  ...
  it('only allows users to uncomplete tasks from the checklist status control', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Mark Try a Learn Crew lesson incomplete' }),
    )
  ...
  it('does not allow manually completing externally completed tasks from the checklist', async () => {
    ...
    const completeHomeButton = await screen.findByRole('button', {
      name: 'Mark Try a Learn Crew lesson complete',
    })
  ...
  it('skips the optional Learn Crew guide when the Studio empty section is not rendered', async () => {
    mockPathname = '/apps'
    setStepByStepTourTestState({
      activeTaskId: 'studio',
      activeGuideIndex: 2,
