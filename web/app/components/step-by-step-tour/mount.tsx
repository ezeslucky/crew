...
import { Popover, PopoverContent } from '@crew/crew-ui/popover'
...
export default function StepByStepTourMount({ className }: StepByStepTourMountProps) {
  ...
  const learnCrewEnabled = systemFeatures?.enable_learn_app ?? true
  const stepByStepTourFeatureEnabled = Boolean(systemFeatures?.enable_step_by_step_tour)
  const availableTasks = learnCrewEnabled
  ...
  const activeGuidePlacement =
    activeGuide?.target === STEP_BY_STEP_TOUR_TARGETS.studioEmptyLearnCrew
      ? 'top'
