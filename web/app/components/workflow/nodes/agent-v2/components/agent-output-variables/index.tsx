import type { DeclaredOutputConfig } from '@crew/contracts/api/console/apps/types.gen'
  ...
  function handleConfirm(output: DeclaredOutputConfig, state: EditingState) {
    if (typeof state.outputIndex === 'number' && state.parentPath) {
      ...
      onChange(
        outputs.map((item, outputIndex) =>
          outputIndex === state.outputIndex
            ? insertOutputChildAtPath(item, state.parentPath!, childOutput)
            : item,
        ),
      )
      ...
      onChange(
        outputs.map((item, outputIndex) =>
          outputIndex === state.outputIndex
            ? updateOutputChildAtPath(item, state.childPath!, childOutput)
            : item,
        ),
      )
    } else if (typeof state.outputIndex === 'number') {
      onChange(
        outputs.map((item, outputIndex) => (outputIndex === state.outputIndex ? output : item)),
      )
    } else {
      ...
      <>
        {children.map((child, childIndex) => {
          ...
          const isEditingChild =
            editingState?.outputIndex === outputIndex &&
            pathsEqual(editingState.childPath, childPath)
          return (
            <div
              key={`${childPath.join('.')}-${child.name}-${getOutputTypeOptionValue(child)}`}
            ...
            >
              {isEditingChild ? (
                ...
                <ChildOutputFrame depth={depth}>
                  <OutputRow
                    ...
                    editable={editable}
                    onAddChild={
                      editable && canOutputHaveChildren(child)
                        ? () => handleNewChild(outputIndex, childPath)
                        : undefined
                    }
              ...
              {editingState?.outputIndex === outputIndex &&
                pathsEqual(editingState.parentPath, childPath) && (
        ...
        <div className="flex flex-col">
          {outputs.map((output, index) => {
            ...
            const isEditingOutput =
              editingState?.outputIndex === index &&
              !editingState.childPath &&
              ...
              >
                {isEditingOutput ? (
                  ...
                  <OutputRow
                    ...
                    editable={editable}
                    onAddChild={
                      editable && canOutputHaveChildren(output)
                        ? () => handleNewChild(index, [])
                        : undefined
                    }
                ...
                {editingState?.outputIndex === index &&
                  editingState.parentPath &&
                  !editingState.parentPath.length && (
