...
import { SegmentedControl, SegmentedControlItem } from '@crew/crew-ui/segmented-control'
...
export function DisplayContent(props: DisplayContentProps) {
  ...
  const viewOptions = [
    {
      value: ViewMode.Code,
      label: t(($) => $['nodes.templateTransform.code'], { ns: 'workflow' }),
      iconClassName: 'i-ri-braces-line',
    },
    {
      value: ViewMode.Preview,
      label: t(($) => $['common.preview'], { ns: 'workflow' }),
  ...
  const chunkType = useMemo(() => {
    ...
    if (schemaType === 'parent_child_structure') return ChunkingMode.parentChild
    if (schemaType === 'qa_structure') return ChunkingMode.qa
  }, [previewType, schemaType])

  const parentMode = useMemo(() => {
    if (previewType !== PreviewType.Chunks || !schemaType || !jsonString) return undefined
    ...
    >
      ...
      <div className="flex flex-1 overflow-auto rounded-b-[10px] pr-1 pl-3">
        ...
        {selectedViewMode === ViewMode.Preview &&
          (previewType === PreviewType.Markdown ? (
            <Markdown
              className="grow overflow-auto rounded-lg px-4 py-3"
              content={(mdString ?? '') as string}
            />
          ) : (
            <ChunkCardList
              chunkType={chunkType!}
