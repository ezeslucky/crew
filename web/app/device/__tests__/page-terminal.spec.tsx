...
describe('error_sso dedicated view', () => {
  ...
  const EMAIL_COPY = 'deviceFlow.ssoError.emailBelongsToCrewAccount'
  ...
  it('shows the email special-case copy', async () => {
    mockSearchParams = { sso_error: 'email_belongs_to_crew_account', user_code: 'ABCD-3456' }
    render(<DevicePage />)
  ...
  it('never surfaces the raw backend code', async () => {
    ...
    expect(screen.queryByText('email_belongs_to_crew_account')).not.toBeInTheDocument()
  })
