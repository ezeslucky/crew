...
describe('useDocLink', () => {
  ...
  describe('Base URL handling', () => {
    ...
    it('should remove trailing slash from base URL', () => {
      const baseUrlWithSlash = 'https://docs.crew.ai/'
      const { result } = renderHook(() => useDocLink(baseUrlWithSlash))
      const url = result.current('/use-crew/getting-started/introduction')
      expect(url).toBe('https://docs.crew.ai/en/cloud/use-crew/getting-started/introduction')
    })

    it('should handle base URL without trailing slash', () => {
      const baseUrlWithoutSlash = 'https://docs.crew.ai'
  ...
  describe('Path handling', () => {
    it('should handle path parameter', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/getting-started/introduction')
      expect(url).toBe(`${defaultDocBaseUrl}/en/cloud/use-crew/getting-started/introduction`)
    })
    ...
    it('should keep explicit product docs path without adding another product prefix', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current(
        '/cloud/use-crew/getting-started/introduction' as DocPathWithoutLang,
      )
      expect(url).toBe(`${defaultDocBaseUrl}/en/cloud/use-crew/getting-started/introduction`)
    })
  })
  ...
  describe('PathMap handling', () => {
    it('should use path from pathMap when locale matches', () => {
      ...
      const pathMap: DocPathMap = {
        'zh-Hans': '/use-crew/getting-started/introduction',
        'en-US': '/use-crew/workspace/tools#mcp',
      }

      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/workspace/tools#mcp', pathMap)
      expect(url).toBe(`${defaultDocBaseUrl}/zh/cloud/use-crew/getting-started/introduction`)
    })
    ...
    it('should use default path when locale not in pathMap', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/workspace/tools#mcp', pathMap)
      expect(url).toBe(`${defaultDocBaseUrl}/ja/cloud/use-crew/workspace/tools#mcp`)
    })

    it('should handle undefined pathMap', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/getting-started/introduction', undefined)
      expect(url).toBe(`${defaultDocBaseUrl}/en/cloud/use-crew/getting-started/introduction`)
    })
  })
  ...
  describe('Product prefix handling', () => {
    it('should add cloud product prefix for product docs available in both editions', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/workspace/tools#mcp')
      expect(url).toBe(`${defaultDocBaseUrl}/en/cloud/use-crew/workspace/tools#mcp`)
    })
    ...
    it('should add self-host product prefix for product docs available in both editions outside cloud edition', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/workspace/tools#mcp')
      expect(url).toBe(`${defaultDocBaseUrl}/en/self-host/use-crew/workspace/tools#mcp`)
    })
    ...
    it('should use the existing cloud docs path for cloud-only product docs outside cloud edition', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/workspace/subscription-management#crew-for-education')
      expect(url).toBe(
        `${defaultDocBaseUrl}/en/cloud/use-crew/workspace/subscription-management#crew-for-education`,
      )
    })
    ...
    it('should use the self-host Start node docs path outside cloud edition', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/nodes/start')
      expect(url).toBe(`${defaultDocBaseUrl}/en/self-host/use-crew/nodes/start`)
    })
    ...
    it('should not add a product prefix for unknown productless paths', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-crew/unknown-page' as DocPathWithoutLang)
      expect(url).toBe(`${defaultDocBaseUrl}/en/use-crew/unknown-page`)
    })
    ...
    it('should keep self-host deploy paths without adding use-crew product prefix', () => {
      mockConfig.IS_CLOUD_EDITION = true
  ...
  describe('Edge Cases', () => {
    it('should handle path with anchor', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current(
        '/use-crew/getting-started/introduction#overview' as DocPathWithoutLang,
      )
      expect(url).toBe(
        `${defaultDocBaseUrl}/en/cloud/use-crew/getting-started/introduction#overview`,
      )
    })

    it('should handle multiple calls with same hook instance', () => {
      const { result } = renderHook(() => useDocLink())
      const url1 = result.current('/use-crew/getting-started/introduction')
      const url2 = result.current('/use-crew/workspace/tools#mcp')
      expect(url1).toBe(`${defaultDocBaseUrl}/en/cloud/use-crew/getting-started/introduction`)
      expect(url2).toBe(`${defaultDocBaseUrl}/en/cloud/use-crew/workspace/tools#mcp`)
    })
  })
})
