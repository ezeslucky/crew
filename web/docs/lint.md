...
```sh
vp check web/app/components packages/crew-ui/src/button
vp check --fix web/app/components packages/crew-ui/src/button
```
...
| Override-scoped settings | The three Crew UI Tailwind rules are disabled with the rest of ESLint's code path. Oxlint still applies the web `react-x.additionalStateHooks` setting globally because it cannot scope settings to an override. |
| Oxlint disable severity  | Oxlint only accepts `reportUnusedDisableDirectives` at the root, where it remains `warn`; the former Crew UI-specific ESLint `error` severity is no longer applied to code files.                                |
