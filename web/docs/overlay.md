...
Use `@crew/crew-ui/*` primitives for overlays in new and modified web
...
- Use `@crew/crew-ui/dialog` for modal surfaces that need focus
  management, scroll locking, escape handling, and outside-press dismissal.
- Use `@crew/crew-ui/alert-dialog` only for destructive or must-confirm
  decisions.
- Use `@crew/crew-ui/drawer` for side panels, setup panels, and nested
  editor panels that must behave like a drawer. Do not add separate web drawer
  wrappers.
- Use `@crew/crew-ui/popover` or the web `Infotip` wrapper for
  explanatory content, long help text, rich layout, or interactive content.
- Use `@crew/crew-ui/tooltip` only for short, non-interactive labels where
...
All body-portalled Crew UI overlays use `z-50`. Toast uses `z-60`. The app root
