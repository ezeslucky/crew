...
This document is the single source of truth for automated frontend tests under `web/` and `packages/crew-ui/`. Tests should protect product behavior and make refactoring safer. They are not a file-by-file completion exercise.
...
- Follow `packages/crew-ui/README.md` for the Storybook and Vitest boundary of Crew UI primitives.
...
Mocks must preserve the public contract needed by the test. Do not replace Crew UI or legacy base primitives with semantically inaccurate stubs just to make a test easier.
...
- Tests under `packages/crew-ui/` use separate Vitest Browser Mode projects: unit specs load the package styles through `vitest.setup.ts`, while Storybook tests run stories through `@storybook/addon-vitest`.
