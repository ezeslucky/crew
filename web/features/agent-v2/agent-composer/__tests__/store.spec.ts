import type { AgentSoulConfig } from '@crew/contracts/api/console/agent/types.gen'
...
describe('agent composer store conversions', () => {
  ...
  it('should hydrate editable form state from an AgentSoulConfig and preserve it in the config snapshot', () => {
    ...
    expect(publishConfig.tools?.crew_tools).toEqual([
      expect.objectContaining({
        provider: 'DuckDuckGo',
        provider_id: 'duckduckgo',
        provider_type: 'builtin',
    ...
    expect(publishConfig.tools?.crew_tools?.[0]).not.toHaveProperty('name')
