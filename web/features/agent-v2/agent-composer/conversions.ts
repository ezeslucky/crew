import type {
  ...
  AgentSoulConfig,
} from '@crew/contracts/api/console/agent/types.gen'
import type {
  AgentCliTool,
  AgentComposerModel,
  AgentFileNode,
...
type AgentSoulCrewToolConfig = NonNullable<
  NonNullable<AgentSoulConfig['tools']>['crew_tools']
>[number]
...
type AgentSoulToolRuntimeParameterValue = NonNullable<
  AgentSoulCrewToolConfig['runtime_parameters']
>[string]
...
const getCrewToolActionId = (tool: AgentSoulCrewToolConfig) =>
  `${tool.provider_id ?? tool.provider ?? tool.plugin_id ?? 'provider'}:${tool.tool_name ?? tool.name ?? 'tool'}`

const toCredentialVariant = (tool: AgentSoulCrewToolConfig) => {
...
const toProviderToolFormState = (
  ...
  for (const tool of config?.tools?.crew_tools ?? []) {
    ...
    const actionId = getCrewToolActionId(tool)
...
export const formStateToAgentSoulConfig = ({
  ...
  return {
    ...
    tools: {
      ...baseConfig?.tools,
      crew_tools: toCrewToolConfigs(formState.tools, formState.toolSettings),
