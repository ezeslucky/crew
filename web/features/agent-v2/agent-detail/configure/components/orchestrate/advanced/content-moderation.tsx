...
import type { AgentSoulAppFeaturesConfig } from '@crew/contracts/api/console/agent/types.gen'
...
function AgentContentModerationSettingsContent() {
  ...
  const persistModeration = useCallback(
    (nextModeration?: ModerationConfig) => {
      if (!nextModeration) return
      ...
      setAppFeatures((appFeatures) => ({
        ...
        sensitive_word_avoidance:
          nextModeration as AgentSoulAppFeaturesConfig['sensitive_word_avoidance'],
      }))
    },
  ...
  const openSettings = useCallback(
    (payload: ModerationConfig = moderation as ModerationConfig) => {
  ...
  const providerContent = useMemo(() => {
    ...
    return (
      codeBasedExtensionList?.data.find((item) => item.name === moderation?.type)?.label[locale] ||
      '-'
    )
