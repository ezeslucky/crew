      ...
      <EnvEditorCell role="cell" className="justify-center">
        {editable && (
          <button
            type="button"
            aria-label={t(
              ($) => $['agentDetail.configure.advancedSettings.envEditor.deleteVariable'],
              {
                key:
                  variable.key ||
...
export function AgentEnvEditor() {
  ...
  const handleImportEnvVariables = async (file: File) => {
    const { invalidLineCount, variables } = parseEnvImport(await file.text())
    const importedVariables = variables.map(createEnvVariableFromEntry)

    if (invalidLineCount > 0) {
      toast.error(
        t(($) => $['agentDetail.configure.advancedSettings.envEditor.importSkippedInvalidLines'], {
          ...
          <>
            <input
              ...
              onChange={(event) => {
                ...
                if (file) void handleImportEnvVariables(file)
              }}
            />
            <Tooltip>
              <TooltipTrigger
