...
const toAgentTool = (tool: AgentSoulCrewToolConfig) => ({
...
export const buildChatConfig = ({
  ...
  const crewTools = agentSoulConfig?.tools?.crew_tools ?? []
  ...
  return {
    ...
    agent_mode: {
      enabled: crewTools.length > 0 || cliTools.length > 0,
      strategy: AgentStrategy.functionCall,
      tools: [
        ...crewTools.map(toAgentTool),
