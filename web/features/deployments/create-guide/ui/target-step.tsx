...
import type { Environment } from '@crew/contracts/enterprise/types.gen'
import { Button } from '@crew/crew-ui/button'
import { cn } from '@crew/crew-ui/cn'
import { RadioControl, RadioGroup, RadioItem } from '@crew/crew-ui/radio'
...
import { EnvVarBindingsPanel } from '@/features/deployments/shared/components/env-var-bindings'
import { RuntimeCredentialBindingsPanel } from '@/features/deployments/shared/components/runtime-credential-bindings'
...
function TargetEnvironmentSection() {
  ...
  const isEnvironmentLoading =
    environmentsIsLoading || (environmentsIsFetching && environments.length === 0)
...
function TargetBindingSection() {
  ...
  const isBindingLoading =
    deploymentOptionsIsLoading || (deploymentOptionsIsFetching && !deploymentOptions)
  ...
  return (
    <RuntimeCredentialBindingsPanel
      ...
      bindingCountLabel={t(($) => $['createGuide.target.bindingCount'], {
        count: bindingSlots.length,
      })}
...
function TargetEnvVarSection() {
  ...
  return (
    <EnvVarBindingsPanel
      ...
      envVarCountLabel={t(($) => $['createGuide.target.envVarCount'], {
        count: envVarSlots.length,
      })}
      onChange={setEnvVar}
      listScrollable={false}
      className="border-components-option-card-option-border bg-components-option-card-option-bg"
