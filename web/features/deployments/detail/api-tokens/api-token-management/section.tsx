...
import type { ApiKey, Environment } from '@crew/contracts/enterprise/types.gen'
...
function DeveloperApiEndpoint({ apiUrl }: { apiUrl: string }) {
  const { t } = useTranslation('deployments')
...
export function DeveloperApiSection() {
  ...
  const visibleCreatedApiToken =
    createdApiToken && createdApiToken.appInstanceId === appInstanceId
  ...
  return (
    <div className="flex flex-col gap-4">
      {apiUrl && <DeveloperApiEndpoint apiUrl={apiUrl} />}
