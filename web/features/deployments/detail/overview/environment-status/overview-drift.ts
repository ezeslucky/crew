import type { EnvironmentDeployment, Release } from '@crew/contracts/enterprise/types.gen'
...
export function computeDrift(row: EnvironmentDeployment): Drift {
  if (isUndeployedDeploymentRow(row)) return { kind: 'undeployed' }

  if (!row.currentRelease) return { kind: 'unknown' }
  ...
  if (behind == null) return { kind: 'unknown' }
  if (behind === 0) return { kind: 'up-to-date' }
