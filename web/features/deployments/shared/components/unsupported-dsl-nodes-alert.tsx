...
function translatedNodeType(
  ...
  if (!type) return tDeployments(($) => $['unsupportedDslNodes.unknownType'])

  if (!isWorkflowBlockType(type)) return type
...
function unsupportedNodeTypeLabels(
  ...
  return Array.from(
    new Set(nodes.map((node) => translatedNodeType(node.type, tDeployments, tWorkflow))),
  )
}
...
export function UnsupportedDslNodesAlert({
  nodes,
  className,
}: {
  nodes: UnsupportedDslNode[]
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <span
          aria-hidden
          className="mt-0.5 i-ri-error-warning-fill size-4 shrink-0 text-text-destructive"
        />
        <div className="min-w-0 grow">
          <div className="system-sm-semibold text-text-primary">
            {t(($) => $['unsupportedDslNodes.title'])}
          </div>
          <p className="mt-0.5 system-xs-regular text-text-secondary">{description}</p>
        </div>
      </div>
    </div>
  )
