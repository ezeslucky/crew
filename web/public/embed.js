 ...
 * the crewChatbotConfig should be defined in the html file before this script is included
 * the crewChatbotConfig should contain the token of the chatbot
...
;(function () {
  ...
  const configKey = 'crewChatbotConfig'
  const buttonId = 'crew-chatbot-bubble-button'
  const iframeId = 'crew-chatbot-bubble-window'
  ...
  const originalIframeStyleText = `
    ...
    top: unset;
    right: var(--${buttonId}-right, 1rem); /* Align with crew-chatbot-bubble-button. */
    bottom: var(--${buttonId}-bottom, 1rem); /* Align with crew-chatbot-bubble-button. */
    left: unset;
    width: 24rem;
  ...
  async function embedChatbot() {
    ...
    const baseUrl = config.baseUrl || `https://${config.isDev ? 'dev.' : ''}ucrew.app`
    ...
    function createIframe() {
      ...
      iframe.title = 'crew chatbot bubble window'
      iframe.id = iframeId
    ...
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === 'crew-chatbot-iframe-ready') {
        targetIframe.contentWindow?.postMessage(
          {
            type: 'crew-chatbot-config',
            payload: {
      ...
      if (event.data.type === 'crew-chatbot-expand-change') {
        toggleExpand()
      }
    })
