...
//
// This script fetches the docs.json from Crew-docs repository
...
const DEFAULT_DOCS_JSON_URL =
  'https://raw.githubusercontent.com/ezeslucky/crew-docs/refs/heads/main/docs.json'
...
function generateTypeDefinitions(
  ...
  for (const [section, pathsSet] of Object.entries(groups)) {
    ...
    // Add UseCrewNodesPath helper type after UseCrewPath
    if (section === 'use-crew') {
      lines.push('// UseCrew node paths (without prefix)')
      /* oxlint-disable no-template-curly-in-string */
      lines.push(
        'type ExtractNodesPath<T> = T extends `/use-crew/nodes/${infer Path}` ? Path : never',
      )
      /* oxlint-enable no-template-curly-in-string */
      lines.push('export type UseCrewNodesPath = ExtractNodesPath<UseCrewPath>')
      lines.push('')
    }
  }
