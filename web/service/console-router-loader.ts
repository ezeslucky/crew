...
import { contractLoaders } from '@crew/contracts/api/console/orpc.gen'
...
async function loadEnterpriseContract(): Promise<AnyContractRouter> {
  const { contract } = await import('@crew/contracts/enterprise/orpc.gen')
  return { enterprise: contract }
}

async function loadKnowledgeFsContract(): Promise<AnyContractRouter> {
  const { contract } = await import('@crew/contracts/knowledge-fs/orpc.gen')
