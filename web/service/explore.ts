import type {
  BannerListResponse,
  BannerResponse,
  GetExploreAppsLearnCrewResponse,
  GetExploreAppsResponse,
  RecommendedAppDetailResponse,
  RecommendedAppInfoResponse,
  RecommendedAppResponse,
} from '@crew/contracts/api/console/explore/types.gen'
import type {
...
} from '@crew/contracts/api/console/installed-apps/types.gen'
...
const normalizeLearnCrewAppsResponse = (
  response: GetExploreAppsLearnCrewResponse,
): LearnCrewAppsResponse => {
  return {
    recommended_apps: response.recommended_apps.map(normalizeRecommendedApp),
  }
}
...
export const fetchLearnCrewAppList = (language?: string) => {
  if (!language)
    return consoleClient.explore.apps.learnCrew.get({}).then(normalizeLearnCrewAppsResponse)

  return consoleClient.explore.apps.learnCrew
    .get({
      query: { language },
    })
    .then(normalizeLearnCrewAppsResponse)
}
