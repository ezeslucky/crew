...
export const useLearnCrewAppList = () => {
  const locale = useLocale()
  const learnCrewAppsInput = locale ? { query: { language: locale } } : {}
  const learnCrewAppsLanguage = learnCrewAppsInput?.query?.language

  return useQuery({
    queryKey: [
      ...consoleQuery.explore.apps.learnCrew.get.queryKey({ input: learnCrewAppsInput }),
      learnCrewAppsLanguage,
    ],
    queryFn: async () => {
      const { recommended_apps } = await fetchLearnCrewAppList(learnCrewAppsLanguage)
