...
function isTrustedCrewHostname(hostname: string) {
  return hostname === 'crew.ai' || hostname.endsWith('.crew.ai')
}
...
function validateLoginRedirectTarget(
  ...
  if (url.protocol !== 'https:' || url.port !== '' || !isTrustedCrewHostname(url.hostname))
    return null
